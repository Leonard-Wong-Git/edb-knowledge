...
| 📚 **指引文件庫** | 148 份官方 EDB 指引（in-app 瀏覽庫；公開 `guidelines.json` 端點為 39 份子集），按類別 → 子類別 → 年份三層分組導覽 |
...
> 註：`148` 為 in-app 指引瀏覽庫（總文件基礎）；公開 `guidelines.json` API 端點為其子集 **39** 份（公開契約）。
