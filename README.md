...
| 功能 | 說明 |
|------|------|
| ℹ️ **平台介紹** | 平台定位、動態統計（455 已核實事實 / 17,602 知識片段 / 177 指引 / 288 來源，由 `_meta.stats` 動態取）、核心功能說明 |
| 🔍 **政策搜尋** | Channel-B 語義搜尋：從 17,602 個 EDB 官方原文知識片段檢索，整理回答並附原始文件出處（含頁碼） |
...
> 系統架構總覽（RAG）：用戶 → 前端（app.html / React SPA / GitHub Pages）→ 後端（Render · Node.js + TypeScript · API Gateway）→ RAG 核心（Query 嵌入 → 主題路由 → 向量搜尋 → LLM 合成）→ 知識庫（Supabase pgvector 17,602 chunks + 455 已核實事實）；外部：OpenAI（Azure 備援）、Cloudflare 免 Cookie 統計。
...
| 來源文件 | **120 份** |
| Channel B 向量索引 | **17,602 chunks**（Supabase pgvector，線上） |
