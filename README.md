...
[![Platform](https://img.shields.io/badge/platform-v3.3.7-teal)](CHANGELOG.md)
...
| 功能 | 說明 |
|------|------|
| ℹ️ **平台介紹** | 平台定位、動態統計（455 已核實事實 / 17,517 知識片段 / 170 指引 / 300 來源；片段與來源數由 `_meta.stats` 取，指引數由 `GUIDELINES_REGISTRY.length` 即時計）、核心功能說明 |
...
> 註：`170` 為 in-app 指引瀏覽庫（v3.3.0 由 166 升至 177；v3.3.4 移除 1 個、v3.3.5 再移除 5 個「同一份文件掛兩個 id」的重複條目後為 171；v3.3.7 退役 `g24` 後為 170）；公開 `guidelines.json` API 端點為其全集投影 **151** 份（v2.6.2 起對齊 in-app 標籤，移除 7 條重複／已退役條目）。兩者仍刻意不同，非漂移。
