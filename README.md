...
| 功能 | 說明 |
|------|------|
| ℹ️ **平台介紹** | 平台定位、動態統計（455 已核實事實 / 14,505 知識片段 / 161 指引 / 120 來源，由 `_meta.stats` 動態取）、核心功能說明 |
| 🔍 **政策搜尋** | Channel-B 語義搜尋：從 14,505 個 EDB 官方原文知識片段檢索，整理回答並附原始文件出處（含頁碼） |
| 📄 **文件分析** | 上載學校文件（PDF / Word / 貼文字），逐段比對 EDB 官方指引並附相關指引內容、來源連結及頁碼；檔案在瀏覽器內抽取文字，原始檔案不上載（S154） |
...
| 來源文件 | **120 份** |
| Channel B 向量索引 | **14,505 chunks**（Supabase pgvector，線上） |
