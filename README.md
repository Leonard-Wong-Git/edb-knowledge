...
| 功能 | 說明 |
|------|------|
| ℹ️ **平台介紹** | 平台定位、動態統計（455 已核實事實 / 14,674 知識片段 / 161 指引 / 120 來源，由 `_meta.stats` 動態取）、核心功能說明 |
| 🔍 **政策搜尋** | Channel-B 語義搜尋：從 14,674 個 EDB 官方原文知識片段檢索，整理回答並附原始文件出處（含頁碼） |
| 📄 **文件分析** | 上載學校文件（PDF / Word / 貼文字），逐段比對 EDB 官方指引並附相關指引內容、來源連結及頁碼；檔案在瀏覽器內抽取文字，原始檔案不上載。**S159 Phase 2**：可選本校校類（小/中/特/幼）令提示按校類調整，並可下載「標註版 Word」（原文逐段附指引引用，於瀏覽器內生成、不上載）（S154→S159） |
| 📚 **指引文件庫** | 161 份官方 EDB 指引（in-app 瀏覽庫；公開 `guidelines.json` 端點為 152 份全集投影），按類別 → 子類別 → 年份三層分組導覽 |

> 公眾 app（`app.html`）為 **Channel-B-only 唯讀** 四 tab 介面（S151 移除 admin；S154 加入文件分析）。Channel A 人工策展 / 管理員登入功能已移除；`role_facts.json` / `knowledge.json` 凍結 @455 仍作對外資料契約。
...
| 來源文件 | **120 份** |
| Channel B 向量索引 | **14,674 chunks**（Supabase pgvector，線上） |
