...
[![Platform](https://img.shields.io/badge/platform-v3.3.6-teal)](CHANGELOG.md)
...
| 功能 | 說明 |
|------|------|
| ℹ️ **平台介紹** | 平台定位、動態統計（455 已核實事實 / 17,517 知識片段 / 171 指引 / 300 來源；片段與來源數由 `_meta.stats` 取，指引數由 `GUIDELINES_REGISTRY.length` 即時計）、核心功能說明 |
| 🔍 **政策搜尋** | Channel-B 語義搜尋：從 17,517 個 EDB 官方原文知識片段檢索，整理回答並附原始文件出處（含頁碼） |
...
| 📚 **指引文件庫** | 171 份官方 EDB 指引（in-app 瀏覽庫；公開 `guidelines.json` 端點為 158 份全集投影），按類別 → 子類別 → 年份三層分組導覽 |
...
`mobile.css` + `mobile.js` 偵測 **視窗闊度 ≤640px** 時啟用手機 shell。**S211 起唔再睇裝置 UA**：`mobile.css` 全份規則都喺 `@media (max-width: 640px)` 之內，舊有「或手機 UA」一閘令 iPad／平板／手機橫放（UA 係手機但闊過 640px）建出一個冇樣式嘅 shell，將真身 app 推低約 362px。闊過 640px 嘅流動裝置現改用響應式桌面版面。

底部導航入口由 `window.FEATURE_TABS` 過濾，現行 **3 個**：🔍 搜尋 / 📚 指引文件 / ℹ️ 平台介紹（📋 範本下載 隨 S204 收起）。首次開啟顯示 **4 步導覽（onboarding tour）**，開場白嘅功能數目由過濾後入口數推算（S211，唔再寫死），再引導選擇崗位角色（`k1_mobile_tour_v1` localStorage gate）。

> ⚠️ `mobile.css` / `mobile.js` 喺 4 個 HTML 以 `?v=<PLATFORM_VERSION>` 引用作快取鍵。**改動呢兩個檔必須同時推 `PLATFORM_VERSION`**，否則回頭客會繼續讀快取舊檔（S211 實際撞過，差啲推咗個空氣修正）。

| 功能 | 手機 | 桌面 |
|------|:----:|:----:|
| 政策搜尋 / 指引文件 / 平台介紹 | ✅ 完整 | ✅ |
| 📋 ~~範本下載~~〔S204 起收起〕 | — 底欄已無此入口 | — 分頁已收起 |
...
> 系統架構總覽（RAG）：用戶 → 前端（app.html / React SPA / GitHub Pages）→ 後端（Render · Node.js + TypeScript · API Gateway）→ RAG 核心（Query 嵌入 → 主題路由 → 向量搜尋 → LLM 合成）→ 知識庫（Supabase pgvector 17,517 chunks + 455 已核實事實）；外部：OpenAI（Azure 備援）、Cloudflare 免 Cookie 統計。
...
| 指引文件庫（in-app 瀏覽） | **171 份** |
| 來源文件 | **300 份**（Supabase distinct `source_id` 307 扣 7 個 `role_facts_*` 偽來源；`source_registry.json` 登記 281） |
| Channel B 向量索引 | **17,517 chunks**（Supabase pgvector，線上） |

> 註：由 792 條去重整合至 455 條唯一事實（2026-05-16，commit 711f911；可逆日誌 `dev/DEDUP_LOG_2026-05-16.md`）。
> 註：`171` 為 in-app 指引瀏覽庫（v3.3.0 由 166 升至 177；v3.3.4 移除 1 個、v3.3.5 再移除 5 個「同一份文件掛兩個 id」的重複條目後為 171）；公開 `guidelines.json` API 端點為其全集投影 **158** 份。兩者刻意不同，非漂移。
