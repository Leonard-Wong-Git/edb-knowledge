...
| Backend | 編譯正常（`backend/dist/` 已產生）；Channel B = **Supabase pgvector**（`match_wiki_chunks` RPC，非本地 wiki_index）；線上 `https://edb-knowledge.onrender.com`（Render free tier，idle 15min 冷啟 ~30s） | `ls dist` / 讀 `wikiRepository.ts` |
