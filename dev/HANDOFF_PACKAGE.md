...
| 知識三層同步 | `role_facts.json` 與 `dev/knowledge/role_facts.json` **byte-identical（md5 一致）**，`knowledge.json` 同為 v2.3.0；`_meta.stats = {facts:`**455**`, chunks:10736, sources:120, guidelines:39, topics:7}`（2026-05-16 dedup 792→455，commit `711f911`，reversible log `dev/DEDUP_LOG_2026-05-16.md`）。**Supabase wiki_chunks live total = 10,133**（S125 CB-3 Option C broader batch-4 post-replace：S119 B-2 39 + S120 pilot 3 + S122 batch-1 10 + S123 batch-2 10 + S124 batch-3 10 + S125 batch-4 10 = **82 sources page-carried**；S125 net -120 來自 10 batch-4 sources：10 canonical chunker normalization -14~-32%，無 +>50% recovery cap-hit；DELETE 537 INSERT 417；whole-vault page-resolvable **~76.0%**；`knowledge.json._meta.stats.chunks` 仍係 build-time 數字，已 doc-debt drift 但非 blocker）。E.2「三層脫節」風險現時 **clean** ✅ | md5 + JSON parse |
