# QC Review Report — verify_issues Assessment

> Generated: S156 session — 4 surgical modal fixes applied; remainder for Leonard's review

## Summary

| Domain | Total Issues | Fixed (S156) | Needs Leonard Review |
|---|---|---|---|
| sen | 48 | 1 | 47 |
| gifted | 34 | 1 | 33 |
| gov_admin | 23 | 0 | 23 |
| qa_inspection | 23 | 2 | 21 |
| **Total** | **128** | **4** | **124** |

## Issue Types

- **distorted-number**: Scope, terminology, missed verbs, numeric errors, modal distortion (情態詞) — includes obligatory content narrowed/expanded beyond source
- **fabricated**: Rewriter added text (purpose clauses, extra obligations, qualifiers) with no source support
- **bad-citation**: Clause cites a source that doesn't match the covered item's actual source

---

## 特殊教育需要 (SEN) (48 issues)

| # | Status | Kind | Item | Sec | Fix / Action Required |
|---|---|---|---|---|---|
| 0 | 🔍 Review | distorted-number | 0 | — | Restore source wording: Clause 0 covers item 0 but omits the stated purpose of the requirement: '照顧學習差異，提升整體的教學效能'. Item 0 r |
| 1 | 🔍 Review | distorted-number | 12 | — | Restore source wording: Clause 3 covers item 12, whose req requires management to '策劃、執行、監察和評估支援措施' (four actions: plan, imp |
| 2 | 🔍 Review | distorted-number | 17 | — | Restore source wording: Clause 6 covers item 17, which requires strengthening '學校領導人員及教師' (both leadership and teachers) in  |
| 3 | 🔍 Review | distorted-number | 19 | — | Restore source wording: Clause 6 table row 1 merges item 19's content ('適切融入所有教職員的日常工作中，包括將焦點數據收集和觀課活動納入學校發展和課程規劃行動方案') into |
| 4 | 🔍 Review | distorted-number | 8 | — | Restore source wording: Item 8 req and its source quote both state the SEMIS submission deadline as '每學年8月31日前' (strictly be |
| 5 | 🔍 Review | fabricated | 0 | — | Remove or rephrase: Clause 1 writes '本校須維護...學生支援紀錄冊'，但 item 0 的 req 與 quote（g19 p.76）只有'檢視'（review），從無'維護'（maintain）。'維 |
| 6 | 🔍 Review | fabricated | 3 | — | Remove or rephrase: Clause 5 將 item 3 的主體由'中學'改為'本校'，把原條文明確只適用於中學（g19 p.78 quote：'如中學考慮降低學生的支援層級'）的義務，靜默擴展至所有學校，超出源文範圍。 |
| 7 | 🔍 Review | distorted-number | 3 | — | Restore source wording: Clause 5 使用'第三層支援'，但 item 3 req 及 quote（g19 p.78）的原用詞為'第三層級'，非'第三層支援'，術語不對應。 |
| 8 | 🔍 Review | distorted-number | 1 | — | Restore source wording: Clause 0 drops '延展' from item 1's requirement. Original (g10 p39): '決定如何運用及延展中央課程架構所給予的彈性'. Clause 0 |
| 9 | 🔍 Review | distorted-number | 37 | — | Restore source wording: Clause 11 omits the specific mechanism from item 37 (g06 p217): '在放學後設家課輔導小組等，以有效支援有需要的學生'. Neither  |
| 10 | 🔍 Review | distorted-number | 44 | — | Restore source wording: Clause 11 omits the obligation from item 44 (g06 p222): '並提供課後支援予有需要學生'. The clause preserves the pa |
| 11 | 🔍 Review | distorted-number | 49 | — | Restore source wording: Clause 11 renders item 49 (g06 p138) as '減輕學生完成課業的負擔', omitting '焦慮'. The source quote states '減輕他們完 |
| 12 | 🔍 Review | fabricated | 12 | — | Remove or rephrase: Clause 8 (covers [25,31,9,12]; cites g10 p15/p78/p41) adds the sentence '學生的認知能力（包括數學能力）及溝通能力發展應作為個別 |
| 13 | 🔍 Review | distorted-number | 0 | — | Restore source wording: Clause 2 table row「量表填寫」時間寫「12月至2月」，但 item 0 原文（g19 p49）明確記載「12月至1月」。改寫把 item 0 與 item 8 合併入同一表格列，採用 |
| 14 | 🔍 Review | fabricated | 19 | — | Remove or rephrase: Clause 12 加入「協助識別有特殊教育需要的學生，並提供適切的跟進建議」作為校本教育心理服務的用途說明。sag_2025_11 p186 的 quote 只記載 2016/17 學年起提供校本教 |
| 15 | 🔍 Review | distorted-number | 19 | — | Restore source wording: Item 19 req 包含「辦學團體可按規定比例聘用駐校教育心理學家」，此條款在 clause 12 完全缺失，令聘用比例規定這一關鍵要求被遺漏。 |
| 16 | 🔍 Review | distorted-number | 3 | — | Restore source wording: Clause 0 covers id3。id3 quote 原文：「所有學生（包括有殘疾的學生）」，改寫變為「（包括有特殊教育需要的學生）」。《教育實務守則》第17.1段原文類別係「有殘疾的學生」，並 |
| 17 | 🔍 Review | distorted-number | 2 | — | Restore source wording: Clause 5 covers id2。id2 req 明確限定「特殊學校學校人員必須體恤學生的情況並因應其特殊教育需要，提供適切的測考調適或特別考試安排，例如延長考試時間或使用特別輔助器材等」，so |
| 18 | 🔍 Review | distorted-number | 31 | — | Restore source wording: Clause 17 covers id31。id31 req 明確限定「特殊學校應為學生的溝通模式制定校本政策，並提供機會予學生學習理解和表達」，source: g10 p61。改寫改為「本校應為學生 |
| 19 | 🔍 Review | distorted-number | 2 | — | Restore source wording: 源文件 (page 22) quote 寫「改動試題內容或評估準則」，改寫條款改成「改動試題內容或評分準則」。「評估準則」（assessment criteria）與「評分準則」（marking/sc |
| 20 | 🔍 Review | fabricated | 22 | — | Remove or rephrase: 改寫表格（covers 7,21,22,23）為「中度視障學生」一行加入「休息安排：按個別情況安排」。源文件 item 22（page 25）只列明全失明及嚴重視障學生的休息間隔（中學生每45分鐘休息 |
| 21 | 🔍 Review | fabricated | 20 | — | Remove or rephrase: Clause 12 table (covers items 20 & 21) adds '第一層支援（視乎個別情況而定）' as the support tier for 輕度聽障(26–40分貝)  |
| 22 | 🔍 Review | fabricated | 11 | — | Remove or rephrase: Clause 10 (covers items 11 & 13) removes the '特殊學校' scope qualifier present in both original items.  |
| 23 | 🔍 Review | fabricated | 12 | — | Remove or rephrase: Clause 11 (covers item 12) removes the '特殊學校' scope qualifier. Item 12 req: '特殊學校須教導學生表達「不」的否定或拒絕回應， |
| 24 | 🔍 Review | distorted-number | 10 | — | Restore source wording: Item 10 requires '須在中、小學概覽內的專欄' (both primary and secondary school overviews). Clause 0 writes '須在學校 |
| 25 | 🔍 Review | distorted-number | 2 | — | Restore source wording: Item 2 requires the school to '清楚列出所運用的額外資源（包括「學習支援津貼」、「有特殊教育需要非華語學生支援津貼」）及聘請...的具體工作和工作時間表' — two d |
| 26 | 🔍 Review | distorted-number | 16 | — | Restore source wording: Item 16 requires '官立及資助小學' (both government AND aided primary schools) to collect year-6 language-im |
| 27 | 🔍 Review | fabricated | 15 | — | Remove or rephrase: Item 15 source req and quote both require '必須獲得家長同意' (parental consent). Clause 9 upgrades this to ' |
| 28 | 🔍 Review | fabricated | 3 | — | Remove or rephrase: Item 3 source req and quote both say '接受有關特殊教育的培訓' (SEN training). Clause 3 writes '接受有關特殊教育的專業培訓' b |
| 29 | 🔍 Review | bad-citation | 5 | — | Verify correct source citation / Clause 0 covers item 5 (source: g10, page 72) but its citations list only g19/p6 |
| 30 | 🔍 Review | distorted-number | 5 | — | Restore source wording: Item 5 req states the purpose as '以發揮學生在家學習的最大效能' (maximise in-home learning effectiveness). Clause  |
| 31 | ✅ Fixed | distorted-number | 1 | — | 須每年→宜每年 (ch9/clause2) |
| 32 | 🔍 Review | distorted-number | 24 | — | Restore source wording: Item 24 req specifies '成績稍遜學生（小學適用）', making the under-performing-student submission a primary-schoo |
| 33 | 🔍 Review | fabricated | 20 | — | Remove or rephrase: Clause 6 appends '本校在收到上述資料後，須依照本章所訂程序妥善存放及跟進'. Item 20's req and source quote (g19/p48) only descri |
| 34 | 🔍 Review | fabricated | 0 | — | Remove or rephrase: Clause 0 inserts '每學年' (annually), imposing an annual-frequency obligation not present in item 0's r |
| 35 | 🔍 Review | fabricated | 5 | — | Remove or rephrase: Clause 0 uses '（包括管理層及統籌課程的教師）' in '有系統地規劃全校教師（包括管理層及統籌課程的教師）之持續專業發展'. This phrase originates solely |
| 36 | 🔍 Review | fabricated | 5 | — | Remove or rephrase: Clause 0 table row 2 states '由可調批核職級審批出席申請' as an approval procedure for 教育局其他特殊教育需要相關課程. This proce |
| 37 | 🔍 Review | distorted-number | 0 | — | Restore source wording: Item 0 requirement and source quote (g19:60) use '應' (should). Clause 0 uniformly uses '須' (must), u |
| 38 | 🔍 Review | distorted-number | 6 | — | Restore source wording: Item 6 requirement and source quote (g06:133) use '應' (should/参照). Clause 0 uniformly uses '須' (must |
| 39 | 🔍 Review | distorted-number | 7 | — | Restore source wording: Item 7 requirement and source quote (g06:96) use '應' (should). Clause 0 uniformly uses '須' (must), m |
| 40 | 🔍 Review | fabricated | 2 | — | Remove or rephrase: Clause 1（covers 2,5）寫「提升全體教職員的評估素養」，但 g10 p.12（item 2 原文）明確指「學校領導人員及教師」，並非全體教職員。把對象擴大至全體教職員屬無來源新增義務。 |
| 41 | 🔍 Review | fabricated | 4 | — | Remove or rephrase: Clause 2（covers 4）有兩處虛構：(1) 寫「以提升學生生活質素為課程及服務的終極目標」，source（cgss_2024 p.16）只說「以提升學生生活質素為終極目標」，「課程及服務的 |
| 42 | 🔍 Review | bad-citation | 8 | — | Verify correct source citation / Clause 5（covers 8）引用 sag_2025_11 p.104，但 item 8 的 source 僅為 sag_2025_11 p.60。p.1 |
| 43 | 🔍 Review | distorted-number | 0 | — | Restore source wording: 原始義務是「在一個月內送交」，觸發時點為轉校；改寫改為「於學生正式離校後一個月內」，加插「正式離校後」作倒數起點，原文（g19 p82 quote）並無此觸發條件，屬時限錨點改寫。 |
| 44 | 🔍 Review | distorted-number | 12 | — | Restore source wording: 原始 req 及 quote（g10 p52）均為「為學生提供較個人化的課程選擇」，適用對象是特殊學校全體學生；改寫將對象縮窄為「嚴重智障學生」，無任何原文依據，屬對象範圍失真。 |
| 45 | 🔍 Review | fabricated | 21 | — | Remove or rephrase: 改寫在 item 21 對應段末加入「確保支援不因學校轉換而中斷」；原始 req（g06 p15）及 quote 均無此目的語句，屬無來源添加。 |
| 46 | 🔍 Review | fabricated | 4 | — | Remove or rephrase: 改寫在 item 4 對應句末加入「以加強其社區適應能力」；原始 req 及 quote（g10 p72）均只說「編排定期體驗各種社區環境的機會」，無任何目的說明，屬無來源添加。 |
| 47 | 🔍 Review | fabricated | 19 | — | Remove or rephrase: 改寫將「支援學生適應不同階段銜接」延伸為「適應不同階段銜接過程中的身心需要」；原始 quote（g06 p313）只說「提供適切的協助」，req 亦無「身心需要」字眼，屬無來源添加詞語。 |

## 資優教育 (Gifted) (34 issues)

| # | Status | Kind | Item | Sec | Fix / Action Required |
|---|---|---|---|---|---|
| 0 | 🔍 Review | fabricated | 17 | — | Remove or rephrase: Clause 1 將 items 6 & 17 的「優質教育的一環」改寫為「優質教育不可或缺的組成部分」，插入「不可或缺」強化語氣，此詞在兩個原始 item 的 req 及 quote 均無出現，屬無 |
| 1 | 🔍 Review | fabricated | 4 | — | Remove or rephrase: Clause 2 將 item 4 原文「兩個概念」改寫為「兩個核心概念」，「核心」一字在 item 4 的 quote（gifted_tp_resource_kit p.4）中無出現，屬加插修飾詞。 |
| 2 | 🔍 Review | distorted-number | 10 | — | Restore source wording: Clause 3 將 item 10 原文「學校教育宗旨及資源」改寫為「學校教育宗旨及可用資源」，加入「可用」二字縮窄義務範圍，與 item 10 的 req 及 quote 不符。 |
| 3 | 🔍 Review | bad-citation | 18 | — | Verify correct source citation / Clause 7（covers [18]）引用 (gifted_tp_resource_kit, page 7)，但 item 18 的 source_id 為 |
| 4 | 🔍 Review | fabricated | 0 | — | Remove or rephrase: Clause 0 加咗原始 item 0 及 source quote 均無嘅目的語句「以領導全校推行校本資優教育工作」。Item 0 req 只要求校長認同理念及接受培訓，無提培訓目的；source |
| 5 | 🔍 Review | distorted-number | 9 | — | Restore source wording: Item 9 原文用「可因應」（許可式義務，schools MAY adapt），但 Clause 1 將整段框架設為「須完成以下前期分析工作」（強制式），把 item 9 的許可式要求改寫成強制要求 |
| 6 | 🔍 Review | fabricated | 3 | — | Remove or rephrase: Clause 5 table 第三列「第三層次：校外支援（按校情而定）/ 按實際情況統籌安排」在 covers=[10,3,4] 中無任何原始 item 作為來源：item 10 屬第一層次，item |
| 7 | ✅ Fixed | distorted-number | 19 | — | 統籌人員須協調→應協調 (ch2/clause8) |
| 8 | 🔍 Review | fabricated | 22 | — | Remove or rephrase: Clause 9 加咗「以確保資優教育工作具系統性及持續性」目的語句，但 item 22 req 及 source quote 均只列明四個規劃範疇，無提此目標；屬憑空添加的義務性目的語句。 |
| 9 | 🔍 Review | distorted-number | 20 | — | Restore source wording: Item 20 requires naming both preset reports: '依學生報告一、依識別標籤報告二'. Clause 9 (covers [18,19,20,37]) writ |
| 10 | 🔍 Review | bad-citation | — | — | Verify correct source citation / Clause 2 (covers items 52,53) cites {source_id:'g14', page:-1}. Items 52 and 53  |
| 11 | 🔍 Review | bad-citation | 7 | — | Verify correct source citation / Clause 2 covers item 7（source: gifted_policy_docs, page 3）但 clause 2 的 citations |
| 12 | 🔍 Review | fabricated | 8 | — | Remove or rephrase: Clause 4 在結尾加入「以持續優化對資優及高能力學生的支援」。Item 8 的 req（共同商討並作出適切的課程方向與教學策略調整）及 quote（教師之間的溝通和合作；觀察學生在不同的主題或教 |
| 13 | 🔍 Review | fabricated | 3 | — | Remove or rephrase: Clause 1 adds '符合本校學生特質' — a qualifier stating the curriculum must suit the school's own student cha |
| 14 | 🔍 Review | fabricated | 4 | — | Remove or rephrase: Clause 2 point (三) appends '協助其發揮潛能' as the stated purpose of learning/expression-skill training. It |
| 15 | 🔍 Review | fabricated | 10 | — | Remove or rephrase: Clause 5 introduces '訂定個人化的教育目標及學習計劃' — '個人化' (individualised/personalised) does not appear in item  |
| 16 | 🔍 Review | distorted-number | 1 | — | Restore source wording: Item 1 requires '提供輔導活動及特定空間' — two specific concrete provisions (counselling activities AND a dedic |
| 17 | 🔍 Review | distorted-number | 5 | — | Restore source wording: Item 5 uses '宜' (recommended/should), indicating a recommended practice. Clause 3 rewrites as '本校持續推 |
| 18 | 🔍 Review | fabricated | 9 | — | Remove or rephrase: Item 9 and its source quote say '為學生訂立及推行輔導計劃' — applying to students generally. Clause 2 inserts '有 |
| 19 | 🔍 Review | bad-citation | 18 | — | Verify correct source citation / Clause 0 引用 (g06, page 144)，該來源屬於 item 18（「對於雙重特殊資優學生，學校不宜只關注其學習障礙，應以「強項為本」(Stre |
| 20 | 🔍 Review | bad-citation | 14 | — | Verify correct source citation / Clause 5 引用 (gifted_ge_series, page 65)，該來源屬於 item 14（教育局2019年更新《全校參與模式融合教育運作指南》 |
| 21 | 🔍 Review | fabricated | 1 | — | Remove or rephrase: Clause 1 表格「個案管理」行加入「定期評估支援成效」一項義務。Clause 1 所覆蓋的 items [0,1,2,17,18] 的原文引句均無此要求：item 0 quote 只提「度身訂造 |
| 22 | 🔍 Review | fabricated | 7 | — | Remove or rephrase: Clause 7 在 item 7 原有範圍（舉行教師專業培訓活動，增進對雙重特殊資優學生的認識，了解如何處理其困擾和難題）之外，加入「如何在課堂中有效實踐「強項為本」的支援策略」。Item 7 來源 |
| 23 | 🔍 Review | distorted-number | 2 | — | Restore source wording: 原始 item 2 明確列出三個課程層次：(1) 職前師訓的核心課程、(2) 在職師訓課程、(3) 為教師而設的長期發展課程。Clause 0 將「職前師訓的核心課程」整句刪去，改寫為「將資優教育培訓 |
| 24 | 🔍 Review | distorted-number | 7 | — | Restore source wording: 原始 item 7 的 req 及 quote 均明確並列「每所學校、每位教師皆有責任推行資優教育」，學校與教師雙重責任並重。Clause 0 只寫「每位教師均有責任推行資優教育」，刪去「每所學校」， |
| 25 | 🔍 Review | fabricated | 1 | — | Remove or rephrase: Clause 2 加入「本校鼓勵教師修讀該深造課程」一句。原始 item 1 的 req 及 quote（gifted_tp_resource_kit p.16）只描述完成六個模組後頒發電子證書的機制 |
| 26 | 🔍 Review | fabricated | 0 | — | Remove or rephrase: Clause 1 加咗「協助家長深入了解子女的發展需求」，此目的語句在 source（page 31 quote：「提供特別為資優學生家長而設計的親職教育與輔導」）完全不存在，屬無中生有。 |
| 27 | 🔍 Review | fabricated | 0 | — | Remove or rephrase: Clause 1 將 source 原文「困難」改寫為「學習困難」，加入「學習」限定詞，縮窄原意並添加 source 不支持的具體化表述。 |
| 28 | 🔍 Review | fabricated | 2 | — | Remove or rephrase: Clause 2 正文及 table row 2 加入「突發事件」，item 2 的 source quote（page 255）只提「任何新行為」，並無「突發事件」一詞，屬義務/語義擴充。 |
| 29 | 🔍 Review | fabricated | 3 | — | Remove or rephrase: Table row 3（跨專業協調會議）寫「就學生心理或情緒健康風險進行評估」，item 3 的 source（page 253 quote：「家長和教師應該互相協調，並維持一致的期望」）只提協調與諮 |
| 30 | 🔍 Review | distorted-number | 11 | — | Restore source wording: Item 11 quote: '並於2016及2018年共向基金注資共16億元'（明確列出兩個注資年份：2016年及2018年）。Clause 0 改寫為'至2018年共向基金注資16億元'，用'至2 |
| 31 | 🔍 Review | distorted-number | 1 | — | Restore source wording: Item 1 quote: '「學校提名」中增設「校長提名」'（校長提名是學校提名渠道之下的子項目）；item 0 page 23 亦說'學校提名（包括校長提名）'。Clause 1 的表格將「學校提 |
| 32 | 🔍 Review | distorted-number | 15 | — | Restore source wording: Item 15 quote (gifted_policy_docs p7): '例如輔導、參加良師計劃、提早入讀較高班級等'，明確包含'提早入讀較高班級'（跳級／加速入讀）。Clause 7 改寫為' |
| 33 | 🔍 Review | fabricated | 2 | — | Remove or rephrase: Clause 1 (covers item 2, source g14) appends「以照顧不同能力學生的學習需要」after the 拔尖保底 policy statement. This pu |

## 校務行政 (Gov Admin) (23 issues)

| # | Status | Kind | Item | Sec | Fix / Action Required |
|---|---|---|---|---|---|
| 0 | 🔍 Review | distorted-number | 3 | 8 | Restore source wording: Clause 1 merges items 1 and 3 into one sentence and applies '書面同意' (written consent) to item 3's app |
| 1 | 🔍 Review | distorted-number | 4 | 8 | Restore source wording: Clause 2 text reads '修訂圖則及記錄' (及 = and), but item 4's req and source quote both use '修訂圖則／記錄' (slash |
| 2 | 🔍 Review | distorted-number | 2 | 9 | Restore source wording: Clause 2 covers id 2，原始 item 及 quote 寫「校董會」（泛指學校校董會），但 clause 改寫為「法團校董會」（Incorporated Management Com |
| 3 | 🔍 Review | fabricated | 4 | 9 | Remove or rephrase: Clause 3 covers id 4，加入「校名更改獲批後，本校須適時更換相關牌匾，確保所展示的名稱與教育局登記的註冊名稱一致」——此更換義務及一致性要求不見於 source id 4 的 req |
| 4 | 🔍 Review | fabricated | 5 | 9 | Remove or rephrase: Clause 4 covers id 5，原始 req/quote 只說「不得自稱或聲稱為『國際學校』」，clause 額外列明「包括但不限於校名、宣傳材料及對外通訊」三類禁止途徑，此具體列舉不見於  |
| 5 | 🔍 Review | distorted-number | 0 | 10 | Restore source wording: item 0 req 明確列出「資助學校及按位津貼學校」兩類學校，clause_0 改寫為「本校（資助學校）」，完全刪去「按位津貼學校」，縮窄計劃適用範圍。 |
| 6 | 🔍 Review | distorted-number | 5 | 10 | Restore source wording: item 5 quote：「該計劃為學生提供公眾責任及團體人身意外保險的保障」。clause_0 改為「為本校師生及相關人員提供公眾責任及人身意外保障」：(1) 受保對象由「學生」擴大為「師生及相關人 |
| 7 | 🔍 Review | fabricated | 4 | 10 | Remove or rephrase: clause_2 在義務句末加上「以確保其獲得適切保障」，item 4 source（bip_insurance_notes_2025 p2）quote 為「校方應為他們另外投購保險」，原文無任何目的 |
| 8 | 🔍 Review | fabricated | 6 | 10 | Remove or rephrase: clause_3 加上「相關決定由法團校董會按本校實際情況審議」，item 6 source（sag_2025_11 p254）quote 僅為「學校應自行決定是否需要為非學校部分及非標準的家具及設備 |
| 9 | 🔍 Review | distorted-number | 3 | 11 | Restore source wording: 原始 item 3 req/quote 明確列出「校董會／法團校董會服務合約期」兩個選項，改寫 clause 2 只保留「法團校董會服務合約期」，刪去「校董會」選項，縮窄原意。另外原文「表現」被改寫成 |
| 10 | 🔍 Review | distorted-number | 5 | 11 | Restore source wording: 原始 item 5 req/quote 明確列出「學校管理委員會或校董會」作為提交對象，改寫 clause 4 只寫「校董會」，刪去「學校管理委員會」選項，令未設法團校董會的學校適用性失準。 |
| 11 | 🔍 Review | fabricated | 4 | 11 | Remove or rephrase: Clause 3 的 table 「說明」欄為改寫自行加入的描述文字（「闡述本校之整體辦學願景及使命宣言」、「列明本校之具體辦學方向及目標」、「載列本校校訓」、「就本校整體表現作全面回顧及分析」、「列 |
| 12 | 🔍 Review | fabricated | 2 | 12 | Remove or rephrase: Clause 0 加入「負責主持遴選程序」，此目的語句在 item 2 的 req 及 quote 均不存在（source 只說委任校長遴選委員會須向法團校董會負責，並無「主持遴選程序」字眼）。 |
| 13 | 🔍 Review | fabricated | 3 | 12 | Remove or rephrase: Clause 1 table row 4 行動欄寫「告知教師改編職系後的薪金詳情」，在 item 3 的 quote（「把薪金詳情告知稅務局局長」）及 req 中，並無「教師改編職系後的」此限定修飾語 |
| 14 | 🔍 Review | fabricated | 4 | 12 | Remove or rephrase: Clause 3 加入「事先」（「除非事先徵得教育局常任秘書長批准」），item 4 的 quote 原文為「除非徵得教育局常任秘書長批准」，並無「事先」此時序強化語，屬加強義務的虛構添加。 |
| 15 | 🔍 Review | distorted-number | 5 | 13 | Restore source wording: Item 5 source quote says '每隔一段時間查核未完個案的進度'（不定期、視情況而定），clause 7 改寫為'定期查核未完個案的進度'。'定期'含有固定週期之意，與原文'每隔一 |
| 16 | 🔍 Review | fabricated | 4 | 13 | Remove or rephrase: Clause 6 在兩個條件分支之前新增一句'本校在校舍進行商業活動，須事先取得相關批核機構的書面批准。'原始 item 4 quote（p.147）只列兩個具體情況（未設/設有法團校董會），並無任何 |
| 17 | 🔍 Review | fabricated | 9 | 13 | Remove or rephrase: Clause 7 在申報利益後加入'並被禁止接觸相關資料'。Item 9 quote（p.122）只說'該人員應該向校方申報利益'，並無任何關於禁止接觸資料的規定；其餘三個 covered items |
| 18 | 🔍 Review | fabricated | 13 | 13 | Remove or rephrase: Clause 9 將原文'有關法例的規定'擴寫為'《個人資料（私隱）條例》及相關法例的規定'，點名具體法例。Item 13 的 req 及 quote（p.23）均只說'有關法例'，未點名任何具體條例 |
| 19 | 🔍 Review | fabricated | 0 | 14 | Remove or rephrase: Clause 0 opens with「依據《強制舉報虐待兒童條例》」。Items 0、1、12 的 source quote（p.95、p.97）均無提及此條例名稱，屬加入 source 不支持的法 |
| 20 | 🔍 Review | fabricated | 10 | 14 | Remove or rephrase: Clause 5（covers 10, 13）將 source quote「督導、統籌及改善」改寫為「督導、統籌及持續改善」，插入「持續」一字，source p.69 及 p.101 均無此字。 |
| 21 | 🔍 Review | fabricated | 13 | 14 | Remove or rephrase: Clause 5（covers 10, 13）在午膳一小時後加上「以支持學生的健康飲食習慣」目的語句，item 13 source quote（p.101）無此表述。 |
| 22 | 🔍 Review | fabricated | 14 | 14 | Remove or rephrase: Clause 7（covers 14）加入「本校就學生訓育制度制定書面政策」，item 14 的 req 及 source quote（p.73）均無「書面政策」字眼，屬新增 source 不支持的義 |

## 質素保證與視學 (QA) (23 issues)

| # | Status | Kind | Item | Sec | Fix / Action Required |
|---|---|---|---|---|---|
| 0 | 🔍 Review | fabricated | 0 | 1 | Remove or rephrase: Clause 1 第二句「法團校董會為本校最高管治機構，負責監督學校整體運作，確保校政依法施行」屬虛構增添：原始 quote（sag_2025_11 p.14）只說成立法團校董會、由持分者共同參與校政 |
| 1 | 🔍 Review | fabricated | 1 | 1 | Remove or rephrase: Clause 2 table「責任架構」欄兩格均屬虛構：「法團校董會授權校長執行，結果向法團校董會匯報」及「由校長統籌，按年檢視及更新」，原始 quote（sag_2025_11 p.14）只提發展考 |
| 2 | 🔍 Review | fabricated | 1 | 2 | Remove or rephrase: Clause 0 inserts '各發展周期須涵蓋以下主要內容：' — '各發展周期' does not appear in sag_2025_11 p.134 quote ('主要內容包括以下各項 |
| 3 | 🔍 Review | distorted-number | 4 | 3 | Restore source wording: Item 4 原文要求：「教育局亦會監管學校教學質素並作出校外評估」（EDB 主動監管及作客觀校外評估）。Clause 2 正文完全刪去此義務，Table 教育局行僅寫「按教育局要求提交自評報告」，角 |
| 4 | 🔍 Review | fabricated | 3 | 3 | Remove or rephrase: Clause 2 table 列出四類持份者（法團校董會、家長、教職員、教育局）及具體匯報形式與頻次（每年一次、每年至少一次、每學年至少一次、按教育局規定），原始 items 3 及 4 及其引用來源 |
| 5 | 🔍 Review | distorted-number | 0 | 4 | Restore source wording: 體適能表現 在 clause 1 表格被標為「新增項目／現有項目下新增子項目」，但原始 quote 明確指「學校表現評量新增三個項目，亦在部分現有項目下新增子項目」——三個項目（包括體適能表現）全屬新 |
| 6 | 🔍 Review | fabricated | 2 | 4 | Remove or rephrase: clause 2 在覆蓋 item 2 的段落加入「確保評估結果能真實反映本校的實際情況及發展需要」，但 item 2 的 source quote（edbc15_2022_accountabilit |
| 7 | 🔍 Review | fabricated | 3 | 4 | Remove or rephrase: clause 2 在覆蓋 item 3 的段落加入「『要點問題』應作為整全反思的引導框架，而非逐項勾選的核查機制」。item 3 的 source quote（perf_indicators_2022 |
| 8 | ✅ Fixed | distorted-number | 2 | 5 | 須以不記名→應以不記名 (sec5/clause1 table) |
| 9 | 🔍 Review | distorted-number | 4 | 5 | Restore source wording: Item req states '學校須委派合適的教師'（須 = must），but clause2 table row '統籌責任' uses '本校宜委派合適的教師'（宜 = should/rec |
| 10 | 🔍 Review | fabricated | 0 | 6 | Remove or rephrase: 原文 quote 用「不應把學業增值資料用作宣傳用途」（should not，規範性建議），改寫句改為「不得將…用作宣傳用途」（must not，強制禁止）。情態升級無原文支持，屬虛構加強義務。 |
| 11 | 🔍 Review | fabricated | 0 | 7 | Remove or rephrase: Clause 1 (covers 0,1) uses '法定文件'（法定文件）— 原始 item 0 req/quote 只寫「文件」/「相關文件」，無任何「法定」或「statutory」修飾；此字眼 |
| 12 | 🔍 Review | fabricated | 0 | 7 | Remove or rephrase: Clause 1 (covers 0,1) 末句新增「確保上述文件齊備並如期提交」— '如期'（on time）時限義務在 item 0 的 req 及 quote 均無出現，屬虛構義務 |
| 13 | 🔍 Review | fabricated | 2 | 7 | Remove or rephrase: Clause 4 (covers 2) 將原始「效能研究建議向公眾發布…」改寫為「本校須配合教育局就視學報告的透明度要求」— 原文主詞係「效能研究」（研究建議），改寫虛構「教育局」為發令主體，並將研究 |
| 14 | 🔍 Review | fabricated | 3 | 7 | Remove or rephrase: Clause 3 (covers 3) 在句首加入「外評完成後」— item 3 的 req 及 quote 無任何時間限定，該義務屬持續性一般規定而非視學後專屬要求；此時限限制屬虛構 |
| 15 | ✅ Fixed | distorted-number | 2 | 8 | 須制訂→應制訂, 法團校董會→校董會/學管會 (sec8/clause1) |
| 16 | 🔍 Review | fabricated | 0 | 8 | Remove or rephrase: clause 0：'積極蒐集其意見'中'積極'一詞，在 perf_indicators_2022 p.13（item 0）及 p.10（item 1）原文均無，屬加入原文不支持之程度副詞，構成新義務語 |
| 17 | 🔍 Review | fabricated | 2 | 8 | Remove or rephrase: clause 1：'並提交法團校董會審議'，sag_2025_11 p.133（item 2）原文只說'校董會亦需要負起指導及監察的角色，適時提供意見'，並無提交義務及'審議'程序表述。clause  |
| 18 | 🔍 Review | distorted-number | 2 | 10 | Restore source wording: 原文 quote 明確要求「設立／強化校本監察機制」（設立或強化二選一），改寫 clause 1 只寫「設立校本監察機制」，刪去「強化」選項，改變原文的義務範圍。 |
| 19 | 🔍 Review | fabricated | 2 | 10 | Remove or rephrase: Table 第二行（圖書館館藏）監察重點加入「資料時效性」，原文 sag_2025_11 p.38 的 quote 及 req 均無提及時效性要求，屬於 source 不支持的新義務。 |
| 20 | 🔍 Review | fabricated | 1 | 11 | Remove or rephrase: Clause 1 將原始 req 及 quote 嘅「校董會」替換為「法團校董會」。原始來源（sag_2025_11 p.204）全文只寫「校董會」，並無限定為「法團校董會」（IMC）。「法團校董會」 |
| 21 | 🔍 Review | fabricated | 0 | 12 | Remove or rephrase: Clause 0 says「持續監察各投訴個案之處理情況」，原文 quote 只係「監察有關情況」，並無「持續」此時態加強詞，屬於 source 唔支持嘅情態加強。 |
| 22 | 🔍 Review | fabricated | 1 | 12 | Remove or rephrase: Clause 1 開首加入「本校須確保迴避原則獲得切實執行」，原 req 及 quote 均無提及「迴避原則」此概念框架，屬於 source 唔支持嘅新增義務語句。 |

---

## Fix Priority (for Leonard)

### High Priority — scope/eligibility distortions

These change who an obligation applies to (特殊學校 vs 全校, 中學 vs 全校, etc.):

- **sen[6]** item=3: Clause 5 將 item 3 的主體由'中學'改為'本校'，把原條文明確只適用於中學（g19 p.78 quote：'如中學考慮降低學生的支援層級'）的義務，靜默擴展至所有學校，超出源文範圍。
- **sen[17]** item=2: Clause 5 covers id2。id2 req 明確限定「特殊學校學校人員必須體恤學生的情況並因應其特殊教育需要，提供適切的測考調適或特別考試安排，例如延長考試時間或使用特別輔助器材等」，source: g10 p80。改寫改為「本校必須體恤學生的情況並因應其特殊教育需要，提供適切的測考調適
- **sen[18]** item=31: Clause 17 covers id31。id31 req 明確限定「特殊學校應為學生的溝通模式制定校本政策，並提供機會予學生學習理解和表達」，source: g10 p61。改寫改為「本校應為學生的溝通模式制定校本政策」，刪去「特殊學校」適用範圍限制，將特殊學校專屬政策義務擴展至所有學校，屬義務
- **sen[22]** item=11: Clause 10 (covers items 11 & 13) removes the '特殊學校' scope qualifier present in both original items. Item 11 req: '特殊學校領導層及課程主任應致力...'; item 13 req: '特
- **sen[23]** item=12: Clause 11 (covers item 12) removes the '特殊學校' scope qualifier. Item 12 req: '特殊學校須教導學生表達「不」的否定或拒絕回應，以加強其自我決策能力及提升個人安全' (g10 p63). The rewrite says '本校
- **sen[26]** item=16: Item 16 requires '官立及資助小學' (both government AND aided primary schools) to collect year-6 language-impaired students' reports by June-end. Clause 9 wri
- **sen[32]** item=24: Item 24 req specifies '成績稍遜學生（小學適用）', making the under-performing-student submission a primary-school-only requirement. Clause 7's table row lists '成績
- **sen[44]** item=12: 原始 req 及 quote（g10 p52）均為「為學生提供較個人化的課程選擇」，適用對象是特殊學校全體學生；改寫將對象縮窄為「嚴重智障學生」，無任何原文依據，屬對象範圍失真。
- **gov_admin[5]** item=0: item 0 req 明確列出「資助學校及按位津貼學校」兩類學校，clause_0 改寫為「本校（資助學校）」，完全刪去「按位津貼學校」，縮窄計劃適用範圍。

### Medium Priority — modal distortions (情態詞) not yet fixed

- **sen[7]** item=3: Clause 5 使用'第三層支援'，但 item 3 req 及 quote（g19 p.78）的原用詞為'第三層級'，非'第三層支援'，術語不對應。
- **sen[17]** item=2: Clause 5 covers id2。id2 req 明確限定「特殊學校學校人員必須體恤學生的情況並因應其特殊教育需要，提供適切的測考調適或特別考試安排，例如延長考試時間或使用特別輔助器材等」，source: g10 p80。改寫改為「本校必須體恤學生的情況並因應其特殊教育需要，提供適切的測考調適
- **sen[18]** item=31: Clause 17 covers id31。id31 req 明確限定「特殊學校應為學生的溝通模式制定校本政策，並提供機會予學生學習理解和表達」，source: g10 p61。改寫改為「本校應為學生的溝通模式制定校本政策」，刪去「特殊學校」適用範圍限制，將特殊學校專屬政策義務擴展至所有學校，屬義務
- **sen[24]** item=10: Item 10 requires '須在中、小學概覽內的專欄' (both primary and secondary school overviews). Clause 0 writes '須在學校概覽內的相關專欄', dropping '中、小學' and obscuring that the 
- **sen[37]** item=0: Item 0 requirement and source quote (g19:60) use '應' (should). Clause 0 uniformly uses '須' (must), upgrading the compliance obligation level.
- **sen[38]** item=6: Item 6 requirement and source quote (g06:133) use '應' (should/参照). Clause 0 uniformly uses '須' (must), misrepresenting the obligation level.
- **sen[39]** item=7: Item 7 requirement and source quote (g06:96) use '應' (should). Clause 0 uniformly uses '須' (must), misrepresenting the obligation level.
- **gifted[5]** item=9: Item 9 原文用「可因應」（許可式義務，schools MAY adapt），但 Clause 1 將整段框架設為「須完成以下前期分析工作」（強制式），把 item 9 的許可式要求改寫成強制要求，模態走樣。
- **gifted[17]** item=5: Item 5 uses '宜' (recommended/should), indicating a recommended practice. Clause 3 rewrites as '本校持續推廣...' with no modal qualifier, treating it as a ma
- **gov_admin[0]** item=3: Clause 1 merges items 1 and 3 into one sentence and applies '書面同意' (written consent) to item 3's approval requirement, but item 3's source (sag_2025_1
- **gov_admin[2]** item=2: Clause 2 covers id 2，原始 item 及 quote 寫「校董會」（泛指學校校董會），但 clause 改寫為「法團校董會」（Incorporated Management Committee），兩者係不同法律實體；原文並無限定必須是法團校董會，此替換縮窄了持份者範圍，屬數字/層
- **qa_inspection[9]** item=4: Item req states '學校須委派合適的教師'（須 = must），but clause2 table row '統籌責任' uses '本校宜委派合適的教師'（宜 = should/recommended）。Obligation level distorted downward rela

### Lower Priority — fabricated purpose/qualifier clauses

These add explanatory text (e.g., '以確保…', '以照顧…', '為了…') with no source support.
Generally safe to keep for readability, but flag if the added text creates a new legal obligation.

- sen: 19 fabricated issues pending review
- gifted: 18 fabricated issues pending review
- gov_admin: 15 fabricated issues pending review
- qa_inspection: 17 fabricated issues pending review
