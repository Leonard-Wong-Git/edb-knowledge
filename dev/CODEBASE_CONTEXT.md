...
- `q.html` — **S6 Quick Q&A**: ⌘K modal fallback; autofocus input; local `knowledge.json` fact search; idle/answer/no-confident-answer states; inline citations; template suggestion routes to `t-purchase.html`; Esc → index.html; ?q= hash prefill
- `app.html` — **K1知識平台 FULL REACT SPA** (EDB token system Session 80 + Phase 3 review split + Session 86 copy cleanup): full EDB CSS token retrofit; legacy alias remap (--cd/--mocha/--charcoal → new tokens); approved-fact dataset (live count in SESSION_HANDOFF baseline — 455 as of v2.3.0, 2026-05-16 dedup 792→455); tabs: 平台介紹, 政策搜尋 (已核實資料/來源文件/合併搜尋), 指引文件庫, 通告分析, 知識提煉(Admin split workspace), 知識管理(Admin)
