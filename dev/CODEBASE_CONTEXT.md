...
- `.venv/` — local Python virtual environment used for MemPalace CLI tooling; ignored by git
- `mempalace.yaml` / `entities.json` — local MemPalace project config generated by `mempalace init . --yes`; ignored by git
...
- MemPalace local memory tooling:
  - Preferred shared install: `/Users/leonard/mempalace/.venv`
  - CLI version verified: `MemPalace 3.3.2`
  - Project wing: `claude_edb_knowledge`
  - Project config: `mempalace.yaml`, `entities.json` (both git-ignored)
  - Intended shared palace: `/Users/leonard/mempalace/palace`
  - Useful commands: `/Users/leonard/mempalace/.venv/bin/mempalace --palace /Users/leonard/mempalace/palace status`, `/Users/leonard/mempalace/.venv/bin/mempalace --palace /Users/leonard/mempalace/palace search "query" --wing claude_edb_knowledge`
  - Current shared-palace status: recovered and writable. Final `status` shows 6,785 drawers total; `claude_edb_knowledge` has 5,404 drawers (dev 4,776; design 397; backend 108; general 87; documentation 36).
...
### MemPalace
- Purpose: local-first AI memory/search over project files and session history
- Official sources:
  - GitHub: `https://github.com/MemPalace/mempalace`
  - PyPI package: `mempalace`
  - Docs: `https://mempalaceofficial.com/`
- Version installed locally: `3.3.2`
- Project config:
  - Wing: `claude_edb_knowledge`
  - Preferred palace path: `/Users/leonard/mempalace/palace` (shared local store)
  - Generated files: `mempalace.yaml`, `entities.json` (git-ignored)
- Notes:
  - Official docs warn not to use unofficial domains such as `mempalace.tech`.
  - Core workflow does not require an API key.
  - Shared palace was recovered after ChromaDB segfaults by extracting drawers from SQLite, rebuilding a fresh Chroma collection with `hnsw:num_threads=1`, then swapping the recovered palace into place. This follows the workaround described in MemPalace GitHub issue #974.
  - Old shared palace backup: `/Users/leonard/mempalace/palace.pre-recovery.20260421_0838`
