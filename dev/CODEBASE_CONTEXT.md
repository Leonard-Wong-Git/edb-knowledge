...
- `backend/src/lib/factEmbeddingStore.ts` — shared types + IO for the precomputed Channel A fact embeddings. Canonicalises fact ids as `{topic}/{role}/{index}` and stamps each entry with a sha256 `text_hash` so drift is detectable without re-embedding.
- `backend/scripts/buildFactEmbeddings.ts` — builds / incrementally refreshes `backend/data/fact_embeddings.json`. Reuses vectors whose (id, text_hash) pair is unchanged; batches OpenAI calls (64 per request). Run: `npm run embeddings:build` (needs `OPENAI_API_KEY`); `npm run embeddings:build:dry` for a no-API preview.
- `backend/scripts/verifyFactEmbeddings.ts` — offline drift check between `role_facts.json` and `backend/data/fact_embeddings.json`. Run: `npm run embeddings:verify`; exits 1 on any missing entry, hash mismatch, stale id, or wrong vector dim.
- `backend/data/fact_embeddings.json` — precomputed embeddings store (text-embedding-3-small, dim 1536). Runtime source of truth for Channel A semantic search. Must be rebuilt whenever `role_facts.json` changes.
- `backend/src/api/searchChannelA.ts` — Channel A cosine search over the precomputed fact embedding store. Embeds only the query per request (no more per-request full-corpus re-embed). Loads the store lazily + caches in-process; throws at first query if the store is missing or cannot be read.
...
- `dev/knowledge/role_facts.json` — **mirror** of repo-root `role_facts.json` kept for the dashboard export pipeline and legacy tools. SSOT is the repo-root copy; drift between the two is a hard block on `bump_version.py` (see SSOT guard in `check_ssot_drift`).
...
- `2026-04-19 (Claude_20260419_0001)` Session 80 — Channel A scaling fix. (1) **SSOT sync**: repo-root `role_facts.json` was 109 facts v2.0.0 while `dev/knowledge/role_facts.json` + `app.html` held 1,001 v2.1.0 (Session 79 merge never reached backend). Copied dev/knowledge → repo-root; content hashes now match. (2) **bump_version.py**: removed deleted k1-dashboard.html entry; added repo-root `role_facts.json` as tracked file; new `check_ssot_drift()` blocks any real bump when repo-root and mirror disagree. (3) **Precomputed Channel A embeddings**: new `factEmbeddingStore.ts` + `buildFactEmbeddings.ts` (incremental by sha256 text_hash, batch 64 / OpenAI call) + `verifyFactEmbeddings.ts` (offline drift check). Output: `backend/data/fact_embeddings.json` (text-embedding-3-small, dim 1536). (4) **searchChannelA rewrite**: replaces per-request full-corpus re-embed (1,001 embedding calls/query → 1). Loads store lazily, caches in-process, throws at first query if store missing. (5) npm scripts: `embeddings:build`, `embeddings:build:dry`, `embeddings:verify`. `npm run check` ✅. Offline regression 9 PASS (unchanged from baseline) + 2 pre-existing FAILs: `role-bucket` (1,001-fact merge saturates 600-char budget with all_roles finance facts so subject_head/panel_chair selections collapse) and `schema consistency` (test hardcodes knowledge.json 1.3.1 but file is 1.4.0). Neither caused by this session — both pre-date it and are logged as TODOs for the next pass.
- `2026-04-19 (Claude_20260419_0002)` Session 80 cont'd — Regression cleanup. (1) **knowledgeSelector.ts algorithm change**: `getTopicFacts` now `interleave(all_roles, role_specific)` instead of `[...all_roles, ...role_specific]` → trim. With 1,001 facts, `finance`/`hr`/`curriculum` all_roles pools alone exceeded the 600-char budget, starving role-specific facts entirely and making `subject_head`/`panel_chair` selections identical. Interleave preserves the "baseline first" intent (all_roles still leads at position 0) while guaranteeing role-specific facts a seat whenever the pool has content. Behavioural impact: Circular System prompts for the 3 heavy topics now carry role-specialised facts; some mid-priority all_roles facts may be pushed out — that is the deliberate trade-off. (2) **semanticRegression.ts schema test unfrozen**: was asserting `knowledge.json.version === "1.3.1"` (hardcoded). Replaced with a relational check: `knowledge.json.version === guidelines.json.version` (they are released together), still rejects old 1.3.0 markers. Test now survives normal version bumps. (3) Offline regression: **PASS=11 / PASS with notes=1 / FAIL=0**. `finance_distinct=true`, `in_sync=true`. Next pass still owes: real `npm run embeddings:build` (needs OPENAI_API_KEY), quality sampling of the 1,001 facts, and the knowledge.json public-API republish decision.
