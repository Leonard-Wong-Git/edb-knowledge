...
| 政策搜尋 | **S119：用戶介面行 Channel-B-only**（來源文件＝Supabase 語義搜原文）。Channel A（已核實資料）/ A+B（合併）user-facing 入口全移除、code path 留 dormant（可逆）。backend `/channel-a`·`/combined` endpoint **不刪、仍生存**只係前端唔再 call；Channel A `role_facts.json→knowledge.json` 資料管道**S143 Phase 1 已凍結（frozen @455 facts、knowledge.json 停更、schema 不變、繼續供下游零改變；Q4 ③→① phased，Phase 2 下游轉 Channel B **S146 已完成**（Leonard 確認下游 consumer build+cutover 完成；S202 route-probe 零 channel-a 流量佐證）§A.3；guidelines.json 不凍續 live）**。需 backend（無離線快路；q.html 等檔 dormant 留存） |
...
| ~~✍️ 知識提煉（Admin）~~ | **REMOVED S151**（下游轉 Channel B + Channel A 凍結 @455 → 人工策展淘汰；整個 admin surface 連登入閘從 app.html 刪除，見 §E.10 / §F.11） |
| ~~⚙️ 知識管理（Admin）~~ | **REMOVED S151**（同上：批量管理 / 匯出 / 版本控制 admin UI 全刪；knowledge.json/role_facts.json 凍結資料仍存、下游契約零接觸） |
...
>
> ✅ **EXECUTED（2026-06-03 S140，Leonard 拍板執行；前身 S112 DEFERRED FUTURE / S111 OPEN DECISION）**：公開 `guidelines.json` 已由 39 擴張到 **152**（round-3 公積金 +4）。**round-1**：registry 全集投影 139（剔 9 非文件——7 stat 統計表 + 1 DOCX 申請表 + 1 壞 URL `religious_edu_jss`〔Google grounding redirect、且係 `religious_edu_jss_2024` 重複〕；保留 landing 頁 + g10/g16/g28〔format=INDEX 但係真指引、原已公開〕）。**round-2**：landing-page curate（agent-team 3 curate + 1 audit + 主 agent egress 逐份核實）由 16 課程文件目錄頁 159 連結揀 9 真‧KLA/課程指引全文 PDF 加入 registry（→157 entries）+ 修 3 registry data bug，公開 139→**148**。走咗 §3 HIGH-risk PLAN（Leonard 多輪確認範圍）。下游 Circular System curriculum 桶 25→**132**（會脹但不 break；topic 篩選 `guidelines[topic]` 仍 work）。**round-3**：Leonard 指定加教職員公積金覆蓋（registry 標題層之前 0 公積金）—— 加 4 entry（公積金主題 hub + 津貼/補助學校公積金一般資料 Q&A + 2013教育修訂條例 FAQ，全 category=人力資源/hr、HEAD-200 驗證）→ 公開 148→**152**、registry 157→**161**、hr 桶 2→6。**版本 2.2.0→2.5.0；現有條目 0 lost（regression guard verified）**。生成機制：NEW `dev/build_guidelines.py`（registry=SSOT）。`Suppl_guide`（非華語補充指引全文 PDF）held 待人核。
...
- Embedding：OpenAI `text-embedding-3-small`（固定）。LLM 預設 `gpt-4.1-nano`（`OPENAI_MODEL` 可覆寫，Responses API）。**S211：相關性判斷閘行獨立 model，預設 `gpt-4.1-mini`（`JUDGE_MODEL` 可覆寫），與合成 model 分開 —— 判斷判斷閘行為前先確認是哪一個變數。**
...
- 環境變數（`backend/.env`）：`OPENAI_API_KEY`、`OPENAI_MODEL`、`JUDGE_MODEL`、`FEATURE_EXACT_WINDOW_NARROW=0`、`FEATURE_GROUNDED_SYNTHESIS=0`、`PORT=8787`、`CORS_ORIGIN`、`KNOWLEDGE_PATH`。兩個 S214 feature flags 預設關閉；本地 `file://` dev 需 `CORS_ORIGIN=*`。
...
- `dev/CHANNEL_B_SYNC_SPEC.md`：**輸出向** Channel B 增量同步契約（K1-side，Q4 Phase 2；manifest+fetch 端點待 build；S145 **v0.4** build-ready — 下游 consumer 覆文納入：§11.2 resolved 路1、manifest +topic/content_type、向量 L2-norm 實測、profile 校正）。
...
20. **檢索 regression 嘅定義本身要防「日日紅」（S210 實證）** — `dev/source/eval_retrieval.py --compare` 原本當「新源入 top_k」無害（`SET_ADDED` 非 blocking），但當「同一件事撞跌尾位」係 blocking（`SET_LOST`）。**固定 top_k 之下呢係一件事嘅兩個講法** —— 新源入嚟就一定有嘢跌出去。Option A 自動入庫管道每日跑兼自己 push main，所以呢個閘本來會由 2026-08 起朝朝紅，跟住冇人再睇（同 §E 嘅「只入唔出嘅清單必變牆紙」同一條病）。**已固化做法**：新 `DISPLACED` 狀態，三個條件齊先降級 —— (a) 真係有新入者、(b) 每一條掉低嘅位置都喺 `len(before) - len(added)` 呢條 cut line 或以下、(c) after 名單唔短過 before。任何一條唔齊就照 `SET_LOST`。**刻意唔覆蓋**「同一個源攞多咗位」（`added` 為空）嘅情況：`SET_LOST` 嘅意思係「本來搵到嘅源而家完全搵唔到」，嗰個情況確實係，要人睇。**通則：改一個閘之前先問「佢會唔會日日紅」；一個永遠紅嘅閘同一個永遠綠嘅閘一樣冇用。**配套斷言：`--self-test` 內兩條專證佢仲會紅（cut line 以上跌出、after 短過 before），因為一個冇人示範到會紅嘅閘唔算閘（承 §D 11 同紀律「守門要證明佢會紅」）。
...
### E.10 公開站點的 client-side admin 閘門 + Supabase `wiki_chunks` RLS/GRANT 開放（admin-login (a) **CLOSED-BY-REMOVAL S151**；RLS family (b) S121 CLOSED）

> ✅ **S151 (2026-06-08) — (a) 徹底解決：整個 client-side admin surface 已從 `app.html` 移除。** 下游 Circular System 已轉 Channel B 不再食 Channel A + Channel A 凍結 @455 → 人工策展（知識提煉 candidate approve / 知識管理 fact CRUD / 匯出 / FactCard 增刪改）連同登入閘全部刪除（移除 `ADMIN_HASH`/`sha256()`/`AdminPasswordModal`/`adminMode` state/2 個 admin tab/mobile admin bar/`buildAdminSnapshot`/`loadLocalSnapshot`/`INITIAL_REVIEW_STATE`/`INITIAL_CANDIDATES`/`CandidateReviewPanel`/`FactCard`/`ExportModal`/`EditModal` + 2 條 orphaned `<script>` 載入；app.html 4100→2935 行 -1176）。**公眾 app 現淨 3 tab（平台介紹/政策搜尋/指引文件）+ 文件預覽抽屜，完全 Channel-B-only。** 既然再無 admin surface 可繞，cosmetic-gate 攻擊價值 + git-history plaintext-leak vector 兩者皆變 moot（無嘢可解鎖）。**下面 (a) 的 ACCEPTED-conditional 條款由 S151 起作廢**：app.html 不再有 admin 功能；任何將來想重建 admin / approve / write surface 必須由零走 §3 HIGH-risk PLAN + 真正 server-side auth（不可復活 client-side cosmetic gate）。(b) RLS family 仍 S121 CLOSED。
- **根因 (a) admin client-side gate（S131 ACCEPTED + DOCUMENTED；前歸 🔴 OPEN 跨 Sessions 19–130；real-leak surface relocated）**：Admin approve / edit / export / 知識管理 閘門只係瀏覽器端 SHA-256（`ADMIN_HASH` @ app.html:704），整套控制隨靜態站 deploy 上**公開 GitHub Pages** — **不是真正安全邊界**，任何人可繞（自 acknowledge：app.html:693-701 SECURITY NOTE 明寫「COSMETIC / UI-ONLY」「intentionally OUT OF SCOPE」）。**S131 verify 揭兩處 sub-claim 修正**：(i) `dev/archive/SESSION_LOG_2026_Q2.md:190` 寫嘅「(password: internal)」係 **placeholder / 寫錯**（`SHA-256("internal") = 3bed2cb3...054f` ≠ live `ADMIN_HASH = 9d35e7...b318a`），governance 原 cite 此 line 為 leak point 唔正確；(ii) 但 `dev/archive/SESSION_LOG_2026_Q2.md:213` 寫住 `sha256("...REAL-PLAINTEXT-PASSWORD...") matches ADMIN_HASH ✅` form = **TRUE real plaintext leak existing in git history immutable** since Session 28/29 era closeout。即原 §E.10 (a) "password leaked in git" claim 嚴格講係**對的**（但 leak point 唔喺 line 190、喺 line 213，governance 描述 misaligned）。**Attack surface 重新評估近 zero**：(i) admin features effect 全 client-side localStorage / JSON snapshot download；snapshot 內容 = INITIAL_DATA + REVIEW_STATE 已 hardcoded 公開於 source code；繞 gate 嘅 attacker 只 "見到 admin UI" + "下載 snapshot"（內容已 public）→ 不 leak any not-already-public 資料。(ii) Plaintext leak in archive 雖然 git-immutable，但因 attack surface 條件 (i) → 攻擊者攞到 plaintext 唯一可做 = 進入 cosmetic gate 後 "見 admin UI"，獲取 0 net 新資料。(iii) 由此 ACCEPTED rationale = 唔係 "no leak" 而係 "leak attack value ≈ 0 per cosmetic-gate design + snapshot-already-public"；conditional reopen trigger 同樣 = 任何拆掉 client-side-only 前提即須重新評估包含 archive plaintext leak vector。
...
- **防線**：(1) client-side 閘門只當 UI 便利，**永不可當安全邊界**；任何敏感控制（approve / export / 知識管理）暴露前必須有 server-side 驗證。(2) **絕不**把密碼 / API key / token 明文寫入任何 log / session 文件 / commit；archive 入面類似「(password: X)」字樣**動手前先 SHA-256 verify** vs live `ADMIN_HASH`，唔對即係 placeholder / misleading 而非 real leak（S131 case study）。(3) 公開 artifact push 前 grep secret pattern。(4) **S121 codified**：`wiki_chunks` RLS = ON + 1 anon-read SELECT policy + anon/authenticated GRANTS = SELECT only + service_role 全 grants bypass RLS（design 上 upload pipeline 仍 work）。Defense-in-depth：將來 GRANT drift 都會被 row-policy 攔住。**新 invariant**：anon key 對 wiki_chunks 只可 SELECT；anon-write 試圖 必死（RLS deny + GRANT REVOKE 雙重攔截）。若未來真需要 anon-write feature 必走 §3 HIGH-risk + 新 policy（不可悄悄 GRANT）。(5) **跨表延伸**：Supabase scanner 若就其他 table（e.g. future tables）raise 同類 `rls_disabled_in_public`，沿用 S121 ritual = INSPECT GRANTS 全套（唔好淨睇 alert surface）+ ENABLE RLS + 加 policy + REVOKE 不必要 write privileges。(6) **S131 lesson**：governance 寫嘅 "leak" 描述本身可能係 misleading-placeholder / 寫錯（archive line 寫"internal"`、real hash 對唔同 plaintext）→ 任何「password / secret leaked in X」claim 動手前必做 SHA-256 round-trip verify、唔信文字（§G.2 banner 4th instance pattern：archive misleading-placeholder 同 S121 schema.sql / S122 commit-msg / S126 handoff-hypothesis 並列）。
- admin-login client-side gate (a) = **S131 ACCEPTED + DOCUMENTED**：cosmetic gate per code-acknowledged intentional design + real attack surface near-zero + verified-no-git-leak → 從 active OPEN priority 降為 documented-accepted state；RLS / GRANT 部分 (b) = **S121 RESOLVED**。**Future agent 若擴 admin features 或加 sensitive surface 必重新評估、必走 §3 HIGH-risk PLAN**；本條目接受態 conditional on "admin features 維持純 client-side localStorage + snapshot 無真實 write 能力"，任何拆掉此前提（e.g. 加 server-side admin write endpoint / 加 admin-only data access）必 reopen + 重 design auth boundary。下一個 agent 碰 admin / auth / 公開推送前仍必讀此條。
...
2. `app.html` 為主 SPA；`index.html` 為 EDB landing；`k1-dashboard.html`/`landing.html`/`k1-wiki.html` 已刪不復活。**S119（Leonard live-test 後定，§3 HIGH-risk PLAN→確認 done）：政策搜尋 user-facing 介面行 Channel-B-only，A（已核實資料）/ A+B（合併）入口全移除、code path 留 dormant 可逆；此為現行鎖定 surface 決策。前提区分：此係「搜尋介面」決策，唔係「Channel A 資料管道」決策——Channel A `role_facts.json→knowledge.json` 照常餵下游 Circular System，對外契約零接觸；契約收斂（Q4：叫下游改／Channel B 變供料／凍結停供）= S143 Phase 1 EXECUTED（Leonard 明示「執行 Phase 1」、③→① phased）：Channel A 正式凍結 @455 facts、knowledge.json 停更（凍結點 2026-06-05、schema 不變、繼續供下游零改變）、Channel A pipeline code 留 dormant 可逆（endpoint 不刪）。Phase 2（選項①下游 Circular System 轉 Channel B）= 跨 repo、**S146 已完成**（Leonard 確認下游 consumer build+cutover 完成；S202 route-probe 零 channel-a 流量佐證；K1 只備 spec、絕不掂對方 repo §A.3）；選項②（Channel B 變供料）不採（衝突 §F.6）。**S144：Phase 2 整合模型 LOCKED = Incremental sync（manifest-diff delta feed）；K1-side 契約稿 `dev/CHANNEL_B_SYNC_SPEC.md`（過對抗審核 hardening、修 2 blocker：非原子-ingest 誤刪 live 源、版本變更被 304 隱形）→ §11 4 決策 RESOLVED v0.3。S145：下游 Circular System consumer 覆文納入 → §11.2 RESOLVED（embedding 路 1 `text-embedding-3-small`／`include_embedding=true` 零重嵌）、manifest +topic/content_type（反問 a）、向量 pgvector`"[…]"`字串+L2-norm≈1.0 實測（反問 c）、限流 bootstrap pacing（反問 b）、profile 校正（下游真實 = GitHub Actions ephemeral cron 3×/日 + file-based numpy、非常駐/非 pgvector；incremental 仍最優）→ **spec v0.5**（v0.4 + §13 對抗審核澄清）；K1 端點 S145 已 build（`backend/src/api/channelBSync.ts`：manifest+chunks、X-Sync-Key gate、自有 60/min+daily budget、anon-REST、NO CORS、dormant 至設 `CHANNEL_B_SYNC_KEY`；過 5-lens 對抗審核 + 本地 gate smoke PASS）。S146：全鏈完成 —— K1 端點 LIVE（manifest/chunks 401-gated 健康）、下游 consumer build+cutover 完成（Leonard 確認）、`dev/CHANNEL_B_HANDOVER.md` 交接包齊。** Rollback = git revert docs（freeze）/ 刪 channelBSync.ts + 2 route 行（endpoint）。**
...
11. **（S151，Leonard 拍板、走 §3 HIGH-risk PLAN）app.html 不再有任何 admin / 人工策展 / 登入功能。** 公眾 SPA = 3 個唯讀 tab（平台介紹 / 政策搜尋〔Channel-B-only〕/ 指引文件）+ 文件預覽抽屜，完全 Channel-B-only。理由鏈：下游 Circular System 已轉 Channel B 不再消費 Channel A（Leonard 告知）+ Channel A 凍結 @455（§F.2）→ 知識提煉 / 知識管理 等 Channel A 人工策展功能淘汰 → 連 cosmetic 登入閘（曾為 §E.10 最長壽風險）一併移除（CLOSED-BY-REMOVAL）。`knowledge.json` / `role_facts.json` / `guidelines.json` 凍結資料與對外契約**零接觸**（admin UI 只係 client-side localStorage + snapshot，無真實寫能力）。Rollback = `git revert` 該 app.html commit。將來如需重建 admin/write surface 必由零走 §3 HIGH-risk PLAN + 真 server-side auth（不可復活 client-side cosmetic gate）。
...
> - 「148 是過時 registry 計數」嘅舊說法本身就係錯（§B.1 已更正）：161 = app 內庫實數（S140 landing-curate +9 + 公積金 +4 後），152 = 公開端點全集投影（S140；39→139→148→152），兩者皆真。
