...
1. **⚠️ GitHub Pages 404 復原（最優先）**：S163QC closeout 時全站 404（root+app+index 兩域）。站恢復後 live 驗 app.html v3.0.0 header/footer + index.html + 文件標註/範本下載 tabs 0 console err。若長時間未復 → flag Leonard 查 Pages settings（私庫需 web Settings→Pages 睇 build log）/ GitHub Status / empty commit re-trigger。Render backend（P2/P3）已 LIVE 不受影響。
