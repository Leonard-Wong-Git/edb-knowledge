...
4. **【S232 實測，2026-09-24】`origin/main` = `ef53da8`，線上執行碼仍為 `f712172`，`backend/` 零落後**（`git diff --name-only f712172..origin/main -- backend` 為空；S232 三個 PR 都不動 `backend/`，Render 未重新部署）。`/health` `started_at` 2026-09-24T17:37:51Z，`cache_b.footnote` **181**（已載入新 footnote）。S232 的 #32、#33、#34 由 **Leonard 親自在 GitHub 網頁合併**。🔴 `main` 仍受 ruleset 保護（`main build gate`），一切改動走 PR。**判斷線上是否落後看 `git diff --name-only <部署commit>..origin/main -- backend`，不要比 hash。** ⚠️ **footnote 入庫後生產要進程重啟才載入**（`_footnoteCache` 無 TTL）：S232 做法是入庫後 21 分鐘不發任何請求，令 Render 免費方案休眠，下一次請求喚醒時即重新載入，並以 `/health` 的 `cache_b.footnote` 數目核實。
...
**S232（2026-09-24）—— GitHub 電郵收窄、警報 @mention、GN10 路線甲（footnote）上線。** Supabase **寫入 1 行**（Leonard 批准）；零 DDL、零 flag 改動、零 `backend/` 改動；外部呼叫：`text-embedding-3-small` 約 12 次（self-test 11 ＋ 入庫 1）、生產搜尋 `synthesize:false` 約 86 次（兩次 `footnote_lead_probe` 各 40 ＋ 窗口檢查 6，全帶 `x-probe`）、生產 `synthesize:true` **2 次**（Leonard 批准）。

1. **GitHub 電郵**：來源是 repository 層的 push 電郵設定（每次 push 一封，不經帳戶通知設定）＋ Watching 電郵。Leonard 已在帳戶層改為：Participating／@mentions 保留 Email、Watching 只留 On GitHub、Actions 只通知失敗、Dependabot 保留 Email、不含自己的更新；並清空四個 repo 的 push 電郵。**push 電郵設定無 API 可讀，只能由 Leonard 在網頁核實。**
2. **警報 @mention（#33）**：`monitor_watchdog.yml` 與 `backend_build_check.yml` 開 issue 時正文頂部加 `@Leonard-Wong-Git`（照 S191 `served_url_check.yml` 做法；只有開 issue 那次會通知，之後靜默改正文不再通知）。驗證：兩檔 YAML 可解析，內嵌 github-script 經 `node --check` 通過，build gate 綠。**未經真實警報觸發驗證。**
3. 🔴 **S231 交接寫的 footnote 前提是錯的，已攔下未寫入**：「普通幼稚園無規定師生比例、只有班額上限」與庫內原文相反。原文：《幼稚園行政手冊》5.1.1.1（PDF p.107）—— 參加幼稚園教育計劃的幼稚園整體師生比例由 1:15（校長計算在內）提升至 1:11（校長不計算在內）；教育局通告第 26/2003 號（《學前機構辦學手冊》附錄 20，p.147–148）—— 每 15 名在場學生須有一名教師當值，每班最少一名。
...
**S230（2026-09-21）實測結果 —— 四批。判讀對照上面那份預先定案逐條讀。**

**第一批 —— instrument 驗證：本機 cosine 與 Supabase RPC 是同一把尺，但要取 4 位小數。** `match_wiki_chunks` 回傳的是 `round((1 - (embedding <=> query_embedding))::numeric, 4)::float`（`dev/supabase_setup.sql:69`），即 **0.70 這個數當初校準時看到的每一個分數都已經過這個 rounding**。實測（裸查詢一條、全庫 40 行）：不取整時 **40/40 全部有差**、最大 5.0e−05；取到 4 位小數後 **0/40 有差**，逐條完全相同。故 `bareCosineForChunk()` 照樣取 4 位小數 —— 否則真值 0.69996 的片段在伺服器端過閘而在本機不過，那正是與「要放回去的那把尺」在邊界上不一致。

**第二批 —— 主指標：改動失敗，而且是按我自己預先定下的接受條件失敗。** 24 條校準 query × 兩個 arm（旗標關／開），bypass 有無發生是**觀察**出來的（stub 判官記錄自己有無被問，不再像 S229 靠「遞降後綴」推斷窗內哪一片是 `mainSearchLead`）。結果：bypass **5 → 0**。得益只有一條 —— 真缺口「校巴司機最低工資係幾多」（應用尺 0.7098 → 裸查詢尺 **0.3203**）正確轉為走判官；代價是另外四條，包括三個正控（`人工智能初探` 0.7842→0.4002、`跟車保母有咩要求` 0.7932→0.5080、`校巴司機安全指引` 0.7771→0.5788）與**預先定案明列必須保住的**「老師病假連續請幾耐先要交醫生紙」（0.7003→0.5391）。artifact：`dev/source/eval_runs/2026-09-21_s230_gate_signal_ab.json`。**所以本節不提出出貨**，旗標留在碼內預設關閉。

**第三批 —— 為甚麼會這樣，以及 S229 的數字獨立重現。** 重跑 `_s229_scaleOffset.ts`（`..._s230_scale_offset_replay.json`）逐項重現 S229：A 尺 CLASS_B 上限 **0.6321**／CLASS_C 下限 **0.6241**（＝S195 原數 0.632／0.624，儀器自我驗證通過）· 路由 A→B **0 至 −0.0667**（不抬分）· 展開 B→C 最多 **+0.3226** · 應用尺上 CLASS_B 有 2 條 ≥0.70。**但這次重跑帶出一個 S229 沒有算的數：24 條校準 query 在校準尺上的最高分只有 0.6457**，即 **0.70 這個數比整個校準集的最高分還高 0.07**。S195 當初正是刻意這樣定的（原文：「Cosine cannot separate…so no choice of number is safe. Raising recall here needs a better JUDGE, not a lower bar.」）。**結論因此改寫：把閘放回校準尺不是「重新收緊 bypass」，而是等於「移除 bypass」** —— 換訊號與換對象在這裡是同一個動作，而 0.70 對新對象不適用。

**第四批 —— 那麼讓判官接手可不可以？不可以，而且有既有證據，零成本。** 凍結驗收集 `dev/source/judge_acceptance_cases.json` 內 `S01_ai_intro_bare_noun`（`人工智能初探`）與 `S02_ict_guide_bare_noun`（`資訊及通訊科技 課程指引`）**屬 answer 半邊、`want: 能`**，但三份記錄的 run（`dev/source/judge_runs/2026-08-02_s202_v4b_frozen35{,_r2,_r3}.json`，`gpt-4o-mini`）**全部判「否」**。即 title 式／裸名詞查詢正是判官答不到而 bypass 救得到的那一類（S183 建這個 bypass 的原因），**失去 bypass 就是失去答案**。同時量了詞面能否分開兩群：**片段層不能**（真缺口 0/3＝0.00，而正控 `跟車保母` 同樣 0/3；`人工智能初探` 0/0 ＝量不到）；**全窗層在「量得到」的四條之中可以**（真缺口 **0.00** vs 老師病假 0.17、跟車保母 0.33、校巴司機安全指引 1.00），但量不到的那一條恰好是必須放行的正控。artifacts：`..._s230_lead_overlap.json`（`separates_among_judgeable: false`）與 `..._s230_window_overlap.json`（`true`）。**這是 S231 的候選形狀，不是本節的結論** —— 5 個 bypass 樣本不足以定任何門檻，而且按 row 41 紀律不得用同一批數據既發現又驗收。

**S230（2026-09-22）第五批 —— 185 題 gold 的 bypass 人口普查：規模比 24 條校準集顯示的大一個量級，而兩個候選規則都被真實人口否決。** 工具 `backend/scripts/_s230_bypassCensus.ts`，artifact `dev/source/eval_runs/2026-09-22_s230_bypass_census.json`。單 arm 即足夠，而這一點先驗證過：`bypass_before` 由「領先片段是 `vault_extract` ＋ 應用尺分數 ≥0.70」推導，對兩 arm 的 24 條逐條核對，**24/24 與觀察值相同**。

1. **今日有 59/185（31.9%）條 gold 查詢跳過判官** —— 24 條校準集只量到 5 條，那個樣本嚴重低估了這道 bypass 的作用面。領先片段是 `vault_extract` 的有 167/185。兩條 error（`plc_confirmed_list_confidential` 逾時、`hr_overtime` Supabase `57014`）—— 本機仍是 service key 的 8 秒上限量 anon 3 秒的系統，見 Risks 2(b)。
2. **按 gold 真值拆開這 59 條**：`FALSE_BYPASS` **5**（語料本來答不到，判官被跳過 —— 正是 S177 那一類）· `WRONG_PASSAGE` **44** · `SOUND` **10**。⚠️ **`WRONG_PASSAGE` 這個數不可當成新缺陷**：它用的是「expected_passage_signature 是否以子字串出現在 top-5 窗內」，比本專案 `chunk_verdict` 那把尺粗且嚴，全 gold 口徑下只有 20.0%（33/165）通過。**而關鍵對照組是**：bypass 那批 18.5%（10/54）vs 非 bypass 那批 20.7%（23/111）—— **兩者實質相同，所以「bypass 特別集中在片段層失敗的題目」這個講法不成立**，不要這樣寫。
3. **五條 `FALSE_BYPASS` 逐條**：`幼稚園 表現指標` 0.7000 · `藝術教育學習領域課程指引 2002 版` 0.7023 · `資優教育學苑 收生 智商分數要求` 0.7212 · `校車服務 每月收費標準` 0.7575 · `校董有冇人工` 0.7023。**五條的共同形狀是「問一個語料裡不存在的數額／版本」**，與 S229 逐條開過的 `校巴司機最低工資`（不在 gold 內）同一類。
4. **rawvec 閘 × 真值交叉表**：`FALSE_BYPASS` 5 條**全部攔到**（5/5）· `WRONG_PASSAGE` 攔 41/44 · **但 `SOUND` 攔掉 7/10**。即在真實人口上這個候選的交易是「攔對 5 條、攔錯 7 條」，比 24 條校準集顯示的 1:4 好，但仍未過關。
5. **被攔掉那 7 條 `SOUND` 揭示這個訊號有系統性偏差，這是比交易比率更重要的理由**：`SEN 邊個負責統籌` 0.7623→0.3349 · `SENCO 職責` 0.7332→0.3097 · `跟車保母 年齡要求` 0.8026→0.5005 · `school bus escort duties` 0.7619→0.5196 · `跟車保母 學童名單` 0.7973→0.5608 · `校車安全帶 拒絕佩戴` 0.7794→0.5912 · `辦學團體校董…人數上限多少？` 0.7089→0.5817。**七條之中六條是二至三個 token 的短查詢**（兩條連 informative bigram 都是 0）。**裸查詢向量對短查詢結構上就是低分 —— 展開不是「虛高」，對短查詢它是令檢索成立的那一半。**所以把閘放回裸查詢尺＝系統性懲罰短查詢，而短查詢正是真實用戶的主要輸入形態。
6. **候選規則（乙）全窗詞面零重疊：在真實人口上死亡。** 59 條之中只會攔 **1** 條（且是 `WRONG_PASSAGE`），**五條 `FALSE_BYPASS` 一條都攔不到** —— 它們的重疊分別是 3/3、6/6、2/3、1/3、1/3，全部非零。24 條校準集之所以令它看似可行，是因為那一條真缺口剛好是 0 重疊，**那是巧合而非機制**。另有 9 條 bypass 查詢無 informative bigram，該規則對它們本來就量不到。
7. **順帶量到一個 rawvec 閘會造成的設計層回歸**：`半日制 24 班 教師人數` 的領先片段是 `searchEstablishmentRows` 的 lexical 精確命中，**依設計記分 1.0000（不是餘弦）**，而它的裸查詢餘弦只有 0.6423 —— 即 rawvec 閘會把 establishment 那條路的 bypass 一併廢掉，而那條路的確定性根本不來自向量相似度。
8. **未量**：被攔掉那 7 條 `SOUND` 交回判官之後，判官會不會答。**這是決定這個候選是淨賺還是淨蝕的唯一未知**，需要 7 次判官呼叫（外部模型批次，依既有邊界要 Leonard 明示批准逐項列明）。另：`forbidden_evidence` 出現在窗內的有 7 條，其中 2 條屬 bypass 那批（交接檔原記 8 條，口徑不同）。

**S230（2026-09-22）第六批的預先定案 —— 判官接手測試（寫在跑之前）。** Leonard 於 2026-09-22 明示批准一批外部模型呼叫：服務 OpenAI、模型為生產判官模型（`getJudgeModel()`，未覆寫時為 `gpt-4.1-mini`）、**上限 59 次**，合成器用 stub 故零合成呼叫。工具 `backend/scripts/_s230_judgeTakeover.ts`，對象是普查量出的 59 條現行 bypass 查詢，配置為候選旗標開啟（即真正的「改動後」行為），逐條記錄判官的原始答覆。

1. **量甚麼**：被 rawvec 閘攔下來的查詢交回判官之後，用戶最終**有無答案**。三種結局分開記：`KEEPS_BYPASS`（閘沒攔）· `JUDGE_ANSWERS`（攔了，判官判「能」，答案照出）· `JUDGE_DECLINES`（攔了，判官判「否」，答案失去）。
2. **技術失敗必須與判決分開**：`judgeCanAnswer()` 在呼叫拋錯時 `return true`（fail-open 去答），所以一次失敗會長得像一次「能」。探針逐條記 `judge_error`，**任何 error 都不計入判決**，並在結論明報條數 —— 這是 S198 那個沉默陷阱的同族。
3. **支持出貨**：五條 `FALSE_BYPASS` 之中 **≥4 條**轉為拒答，**且**十條 `SOUND` 之中被攔的七條有 **≥6 條**判官照樣答。
4. **不出貨**：`SOUND` 被判「否」達 **≥2 條**，**或** `FALSE_BYPASS` 仍有 **≥2 條**獲答。
5. **`WRONG_PASSAGE` 那 44 條只報數、不作出貨依據**（其真值判斷比本專案 `chunk_verdict` 粗，見第五批第 2 點）。但若其中 **≥30 條**轉為拒答，須單獨報為「這個候選會大幅推高拒答率」的警號，即使第 3 條成立亦不得逕自建議出貨。
6. **呼叫預算以機械方式守住**：探針自己數，超過 59 即拋錯中止，不靠自律。

**S230（2026-09-22）第六批實測 —— 判官接手測試：候選通過預先定案的全部出貨條件，這推翻本節第二至五批的「不出貨」結論。** 工具 `_s230_judgeTakeover.ts`，artifact `dev/source/eval_runs/2026-09-22_s230_judge_takeover.json`。判官模型 `gpt-4.1-mini`（生產預設，Render 未覆寫）· **實用 53 次呼叫／批准 59**（59 條之中 6 條閘沒攔，故不需呼叫）· **errors 0 · 無法解析的判決 0 · 未預期的回應形狀 0**。

1. **逐條對照預先定案**：`FALSE_BYPASS` 五條之中 **4 條轉為拒答**（門檻 ≥4，**達標**）· 被攔的七條 `SOUND` **7/7 判官照樣答、0 條拒答**（門檻 ≥6，**達標**）· 兩條停止條件皆未觸發（`SOUND` 拒答 0 < 2；`FALSE_BYPASS` 仍獲答 1 < 2）· `WRONG_PASSAGE` 警號未觸發（拒答 10 < 30，另 31 條照樣答）。
2. **主要得益**：對「語料本來答不到」的查詢，未經任何檢查就作答的曝面由 **5 條降至 1 條**。四條（`藝術教育學習領域課程指引 2002 版`／`資優教育學苑 收生 智商分數要求`／`校車服務 每月收費標準`／`校董有冇人工`）判官判「否」正確拒答。
3. **殘留缺口一條，而它不是這道閘的問題**：`幼稚園 表現指標` 判官仍判「能」—— 候選把它由「跳過檢查而作答」變成「判官錯判而作答」，**結局相同**。這一條要靠判官本身，不是門檻。
4. **零 `SOUND` 損失，而且損失最可能發生的地方正是它證明自己的地方**：被攔的七條有六條是二三個 token 的短查詢（`SEN 邊個負責統籌`／`SENCO 職責`／`跟車保母 年齡要求`／`school bus escort duties`／`跟車保母 學童名單`／`校車安全帶 拒絕佩戴`），**判官七條全部判「能」**。即第五批推論的「短查詢會被誤傷」在答案層並未實現 —— 誤傷只到「失去 bypass」那一層，判官把它接住了。**此處明確更正第五批的推論。**
5. **代價一：10 條 `WRONG_PASSAGE` 由有答案變拒答**（41 條被攔之中）：`DEBP`（裸縮寫，bare 0.1169）·`視學`·`學生使用人工智能時如何核實資料`·`資優教育基金 注資幾多`·`學習支援津貼`·`全校參與訓輔 幾時引入`·`校車司機 守時`·`小學科學科 一筆過津貼`·`家長校董 點選`·`校董會會議 法定人數`。**這 10 條是損失還是正確棄權未讀** —— gold 說可答但窗內沒有 expected signature，所以判官拒答可能是對的。**出貨之前必須逐條讀窗**（≈5.4% 的 gold）。
6. **代價二：候選會【新開】兩條 bypass** —— `小學 STEAM 教育`（applied 0.6983 → bare **0.7079**）與 `幼稚園可以限制派發K1申請表數目嗎`（0.6897 → **0.7253**）。成因是路由會**輕微壓低**分數（S229 量到 A→B 為 0 至 −0.067），所以少數查詢在裸尺上反而更高。兩條 `answerable=true` 但窗內無 signature，**即判官本來會跑而改動後不跑**。出貨前要有決定。
7. **未量的成本：延遲。** 閘在領先片段是 `vault_extract` 時要多一次 Supabase 單行主鍵查詢，而**普查顯示 167/185（90%）的查詢都屬此類**。單行主鍵 select 理應是幾十毫秒級，但 routed p90 是 2,498ms 而 anon `statement_timeout` 只有 3 秒，**啟用之前必須實測這一項**，不可假設。
8. **結論**：候選**由「不出貨」改為「條件性可出貨」**，先決條件四項 —— (a) row 41 要求的 `footnote_lead_probe.py --run` before→after（positive control 零損失）· (b) `judge_probe.py` 要先修到行生產路徑（現況 `blocked`）· (c) 第 5 點那 10 條逐條讀窗 · (d) 第 6 點兩條新開 bypass 的決定。**旗標只有 Leonard 在 Render 改得到。**

**S230（2026-09-22）第七批 —— 四項出貨先決條件全部處理完，另修好兩個會影響判決的自造儀器缺陷，並按量度結果收緊候選本身。**

1. **🔴 儀器缺陷一（會翻轉判決，已修）：signature 比對比本專案的評分器嚴。** 本專案單一口徑是 `squeeze(fold(s))`（`dev/_s213_corpus.py`：NFKC 正規化後刪去所有空白），而第五批那支探針用了原文子字串。逐條讀窗時捉到：`家長校董 點選` 的答案**逐字就在第 0 格**（一條 curated footnote 直接答了），只因 gold 寫半形 `(PTA)` 而片段是全形 `（PTA）` 便被判成「窗內無答案段落」；`小學 STEAM 教育` 則是 signature 被切片邊界斬開。**全 gold 口徑下，原文比對漏掉 25 條 signature 命中。**已改用本專案口徑並加紅測。
2. **改正後重新推導判官表（用已有判官結果，零新呼叫 —— 每條的結局與標籤無關）**：`SOUND` 由 10 升至 **15**（5 條由 `WRONG_PASSAGE` 移入：`半日制 24 班 教師人數`／`遣散費`／`家長校董 點選`／`法團校董會 成立`／`資助則例 法團校董會版本`），`WRONG_PASSAGE` 降至 39。**新表**：`FALSE_BYPASS` 5（拒 4／答 1）· `SOUND` 15（保住 3／答 11／**拒 1**）· `WRONG_PASSAGE` 39（保住 3／答 27／拒 9）。**預先定案仍然全部達標**，但 `SOUND` 拒答由 0 變 1，**距「≥2 即不出貨」只差一條**。那一條正是 `家長校董 點選` —— 答案在第 0 格而判官判「否」，**是判官的錯不是閘的錯**（與 S201 記錄的 D01 同族）。工具 `dev/_s230_relabel.py`（`--self-test` 7 條含一條紅測）、artifact `2026-09-22_s230_bypass_census_nfkc.json`。
3. **🔴 儀器缺陷二（已修）：探針自己不穩。** 第一次重跑因一個 embedding 逾時整個崩潰、零輸出 —— 那個呼叫被放在 try/catch 之外。已加單次重試並把整條 item 包住；重跑後 errors 由 2 降至 **0**，而 `bypass_before` 兩次都是 59（可重現）。
4. **先決條件 (a) `footnote_lead_probe.py` 已跑**（生產端點、`synthesize:false`、**零 LLM 呼叫**）：正控 **26/27** 保住 footnote lead（重疊 min 2／中位 10.5）· 負控 **4/13** 仍取得 lead（重疊上限 3）· errors 0。artifact `dev/source/2026-09-22_s230_footnote_lead.json`。**交接檔長期記的「未跑」缺口已補。**另證明這個候選動不到該探針的觀測量：`synthesize:false` 根本到不了旗標所在的 `synthesizeAnswer()`，所以 row 41(a) 要求的 before→after 對這個改動而言是空對照，**已記錄理由而非跳過**。
5. **先決條件 (b) `judge_probe.py`：修法是講清角色而不是重寫它。** 已在檔頭寫入它量的是校準尺（裸查詢＋全庫）而非閘實際吃的分數，並明文「不得用它驗收 bypass 條件的改動」，同時指向 `_s230_bypassCensus.ts` ＋ `_s230_judgeTakeover.ts` 兩支（行生產路徑＋帶 gold 真值）。`dev/DOC_SYNC_CHECKLIST.md` row 41 的驗收工具欄已同步改寫。**另把那個已知錯標記下而不改動**：`老師病假連續請幾耐先要交醫生紙` 屬 CLASS_B 是錯的（`g04` 原文答得到，S229 已開窗核實），但該清單是人手答案鑰匙、S195 記錄的統計是由它讀出來的 —— 該題在校準尺上是 **0.4964**，改歸 CLASS_C 會令記錄中的正控下限由 0.6241 跌至 0.4964、令已公布的重疊進一步擴大。**這是要連數字一併重述的決定，不是靜默編輯**，故只標示不移動，探針每次跑都會把它單獨印出。
6. **先決條件 (c) 14 條拒答逐條讀窗完成**（4 條 `FALSE_BYPASS` ＋ 10 條當時標為 `WRONG_PASSAGE`）。四條 `FALSE_BYPASS` 的拒答**全部正確**（gold 明言語料無答案）。其餘之中大部分是**正確棄權** —— 答得到那一段本來就不在窗內，改動前照答才是真正的風險。唯一明確損失是 `家長校董 點選`（見第 2 點）。**這是我讀窗的判斷，不是量度。**
7. **先決條件 (d) 兩條新開 bypass 讀完，兩條都良性**：`小學 STEAM 教育` 的窗內第 2 格逐字載有 gold signature 的內容（只是被切片邊界斬開，且來源是 `pri_curr_guide_2024` 而非 gold 期望的 `g06`）；`幼稚園可以限制派發K1申請表數目嗎` 的第 0、1 格都是 `k1_admission_2627` 的 curated footnote。**但下一點把這兩條一併消滅了。**
8. **量出延遲之後把候選收緊：旗標由「取代」改為「疊加」。** 實測那次額外單行讀取（36 個交錯樣本，用普查裡真正被閘讀過的片段 id）：**中位 288ms · p90 639ms · 最慢 1,204ms**（本機；Risks 2(a) 記本機端到端比 Render 慢 1.5–2 秒，單次往返未必同比例，故視為指示性上限）。而第一版在「領先片段是 `vault_extract`」就讀，普查量到那是 **168/185（91%）** 的查詢。**改為只在舊閘本來會放行時（applied ≥ 0.70）才讀**，一次過解決兩件事：額外讀取由 91% 降至 **32%**（59/185），且**候選再也不可能新開 bypass**（第 7 點那兩條隨之消失，因為它們是 applied < 0.70 而 bare ≥ 0.70）。**已量的數字意義不變** —— 判官接手那批對 59 條只用了 53 次呼叫，即「兩道閘都過」的正是保住 bypass 的那 6 條。**✅ 全 185 條確認跑數（`..._bypass_census_conjunction.json`）逐條對得上**：新開 bypass **0**（兩條已消滅）· 改後仍 bypass **6**（與 53 次呼叫吻合）· 額外讀取由 168 條降至 **58**（91% → 31%）· errors 0。⚠️ 該次 `bypass_before` 是 58 而非 59，差的一條是 `半日制 24 班 教師人數` —— **不是收緊造成**，而是 establishment lexical overlay 屬 best-effort、該次沒有命中，於是領先片段不再是那條記分 1.0 的列；屬本專案已記錄的來源層 ±1 重跑噪音（見 `## Current Baseline` 7）。同因 `SOUND` 在該次計為 14 而非 15。**兩次的 `FALSE_BYPASS` 都是 5、`WRONG_PASSAGE` 都是 39。**

**S230（2026-09-22）第八批 —— 奉命「先修好判官再啟用」。結論：那條錯判修不了；而本節一度把一個已知狀態誤報成新缺陷，已收回。** 工具 `backend/scripts/_s230_judgeModelProbe.ts`，**共用 6 次判官呼叫**（3 條 × 2 個模型，碼內硬上限 6；提示由生產自己產生，零重寫、零重建窗）。

1. **那條 SOUND 拒答（`家長校董 點選`）換模型修不了。** `gpt-4.1-mini` 與 `gpt-4.1` **兩個都判「否」**。與 S211 的記載一致 —— 該節為「領先片段逐字寫住規則而判官仍然拒答」這個形狀試過五次改寫提示再加 `gpt-4o`，全部失敗。**所以「先修好判官」不是小補丁，是有前科的難題。**
2. **換模型是嚴鬆交易，沒有免費午餐**：`gpt-4.1` 在 D01 判「否」（正確）而 `gpt-4.1-mini` 判「能」；但 `人工智能初探`（answer 半邊、`want: 能`）剛好相反。3 條樣本不足以下結論，但足以否決「換模型即可」。
3. **🔴 本節收回的三項斷言（重要，因為它們曾被寫進 Risks 並曾據此要求 Leonard 去查 Render）**：曾斷定 (i)「S211 之後判官驗收數字沒有描述過真正在跑的判官」、(ii)「碼內預設那個判官在凍結驗收集上出假答案，屬 row 46 一票否決的未報缺陷」、(iii)「Render 的 `JUDGE_MODEL` 只有 Leonard 查得到，要先去看」。**三項皆錯。** `dev/CODEBASE_CONTEXT.md` 明文記載 S211 已把判官分拆到自己的 `JUDGE_MODEL`、**Render 不需條目、程式預設就是在跑的那個**，「無法由外部讀取」這個問題對判官而言已消失；`CHANGELOG.md` 更記載 S211 **確實**用凍結集量過 `gpt-4.1-mini`（主集 31/33 打平、answer half 12/12 打平、decline half 19/21 打平、**相同的兩個 false answer D01／GN10、無新增虛答**）；而 D01 是 S200 已記錄並在 decline-half override 下接受的既有狀態。
4. **成因（要記住的那一條）**：只讀了 `judge_acceptance.py` 的檔內註釋（`AGENTS.md` §2 第 5 級來源）就下結論，未查 `dev/CODEBASE_CONTEXT.md`（第 3 級）—— **來源優先次序倒轉**。而那個註釋本身確實過時，所以它「看起來」像可信的現況。
5. **真正剩下的發現，比原本小得多**：(a) `judge_acceptance.py` 檔頭過時（已在檔內標明並保留原文）；(b) 它的預設仍是 `gpt-4o-mini`，**忘記傳 `JUDGE_MODEL` 就會量錯舊配置**，已加執行期警告；(c) `dev/source/judge_runs/` 內**零個 `gpt-4.1-mini` 的 run artifact**（13 個 `gpt-4o-mini`、2 個 `gpt-4.1-nano`），S211 那組數字只以 CHANGELOG 散文存在 —— 證據保存缺口，也正是誤讀的來源。
6. **待核實、不下判斷**：S200 那個 override 的理由是「D01 是 footnote-bypass 案例，判官從不服務它」，但 **S201 已移除 footnote bypass**，若如此該前提已失效。
7. **對「改好後再啟用」的影響**：第 1 點成立 —— 那條錯判不是換模型能修的，而 S211 的前科說明改提示的成功率不高。**旗標仍未設。**要繼續就要正面處理「判官拒答明明答得到的段落」這個老問題，而不是再找一個訊號或換一個模型；下一批要跑的是 `judge_acceptance.py` 對 `gpt-4.1-mini` 的一次 run（**35 次呼叫**），把 S211 那組只存在於散文的數字補回 artifact，作為任何判官改動的有效基線。
...
**但這兩條 gold 的片段層判決仍是 FAIL，而且不應再當成路由問題。** 逐條量過該通函全部 8 片段對查詢的餘弦：兩個答案段落在**它自己那份文件內**排第 6（0.5919）與第 5（0.5943），前三名是 0.7111／0.6868／0.6659，而每源配額是 3。修路由之後窗由「SAG 假期段落 ＋ 1 條 KG 片段」變成「3 條該通函片段 ＋ 幼稚園行政手冊」——實質改善，但 gold 量不到。**這兩條已改屬 OP④ 的配額家族。**

**第二批 —— OP②：機制、劑量曲線與噪音底線。** `expandQuery()` 一直把展開詞原文照貼，不論查詢多短，於是用戶自己的字在被 embed 的字串裡佔比隨查詢變短而下降。本專案已經為同一個機制付過三次帳（`staffing` 0.816→0.616、`staff_appraisal` 0.5884→0.4881、本節的 `kg_relocation_grant` 0.5919→0.4994），每次都是「拆一條新路由出來、不設展開」逐條醫。兩個旋鈕（生產一律未設，未設 = 逐位元組等同改動前）：`EXPANSION_MIN_QUERY_CHARS` 短於 N 個碼位就不貼展開詞；`EXPANSION_BALANCE` 保留展開詞，改為重複用戶查詢令它佔至少 φ 的字數比例（`repeatsForBalance()` 解 `rQ/(rQ+E) ≥ b`，上下限 `[1, EXPANSION_REPEAT_CAP]`，預設 cap 8）。

六個 arm、每個 185 題、生產配置 `FEATURE_ROUTE_FIRST_SEARCH=1` 兩側都開，判分一律交回 `_s219_score_before_after.py`（NFKC fold 單一來源）：

| arm | 窗口有改 | 來源層 Δ | 片段層 Δ | 逐條改善／退步 |
|---|---|---|---|---|
| `floor=6` | 10 | ±0 | **+1** | 1↑ / 0↓ |
| `floor=8` | 26 | +1 | **+5** | 5↑ / 0↓ |
| `floor=10` | 51 | +4 | **+5** | 6↑ / **1↓** |
| `balance=.35` | 71 | +1 | **+2** | 2↑ / 0↓ |
| `balance=.5` | 79 | +3 | **+4** | 4↑ / 0↓ |
| **`floor=8` ＋ `balance=.5`** | 81 | **+3** | **+7** | **7↑ / 0↓** |

**噪音底線是量出來的，不是估的**：七個獨立 before 側（同一配置、七次重跑）片段層 `chunk_PASS` **全部 59**，來源層 123／123／123／123／123／122／123。**即片段層的重跑噪音為 0、來源層為 ±1**，所以片段層 +7 遠在噪音之外，而 `floor=6` 的 +1 只勉強站得住。`floor=10` 出現 1 條退步，是劑量曲線的轉角，不應再往上調。

**改善集中在短查詢**，與機制預測一致：合併 arm 的 10 條改善裡，`plain`（1–3 詞）佔 5、`abbrev` 1、`natural_sentence` 1，另 3 條是來源層。逐條：`act_lwl_grant_uses`（全方位學習津貼）· `gifted_2e_support`（雙重特殊資優學生 支援）· `bus_driver_punctual`（校車司機 守時）· `kg_fullday_priority`（全日制幼稚園收生優先考慮哪些家庭）· `fin_home_school`（家校合作津貼）· `fin_dls`（多元學習津貼）· `gov_imc_abbrev`（IMC）片段層轉 PASS；`cur_secondary_central_curriculum`· `gifted_tier3_outside_support`· `kg_teacher_pupil_ratio` 來源層轉 PASS。**零退步**。

**實際影響面（由 artifact 逐條量，不是推算）**：185 條之中 **88 條**路由本來就無展開詞、完全不受影響；**26 條**短查詢被 floor 擋走展開；**71 條**保留展開並重新配重，其中 **17 條**重複次數觸及 cap=8，因而達不到 50% 目標（實際佔比中位 50%、最低 31%、最高 75%）。**cap 會頂住的是展開詞最長那幾條路由**（`digital_education` 152 字、`value_education` 130、`sen` 118）。

**量度過程中修好一個會導致錯誤結論的儀測缺陷**：harness 第一版以 last-write-wins 記錄「送去 embed 的字串」，而 `searchChannelB` 每側 embed 兩次（展開後查詢＋spotlight 用的裸查詢），於是每個 arm 都被記成 `arm-noop`。若照那個輸出下判斷，結論會是「機制沒有用」，實情是「機制沒被看見」。現已改為逐條記錄全部字串，並已把同一儀測移植回 `_s226_knobAB.ts`（連同 `knob_changed_embedded_input` 統計），另在 `DOC_SYNC_CHECKLIST.md` 新增「查詢展開機制改動」row 第 ④ 條寫死這條要求。

**第三批 —— 答案層判官：合併配置判定不出貨，而且找到了原因。** 合成 A/B 用 `_s226_knobAB.ts --set EXPANSION_MIN_QUERY_CHARS=8 --set EXPANSION_BALANCE=0.5 --synthesize`，81 條窗口有改的題目、兩側各合成一次（`gpt-4.1-nano` 起草、`gpt-4.1-mini` 相關性判斷）；判官 `_s227_rubricJudge.ts`、`JUDGE_MODEL=gpt-4.1`、每題兩個次序各判一次。**先校準才用**：同一校準集 4/7（S227 為 5/7，屬交接檔已記載的跨次變異），位置偏誤兩項皆 0。

**判決（81 條，兩批合併 —— 第一批跑到 66 條撞 `gpt-4.1` 的 30,000 TPM 上限而中止，餘 15 條另跑，合共 154 次呼叫）**：TIE 33 · AFTER 14 · WEAK_BEFORE 13 · WEAK_AFTER 11 · BEFORE 3 · 兩側棄權 5 · INCONSISTENT 2。`judge_health` 由逐條重算：judged 76 · `always_first` **1** · `always_second` **1**（對稱且可忽略）。計分：counted 59、須人手覆核而剔除 **22**、after **9.0** 分、before **5.5** 分、**`net_after` +3.5**，門檻 8 → **`NO_MEASURABLE_GAIN`**。門檻是碼內寫死的，跑之前已定，未改。

🔴 **棄權數 before 8 → after 15，而且 10 條是可答的題目。** 其中 `bus_driver_punctual`、`fin_home_school` 的**片段層明明由 FAIL 轉 PASS**，`kg_teacher_pupil_ratio`、`cur_secondary_central_curriculum` 的**來源層明明轉 PASS**，答案卻變成棄權。

🟢 **原因已量到，不是排序變差，是絕對分數被壓低而觸發下游以絕對分數設的閘。** 展開詞會把查詢向量拉近「長篇主題段落」，所以它一直在**抬高絕對相似度**，同時扭曲相對排序。拆開兩個旗標看（只計旗標真的改動了被 embed 字串那些題目）：**被 `floor` 整段拿走展開詞的 26 條 —— 平均 −0.1563、中位 −0.1296、四分位 −0.1980／−0.0673、最差 −0.5467，26 條之中 24 條下跌；只被 `balance` 重新配重的 57 條 —— 平均 −0.0042、中位 −0.0021、四分位 −0.0147／+0.0052、最差 −0.0426。****即 `balance` 也會壓分，但幅度比 `floor` 細約三十倍，而且從未足以跌穿 0.60。**（另有 14 條路由有展開詞、查詢又不短，但 `balance` 算出重複次數 = 1，字串不變、分數不動；把這 14 條算進去會令中位變成 0.0000，那是稀釋出來的數字，不要那樣報。） 最高分跌穿門檻的條數（`VAULT_LEAD_SCORE` 0.70 合成 bypass／`SPOTLIGHT_LEAD_SCORE` 0.60 overlay 放行）：`floor=6` 7／9 · `floor=8` 15／17 · `floor=10` 28／27 · `balance=.35` 4／**0** · `balance=.5` 6／**0** · 合併 17／17。**即 `floor` 是壓分的那一個，`balance` 完全不壓分。**跌穿 0.70 的題目不再 bypass，改由相關性判斷處理，於是棄權。

**這一項是本節最可搬走的教訓**：`VAULT_LEAD_SCORE`／`SPOTLIGHT_LEAD_SCORE`／`min_score` 全部是在「有展開詞」的分數尺上校出來的，所以**任何動搖絕對分數的檢索改動，都必須連同下游門檻一起重新校**，否則檢索層的得益會在合成閘被吃掉，而兩層檢索指標都不會顯示這件事。

**第五批 —— `balance` 單獨配置也判不出貨，所以展開側兩個候選都已量完並關閉。** 合成 A/B 用 `--set EXPANSION_BALANCE=0.5 --synthesize`，79 條窗口有改的題目；判官分四段跑（27＋27＋16＋9 —— 第三段又撞 30k TPM，餘 9 條補跑），合共 **145** 次呼叫。**判決**：TIE 43 · WEAK_AFTER 12 · WEAK_BEFORE 10 · 兩側棄權 7 · AFTER 4 · BEFORE 2 · INCONSISTENT 1。counted 64、剔除 15、after **5.0** 分、before **3.5** 分、**`net_after` +1.5**，門檻 8 → **`NO_MEASURABLE_GAIN`**。`judge_health`：judged 72 · `always_first` 1 · `always_second` 0。棄權數 before 8 → after **11**（合併配置是 15），其中 3 條可答的題目失去答案、1 條可答的取回答案、1 條**不可答**的題目正確地轉為棄權。

**所以兩個展開側候選都已量完並判定不出貨**，而且 `balance` 的 `net_after`（+1.5）比合併配置（+3.5）更低 —— 兩者都遠低於門檻，**不要把這個高低差當成訊號**，那在判官的跨次變異之內。

🔴 **本節最後的定性（兩個讀法並存，不要只取其一）。** 檢索層的得益是真的（+4 至 +7、零退步、噪音 0），但它沒有在答案層兌現。**讀法 (i)**：被兩個以絕對分數設的常數閘吃掉 —— 棄權上升與 0.70 跌穿數都量得到，而且 `floor` 與 `balance` 的壓分幅度差三十倍、對應的棄權升幅也是 7 對 3，方向一致。**讀法 (ii)**：這個量級的改動本來就堆不出 8 分淨值 —— 79 條之中判官判 **43 條 TIE**，即大部分題目的答案在判官眼中根本無分別，而門檻 8 是 S227 為一個「來源層 +7／片段層 +6」的候選定下的。**削弱 (i) 的證據**：`balance` 幾乎不動分數尺（跌穿 0.60 者 0 條）卻仍然只有 +1.5。**削弱 (ii) 的證據**：兩個配置都有可指名的棄權退步，那不是「無分別」而是「變差」。所以 `## Open Priorities` ② 新增未試方向 (c)（重校門檻），同時把「門檻 8 是否適用於這個量級」列為要先決定的事。**在兩者之一有結論之前，不要再跑展開側的 A/B** —— 會重複本節的結果。

**第四批 —— cross-encoder 重排的上限（OP② 餘下方向的成本評估）。** `backend/scripts/_s228_rerankCeiling.ts` 擷取每題兩個窗（生產 top_8 與寬池 top_40 —— 後者正是 `searchWikiRoutedExact` 已經在做的 over-fetch，`fetchCount = topK*5`，即重排可以插進去而不必再檢索一次），判分交回 `_s213_run_gold.score_item`（`dev/_s228_ceiling_report.py`，TS 側不判分）。**166 條有答案簽名的題目：已入窗 59 · 重排最多救得到 42 · 寬池都沒有、救不到 65。**按類型看，`plain` 77 條之中救得到 27 條，是最大的一塊。

**所以 cross-encoder 的天花板是 +42（片段層 59 → 101 為上限），而 65 條（39%）不論多好的重排都救不到 —— 那 65 條要動的是候選生成（索引／路由／切法），不是重排。**「救得到」是雙重上限：入池只是必要條件，而且寬池的每源配額比生產鬆（`max(2, ceil(k/3))`），兩個誤差都指向樂觀，這是上限該有的方向。

**S227（2026-09-15）** —— 答案層判官（Open Priorities ① 交付物）。**零 Supabase 讀寫、零 `backend/src` 改動、零 flag 改動、零部署**；唯一外部呼叫是判官模型，全部經 `--approved-max-calls` 閘，四批共 **74** 次。

- **交付物**：`backend/scripts/_s227_rubricJudge.ts` —— pairwise rubric judge，重播 S226 已存的 `2026-09-15_s226synth_before_after.jsonl`（38 題兩側答案全文），不檢索、不嵌入、不碰 Supabase。`--self-test` **ALL PASS（67 條斷言）**，單檔 typecheck 0（見下「工具鏈假綠」）。**七次紅測全部準確轉紅、逐次還原後 `shasum -a 256` 與紅測前相同**：側別解析反轉（4 條轉紅）· 兩次次序相反被吞成 TIE（2 條）· prompt 洩漏側別（1 條）· 位置偏誤計數器由 AND 改 OR（2 條）· TIE 授權被移走（1 條）· WEAK 由半條當成一條（3 條）· 覆核標記只看單一次序（1 條）。
- 🔴 **判官第一版是壞尺，而且它「看似」有 3/7 成績。** `gpt-4.1-mini` ＋ 直接比較：14 次判決 **11 次揀先出現那一個**，**TIE 零次**，4 條 `INCONSISTENT` 全部是「兩次都揀 ANSWER_1」。**只跑單一次序就會收到一份 7/7 的漂亮報告，而它量的是位置不是內容。**這就是每題兩個次序各跑一次的理由。
- **輪二的機制修正（per-answer 先評分 ＋ 明確授權 TIE）方向對、模型撐不住**：位置偏誤由「永遠揀第一個」翻成「永遠揀第二個」（`always_second` 3），一致率 3/7 **跌至 1/7**。**鐵證**：三條題目的同一個答案，在位置 2 時被列缺陷、在位置 1 時缺陷清單變空 —— **缺陷清單是位置的屬性，不是答案的屬性**。
- 🟢 **換 `gpt-4.1` 判官後位置偏誤清零**（同一 prompt、同一校準集）：`always_first` **0** · `always_second` **0** · `INCONSISTENT` **0** · 14 次判決有 7 次用 TIE · 與人手一致 **5/7**。**變數是模型，不是 prompt** —— 輪二已證明再改 prompt 救不了 `mini`。
- ⚠️ **判官已定位的盲點：它會自行推算數字關係並算錯。** `fin_ac_grant` 被它判成自相矛盾（「無禮堂有蓋操場＝2 個特別室＝$42,526」對「標準禮堂＝2.5 個特別室＝$53,158」），實際 $53,158 ÷ 2.5 × 2 **正好** $42,526，人手判斷才對。同一條準則另一個方向它又準確捉到 `dig_teacher_digital_cpd_hours` 的真缺陷。**故此凡判決依賴準則 3（數字與登記段落不符）者一律自動標 `needs_human_review`，標記在看到總數之前逐題落，不可事後選擇性套用。**
- **校準集本身經得起覆核**：`fin_ac_grant` 逐項開原文核算術，人手方向正確。**但人手那 7 條在全跑中只對 4/7（校準輪是 5/7），即判官跨次跑有變異** —— 這一點寫在此處，不要當成 5/7 是穩定值。
- 🔴 **`FEATURE_LEXICAL_RERANK` 答案層量度完成：測不出改善，判讀 `NO_MEASURABLE_GAIN`，不應出貨。** 38 題（60 次呼叫）：**兩側皆棄權 8**（與 S226 記載的 8 對上）· **TIE 17** · 偏 before 7（`BEFORE` 2 ＋ `WEAK_BEFORE` 5）· 偏 after 5（`AFTER` 1 ＋ `WEAK_AFTER` 4）· `INCONSISTENT` 1 · 剔走 6 條須覆核後 **net_after −1**（門檻 8）。**38 題之中 25 題毫無差別，而 S226 量到「答案文字有變 30/38」—— 文字變了，質素沒變。**
- **結論對判官雜訊穩健（先算才敢下結論）**：把 6 條須覆核題**全部當成 after 完勝**，net 只去到 **+5**；連 `INCONSISTENT` 那條都算給 after 是 **+6**；仍低於門檻 8，而實測是 **−1**。要翻盤需再多兩條，而餘下全部是 TIE 或兩側棄權。
- **判讀門檻在跑之前寫死在碼內**（`SHIP_THRESHOLD = 8`、`WEAK_*` 只計半條、須覆核題剔出分母），並有 6 條斷言守住。**理由是 S226 的教訓：事後才定怎樣詮釋數字，等於容許用壞尺量出對自己有利的結論。**
- ⚠️ **工具鏈假綠（本節順手揪到，不屬本節範圍）**：`npx tsc -p backend/tsconfig.json` 的 `include` 只有 `src/**/*.ts`，**`backend/scripts/*.ts` 十二支工具從來不在 `npm run check` 覆蓋範圍內**。本節第一次跑 typecheck 報「零錯」其實是空過，要單獨指檔才真正檢查。已入 Backlog 治理側。
- 🟢 **【S227 第二批，2026-09-16】拆出 `staff_appraisal` 路由，把 S226 只醫好一半的缺陷補完。** **成因**：S226 把「教師評核」等五個詞加落 `hr_admin`，查詢不再跌落 `curriculum`（生產 run 實錄當時回傳體育／中文／資訊科技課程指引）；但 `hr_admin` 的 `QUERY_EXPANSIONS` 是假期詞彙（`教職員假期 批假 病假 首年 168日 醫生證明`），排序向量因此變成一條**假期查詢**，生產八格全是病假產假。**實測（同一批九條片段，兩個向量）**：答案段落（SAG §7.7 員工考績 p.208，`vault_sag_2025_11_a40e4ed7605169ac`）加展開後 **0.4881** 排第 9（第 8 名 0.6088），裸查詢 **0.5884** 排**第 1**，所有假期段落跌至 0.33–0.40。**展開不是幫不到忙，是把答案由第 1 推到第 9。**
- **修法照 `staffing` 先例**（該路由當年正是為同一理由由 `hr_admin` 拆出，碼內記載 0.816 → 0.616 的同型損失）：新增 `TOPIC_KEYWORDS.staff_appraisal`（排在 `hr_admin` 與 `curriculum` 之前）＋ `SOURCE_SETS.staff_appraisal`（五個來源，成員由全庫詞頻點算得出，**刻意排除** `g13`／`bafs_sss_*`／`ict_sss_2021` —— 它們的「教師評核／員工考績」是科目課程脈絡，收進來等於重建 S226 剛清走的課程污染），並**不設 `QUERY_EXPANSIONS` entry**；同時把 S226 那五個詞由 `hr_admin` 移走以免死碼。
- **QC（逐項實測）**：`npm run check` 0 · `npm run build` 0 · `route_regression` **54/54**（由 50 增至 54，新增四條對照組專責證明新路由沒有搶走 `hr_admin` 的「語文能力評核」、`qa_inspection` 的「校外評核」、`curriculum` 的裸「評核」）· **紅測**：在新路由加入裸「評核」→ **準確轉紅 4 條**（含原有的「視學同校外評核分別」），其餘 50 條零波及，還原後 `shasum -a 256` 與紅測前相同 · **blast radius**：185 條 gold query 掃改前改後，**只有 1 條改變路由**，正是目標 `hr_appraisal` · **行為實測（生產配置 `FEATURE_ROUTE_FIRST_SEARCH=1`）**：答案段落**由窗外第 9 進入第 5（0.5884）**，八格全部考績來源（`supply_teacher_guide`／`sag_2025_11`／`perf_indicators_2022`），一條假期都沒有。
- ⚠️ **量度陷阱（中過一次，已寫入 `dev/DOC_SYNC_CHECKLIST.md` 新 row）**：`_s226_captureQueryVec.ts` **不設 `FEATURE_ROUTE_FIRST_SEARCH`**，而生產是開啟。第一次量 after 時忘了加，量到的窗完全不同（`sag_2025_11` 整份消失、只回傳 6 條）。**凡用該工具做前後對照，必須由外面設這個變數。**
- 🔴 **【本節自我否決】「查詢展開拉偏」不是系統性問題，不要照這個方向改機制。** 拆完路由後順手量了 S226 那 63 條「來源層 PASS、片段層 FAIL」：**37 條走有展開的路由，其中展開令答案段落分數上升 29 條、下降 8 條，中位 delta `+0.0838`**；「裸查詢入得到窗、加展開後入不到」**只有 1 條**。**即展開整體是幫忙的**，`hr_appraisal` 是個案而非通例 —— 本節一度把它當成 ② 的第三條方向，實測後收回。（另有 2 條對照組失敗已排除、未折入總數：`act_priority_values_five_confusable` 中位誤差 0.05、`dig_cloud_privacy_pcpd` 無回傳窗。）證據：`s227_expansion_effect.json`（scratchpad，數字已抄錄於此）。
- ⚠️ **同族但不同案例，已定位未修**：`kg_relocation_grant`（query「幼稚園搬遷津貼」，應走 `edbcm144_2026`）命中了 `hr_admin` 的「**津貼**」，於是同樣被假期展開拉走 —— 裸查詢 0.5919 高過第 8 名 0.5879，加展開後跌到 0.4994。**本節不改**：動「津貼」這個 token 會牽動 `hr_ncs_allowance` 等題，要另外量度，已交由 `## Open Priorities` 承載。
- **artifact**：`dev/source/eval_runs/2026-09-15_s227cal_rubric_judge.json`（輪一 `mini`）· `..._s227cal2_...`（輪二 `mini` 修正版）· `..._s227cal3_...`（輪三 `gpt-4.1` 校準）· `..._s227full_...`（全 38 題）。每份都帶 `judge_health` 四欄、`needs_human_review` 清單、`decision` 判讀區塊與輸入檔的 SHA-256。

**S226（2026-09-15）** —— 逐項證據見本節下方檔名；原始擷取檔逐題留有每個 RPC 的狀態碼與毫秒。

- **工具先驗後用**：`_s219_score_before_after.py --self-test`（6 條，本節加至 11 條）· `_s213_run_gold.py --self-test` 7 條 · `eval_retrieval.py --self-test` · `_s213_eval_metrics.py --self-test` · `qc_report.py --self-test` 全部 ALL PASS。**延遲分析器先讀 S220 舊擷取檔驗證**：重現出交接檔記載的 routed p90 **8,335ms**、`no-route` **56**、route failures **9**（partial 7 ＋ silent-fallback 2，與該 run meta 的 `route_failures: 9` 對上），才用它讀新數據。
- **兩支 harness、兩個問題**：`routeFirstGold.ts`（本機 in-process，同一次 embedding 餵兩側，隔離旗標）答「旗標與語料變動有沒有令檢索退步」；`_s213_run_gold.py`（打生產端點、7 秒節流）答「生產實際幾多分」。**185/185 兩支都跑完**，前者 errors 0、後者 errors 1。
- **旗標效應（本機，關閉 → 開啟）**：來源層 PASS **119 → 122**、FAIL 47 → 44（FAIL→PASS 5 條、PASS→FAIL 2 條）· 片段層 chunk_PASS **54 → 58** · recall@8 **0.325 → 0.349** · 端到端中位 **2,476 → 1,260ms**、p90 3,669 → 2,601ms · routed p90 **2,498ms** · **route failures 0**（S220 是 9）· 超過 8 秒的 RPC **0**（S220 是 18）· 超過 3 秒（anon 真上限）由關閉側 37/185 降至開啟側 13/185 · `no-route` 仍然 **56** 題。
- **語料變動效應（S223 SAG 換版 ＋ kgecg 去重，09-08 → 09-15，同一旗標側比同一旗標側）**：生產配置側（兩邊開啟）**VERDICT_REGRESSED 只有 1 條**（`hr_mpf`），PASS 123 → 122；關閉側 3 條（`hr_mpf`／`hr_pay_adjust_kg`／`kg_teacher_pupil_ratio`），122 → 119。片段層反而 **0.343 → 0.349**（開啟側）。**這就是 S203 PLAN 欠下、S223 取不到 before 基線的那一步，現時用同一旗標側的跨日對照補回。**
- **那一條退步逐項查清（不是換版丟內容）**：`hr_mpf`（查詢「公積金 MPF」）09-08 的 PASS 是靠 **`g24`** 以 0.5098 排第一位撐住，而 `g24` 正是 S223 按計劃退役者（實查現時 **0** 條片段）；它的**主要**預期來源 `edbc00030` 09-08 與 09-15 **兩次都不在前八**，即這題換版前就已答不中本意，只是靠替代來源記了個 PASS。內容仍在庫：新版 SAG **23** 條含「公積金」、**6** 條含「強積金」，`edbc00030` **70** 條、其中 **1** 條帶 gold 簽名。**判定：短查詢排序問題，非資料損失。**同族證據：按題型拆 FAIL 率 `plain`（1–3 詞）**24/77**、`typo` 3/6、`english` 3/9，而 `natural_sentence` 只有 6/43。
- **生產真相與實驗室的差距量到了**：生產 errors **1/185**（`sen_iep_intellectual`，`57014` statement timeout），本機 **0/185**。這正是 Risks 2(b) 警告的方向 —— 本機用 service key 在 8 秒上限下量一個 anon 3 秒的系統，**系統性低估失敗率，實測低估幅度為 1 題**。
- **片段層第一次量到**：166/185 題有段落簽名，生產 chunk recall@8 **0.349**，即約三分之二情況下回傳的八條片段**沒有**真正答到問題那一段；其中 **64 題來源層 PASS 但片段層 FAIL**（拿對文件、拿錯段落）。已按紀律開實例親眼看：`act_kayak_ratio` 窗內 `sch_activities_guide` 出現四次，但「獨木舟 1 名持有有效資格人士:8 名參加者」那一段不在任何一條片段內；對照 `act_national_edu_confusable` 該段在第 0 位。
- **19 條無答案題**：零誤判 PASS。棄權行為另有兩項瑕疵（一條過門檻、一個 forbidden hit），已交由 `## Open Priorities` ④ 承載 —— 同一事實不放兩個家。
- **dev 與 held_out 分開報**：dev PASS 77/125（FAIL 33）· held_out PASS 45/60（FAIL 11）。**held_out 反而較高**，成因未查，不編機制。
- **工具修正（本節唯一的碼改動，兩支 harness 共用一個定義）**：`summarize()` 併入 `_s213_run_gold.py`（`score_item` 旁邊）並由 `_s219_score_before_after.py` import；summary 現時帶 `RECORD_ONLY` ＋ `chunk_PASS`／`chunk_FAIL`／`chunk_RECORD_ONLY`。新增 5 條斷言（含來源層與片段層兩條守恆、一條「無斷言題目不得抬高分母」）。**先證它會紅**：注入 S225 同族缺陷（`chunk_FAIL` 硬寫 0）→ 3 條轉紅；還原後 `shasum -a 256` 前後相同（`c1e3f57f…`）。
- **封版閘前後對照**（重跑 `--check` 實測）：`EVAL_CHUNK_LAYER` **NOT_MEASURED → FAIL**（chunk PASS 54／FAIL 112）· `NOT_MEASURED` 項數 **1 → 0** · `NO_UNMEASURED` **NOT_MET → MET** · releaseGate **4/15 → 5/15** · ERROR 3 → 4 · `overallStatus` 兩邊都是 `ERROR` · `qc_report.json` 逐行 diff 只有 12 加 13 減，**其餘檢查零變動**。⚠️ **那 +1 是量度覆蓋率，不是檢索質素** —— 沒有任何一格由紅轉綠。
- **生產寫入**：零。**`backend/src` 改動**：零。**flag 改動**：零。**DDL**：零。外部呼叫只有 embedding（`text-embedding-3-small` 283 次）與唯讀搜尋。
- **第二批（Leonard 自行重建語料快取、並拍板選項 A 之後）**：
  - **gold 標籤重驗 —— 自 S213 之後第一次。** 183/185 → 重錨一條後 **184/185**，隔離 1。兩條之所以壞，成因相同（`verified_by.chunk_id` 指向 S223 換版時刪掉的片段），但可修性不同：`cpd_mainland_promotion_tour` **純機械重錨**（簽名一字未改、在活語料全庫唯一命中 `vault_sag_2025_11_7442089094f5d8aa`、頁碼 188 → 191 因為新版是另一份 PDF）；`hr_appraisal` **不予重錨、維持隔離**，理由見下。**重錨對判分零影響已實證**：重跑計分器後兩側 summary 逐項相同（119/47/54/112 與 122/44/58/108），唯一 diff 是重生時間戳，已還原。
  - 🔴 **`hr_appraisal` 這條 gold 本身有問題，不是錨點問題。** query 是「教師評核」、intent 寫「查教師表現評核相關指引」，但簽名 `評核方面:f.職員或其好友的子女或親屬申請入學,職員親自面試,並建議` 是**收生／學生表現評核的利益衝突例子**（已開原文讀過：`vault_sag_2025_11_737b00beaa28d546` p.237 與 `vault_icac_school_governance_86fce12915ae4093` p.76 都帶這段），**答不到「教師評核」**。另有一個次要成因：舊簽名的冒號來自已消失的 footnote overlay 排版，活語料是「收生/學生表現評核方面f.」無冒號。**重寫 query 抑或換一段真正講教師評核的段落，是「甚麼算正確」的定義，屬 Leonard 的決定** —— 依 row 60「不通過即隔離，不可靜靜修好」，本節不動它。
  - **完整指標模組首次跑得動**（數字見 `## Current Baseline` 7）。**根因已修**：`_s213_corpus.py` 的 `DEFAULT_CACHE` 本來是某一節 scratchpad 的絕對路徑 —— 那個目錄必然消失，所以這套工具**保證**每節壞一次；已改為 repo 內 `dev/source/.store_cache.json`（28MB 生成檔，已入 `.gitignore`），`S213_STORE_CACHE` 仍可覆寫，重建指令寫在該常數旁邊。**這就是 gold 標籤兩節沒重驗的機械原因。**
  - **選檔改為優先生產端點（Leonard 拍板 A）**：新增 `measures_production()`（`endpoint` 以 http(s) 開頭且非 localhost）與 `eval_source_kind()`；`latest_eval_run()` 在合格 gold run 中優先揀生產那一份，無生產 run 時照舊揀最新並在 detail 明寫「非生產端點」，另會列出未被採用的較新 run。**新增 7 條斷言全綠**，**紅測**：把選檔打回 `runs[0]` → 該條準確轉紅，還原後 `shasum -a 256` 相同（`f1dda0bd…`）。
  - **閘現時報的是生產數字**：`EVAL_LATEST` detail 逐字為「S213 Phase 1 gold-set run · PASS=121 FAIL=44 RECORD_ONLY=19 errors=1 · 共 185 條 query · 量度自 `2026-09-15_s226_prod_gold.json` · 生產端點；⚠️ 另有更新的非生產 run（`..._route_first_before.json`）未被採用」。`EVAL_CHUNK_LAYER` 同步由 54/112（實驗室關閉側）變為 **58/107**（生產）。**閘的計分零變動**：仍 5/15、ERROR 4、`NOT_MEASURED` 0、overall `ERROR` —— 兩側本來都是 FAIL，改的是它在報哪一個配置。
  - **六支工具自測全綠**：`_s213_eval_metrics`／`_s213_run_gold`／`_s213_validate_gold`／`_s219_score_before_after`／`qc_report`／`check_registry_drift`。本節共新增 **12** 條斷言、做過 **2** 次紅測。
- **第三批（③(b) 兩個 registry ERROR）**：
  - 🟢 **`REGISTRY_SERIES` 是假 ERROR，已修 —— 成因是閘漏傳一個參數。** 起手時已發現兩個工具口徑不一（`check_registry_drift --check` 報 `SERIES_UNMONITORED` **0**，封版閘報 **1**），當時記為「未核」。實況：`classify()` 的第七個參數 `series_monitored` 預設為空集，而 `qc_report.check_registry_drift()` **只傳到第六個**，於是每個在服務的年度系列都被歸為「監察讀不到」。`check_registry_drift.py --check` 一直有傳 `monitorable_parents(sources)`，所以兩邊在同一語料上得出不同答案 —— **那個分歧本身就是 bug，不是定義差異**。**改前先核實監察真的讀得到**：`stat_enrolment_report` 的 13 個年份展開成 **13 條各不相同的 http URL**，13 個分片 **全部在服務**（合共 **528** 條片段，與交接檔記載相同），`is_monitorable()` 回 True。修法：補傳第七個參數。**新增 2 條斷言**（一條盯住呼叫點本身，因為回傳的數字分不出「沒有系列」與「漏傳參數」；一條確認該集合非空，否則斷言只是在比較兩個空集），**紅測**：再次漏傳 → 該條準確轉紅，還原後 `shasum -a 256` 相同（`841d5712…`）。**閘前後**：PASS 12 → **13**、ERROR 4 → **3**、`REGISTRY_SERIES` FAIL → **PASS**；releaseGate 仍 5/15（`NO_ERROR_FAIL` 仍被餘下三個 ERROR 擋住）。
  - 🔴 **`REGISTRY_UNMANAGED` 是真缺陷，但修它要生產寫入，未得批准不做。** 實況（已逐條開出來看）：registry 有 `stat_integrated_edu`（5 條、`status=verified`），而庫內另有 **2 條掛在 `stat_integrated`**，兩者**同一標題、同一 URL**（`Integrated_Education_TC.xlsx`）—— 即入庫時的 id 錯配。**兩批內容不同、不是重複**：已登記那 5 條是 53–72 字的敍述式摘要句（例如「2025/26學年公營普通小學有特殊教育需要（SEN）學生總數為33,820人…」），未登記那 2 條是 466／487 字的原始表格傾印（tab 分隔數字），且其中一條由表格中段開始、無標頭（`0\t9000\t9290…`）。逐字雜湊比對：兩批**零重疊**，未登記那 2 條也不是已登記片段的子串。**建議修法（要 Leonard 明示批准）**：把那 2 行的 `source_id` 由 `stat_integrated` 改為 `stat_integrated_edu`（UPDATE 2 行，非 DELETE，舊值已知故可完全回退），改完 7 條同屬已登記來源、受三個監察覆蓋。**次要問題另計**：那條無標頭的表格中段片段本身是 `NO_MIDCLAUSE_START` 家族，重貼標籤不會令它變成好答案材料；若要徹底處理應重抽該 XLSX。
- **第四批（片段層診斷 —— 機制已找到，未動任何檢索邏輯）**：
  - 🟢 **不是切片問題。** 63 題「來源層 PASS、片段層 FAIL」逐題查過答案段落在庫內的位置：`NO_CHUNK_HAS_IT` **0** —— 即每一段答案都**完整存在於某一條片段內**，沒有一段被切割邊界切斷。`TARGET_RETRIEVED` 亦為 0（即計分器沒有錯判）。分類結果：**`SOURCE_CROWDED` 61 · `SOURCE_ABSENT` 2**。**所以重新切片是錯的方向**；問題在同一份文件內部揀哪一條片段。
  - 🔴 **主因是 per-source 上限把答案本身趕走 —— 16/61 題（26%）。** `searchChannelB.ts:1468` 的 `maxPerSource = Math.max(2, Math.ceil(top_k / 3))`，top_k=8 即**每個來源最多 3 格**。實測那 16 題：目標片段的餘弦**高過第 8 名**，而它的來源**已佔滿 3 格**，所以它是「以分數論入得了前八、但不是自己來源的前三」而被上限剔走。最極端兩題：`sec_cloud_shared_responsibility` 目標 **0.5756** vs 第 8 名 0.3781（高 **0.1975**）、`dig_cloud_privacy_pcpd` 目標 0.6480 vs 0.4886（高 0.1594）。該上限的存在理由寫在碼內（防 SAG 415 條壟斷窗口），**所以方向不是拆掉它，而是令它不要剔走該來源分數最高的答案**。
...
- **第六批（Leonard「全做」授權的四項；兩個新 knob 都預設關閉）**：
  - 🟢 **詞面重排是本節唯一兩層同時改善的改動。** `FEATURE_LEXICAL_RERANK=1`（權重預設 0.05）：來源層 PASS **122 → 129**（+7，**7 條修好、零條退步**）、片段層 chunk_PASS **58 → 64**（+6）。**它連本節早上量到的兩個缺陷都一併修好**：`hr_mpf`（SAG 換版唯一的退步）與 `cur_secondary_central_curriculum`（route-first 兩條退步之一）都由 FAIL 轉 PASS。**零額外資料庫成本**（重排的是已經 over-fetch 的 40 條候選）。
  - 🟢 **三點劑量曲線已量，拐點在 0.05，而且是片段層決定的。** 三個權重全部**零退步**：

    | 權重 | 來源層 | 片段層 | 合成器輸入有變 | lead 換人 | 判官繞過翻轉 |
    |---|---|---|---|---|---|
    | 0.02 | 122 → 126（+4） | 58 → 61（+3） | 13/185 | 0 | 0 |
    | **0.05** | 122 → 129（+7） | 58 → **64（+6）** | 38/185 | 2 | **0** |
    | 0.10 | 122 → 132（+10） | 58 → **64（+6，停滯）** | 61/185 | 7 | **1**（`dig_teacher_digital_cpd_hours`） |

    **讀法**：來源層一路升到 +10，但**片段層在 0.05 之後停滯** —— 即再加權重只買到「正確文件到了」，買不到多一段答得到的內容，同時把窗口擾動由 38 題推到 61 題，並開始翻轉判官的常態繞過。**片段層才是可信的那一半，所以建議 0.05：它拿到全部片段層增益，而判官閘零翻轉。** 附帶一項：0.10 會把 `gov_fin_mgmt` 也修好（route-first 另一條退步），即兩條 route-first 退步在 0.10 下全部消失 —— 但代價是上表右邊三欄。
  - 🟢 **合成側的影響半徑已量（零 LLM 成本）**：重排不改分數、只改次序，所以要問的是「餵給合成器與判官閘的東西變了幾多」。由 A/B 原始檔直接算（`selectPrimaryEvidence` 的規則是取前 5 條 `vault_extract`）：**合成器輸入有變 38/185 題（20.5%）· lead 片段換人只有 2 題 · 判官常態繞過（lead 分數 ≥ `VAULT_LEAD_SCORE` 0.70）的決定翻轉 0 題。** 即影響半徑有界。**但這不是答案質素的量度** —— 逐題比較答案文字仍要一次 `synthesize:true` 的 A/B（約 740 次 LLM 呼叫），那一步未做。
  - ⚠️ **但要讀準那 +7 的成分。** 逐條開實例看過：新入窗的片段來自**正確文件**，卻不一定是**答得到的段落** —— `kg_kg_abbrev`（「KG 收生」）新入窗的是 `k1_admission_2627` 講註冊證申請細則那段，與問題只是鬆散相關，片段層仍然 FAIL。所以**可信的那一半是片段層 +6**，來源層 +7 之中有一部分只是「文件到了」。這正是 `_s213_eval_metrics.py` 開頭警告的那件事（窗內任何位置命中即 PASS 是回歸偵測器，不是準確度）。
  - 🔴 **族群配額不成立。** `FAMILY_QUOTA=1`（把校車五版、價值觀兩版、中史兩版各併為一個配額桶）：來源層 **122 → 121**（1 條退步、零條修好）、片段層**完全不變**（58/108）。退步那條是 `bus_driver_punctual` —— 把五個版本壓到共 3 格，反而把答得到的那一版擠走。**所以「五個對象版本吃掉 69 格」這個觀察雖然真，但共用配額不是解法**；真正要處理的是那五份文件本身該不該各自獨立入庫與瀏覽。
  - **實作上一個要記住的教訓（已寫入碼內註釋）**：重排第一版照 footnote lead 的做法用候選集自己做 DF 語料，結果**把訊號本身濾走** —— 候選全部來自路由已選的來源，用戶打的字在裡面很常見，被當成 stopword。smoke 3/3 零改動就是這個。改為直接用查詢 bigram、按查詢長度正規化。**在已收窄的候選池內，「這條片段真的有用戶打的字」是訊號，不是噪音。**
  - **另一個要記住的邊界**：repository 收到的 `query` 是**展開後**的查詢（最多 21 個路由詞彙），所以重排另加 `rerankQuery` 傳原查詢進去 —— 展開詞彙對 embedding 有用，對詞面重疊有害（會獎勵所有談那個題目的片段，而不是答到問題那一條）。
  - **兩條路徑都補上了**：`searchWiki` 與 `searchWikiRoutedExact`。只改前者會量到一條生產不走的路徑（生產由 2026-09-09 起走 route-first）。
  - ⚠️ **這支 A/B harness 的延遲數字不可用**：before 側與 after 側是同一題一前一後跑，第二次享受快取，所以 after 側必然較快（實測 median 1,637 → 1,040ms）。**那不是重排令系統變快**，是呼叫次序造成的，與 Risks 3 警告的冷熱差異同族。只有正確性數字可用。
  - 🟢 **`REGISTRY_UNMANAGED` 已修好（Leonard 自己執行生產寫入）**：2 行 `source_id` 由 `stat_integrated` 改為 `stat_integrated_edu`。核實：`stat_integrated` **0** 條 · `stat_integrated_edu` **7** 條 · 全庫仍 **17,004**（守恆）· 兩行逐個確認 `source_id` 已改而 `content_type` 仍為 `vault_extract`。漂移 `UNMANAGED` **1 → 0**；封版閘 PASS 13 → **14**、ERROR 3 → **2**（只剩兩格 EVAL）。⚠️ **已知且刻意的化石**：chunk id 仍是 `vault_stat_integrated_…`（id 是內容雜湊，改它等於刪除重入連 embedding 重算）。另外那條由表格中段開始、無標頭的片段仍屬 `NO_MIDCLAUSE_START` 家族，要徹底處理須重抽該 XLSX。
  - **（原工具保留）** `dev/_s226_fix_unmanaged_source.py` 有 `--rollback --execute` 可完全回退 —— `dev/_s226_fix_unmanaged_source.py`（預設 dry-run、有 `--execute` 與 `--rollback`、以 id 精確鎖定 2 行、附守恆核對）。dry-run 通過。**生產寫入被 auto mode 分類器擋住（`Modify Shared Resources`），要 Leonard 自己跑。**
  - 🟢 **【S226 修好】`評核` 路由缺陷。** 症狀：`detectQueryCategory('教師評核')` 回 **curriculum**，而 `curriculum` 的 SOURCE_SET 沒有 SAG，所以 `hr_appraisal` 這題回傳的是物理、中文、資訊科技課程指引 —— 答案（SAG §7.7 員工考績 p.208）永遠取不到。**根因**：`curriculum` 的正則擁有裸 token 「評核」（供「課程及評估指引」一類詞彙用），而 `hr_admin` 只有「語文能力評核」，於是人事查詢掉到後面的 curriculum。與 `safety` 擁有裸「氣體」而搶走「氣體定律」同族。**修法**：在 `hr_admin`（排序在 curriculum 之前）補上 `教師評核|員工考績|考績|教師表現管理|表現管理`，**不動 curriculum 的裸「評核」**。**回歸**：`route_regression.mjs` 由 46/46 → **50/50**，新增四條，其中一條是**對照組**（「中國語文課程及評估指引」仍須路由去 curriculum，證明沒有搶錯方向）。**before→after 已跑（row 44 已滿足）**：`before` 用同日同旗標側的 `2026-09-15_s226_route_first_after.json`（同語料、同日，不必另跑）。`eval_retrieval.py --compare` 結果：**blocking failures 0**（零 `SET_LOST`、零 `VERDICT_REGRESSED`、零 `ERROR`）· **`VERDICT_FIXED` 1 條** —— `hr_appraisal` FAIL → PASS，窗口由物理／中文／資訊科技課程指引換成 `g04`＋`sag_2025_11`；另有 1 條 `SET_ADDED`（`cpd_t_standard` 多了 `eng_sss_guide_2021`，工具不視為失敗）。來源層 **122 → 123**、片段層 **58/108 不變**。⚠️ **誠實邊界**：`hr_appraisal` 的片段層仍是 FAIL —— 正確語料現在拿得到，但答案那一段（SAG §7.7, p.208）仍未入前八。**這個修正解決的是「連正確語料都到不了」，不是「答得到」。**
- **第七批（合成側 A/B —— 結論與檢索指標相反，出貨建議因此收回）**：
  - **設計**：只跑**證據集真的有變**那 **38 題**（權重 0.05）。其餘 147 題餵給合成器的東西逐條相同，答案只可能因 LLM 非決定性而異，跑了只會量到噪音。llm 與 judge client 依 `server.ts` 同一方式建立（預設模型合成、`getJudgeModel()` 判官），量的是部署配置。38/38、errors 0。
  - **可自動量到的部分**：答案文字有變 **30/38** · 兩側都棄權 8 · **由作答轉為棄權 0** · 由棄權轉為作答 **1**（`cpd_conduct_registration`）· 答案長度中位 265 → 271。
  - 🔴 **但兩個自動代理都不是有效的評分器，這一點本身是結論。** (a) 「答案是否含 gold 簽名」**0 得 0 失、38 題全部兩側皆無** —— 合成器會改寫且上限約 250 字，簽名幾乎不可能逐字存活，這個代理在此是空的；(b) 我自寫的「數字是否流失」篩查**誤報過**：`fin_ac_grant` 的 after 其實有 `$8,384`／`$53,158`，只是寫成 `$` 前綴無「元」字，我的正則漏了 —— 修好偵測器後由 3 題降為 2 題。**用壞尺量出來的結論，方向剛好對我有利（支持我剛剛才反轉的判斷），所以必須重量。**
  - ⚠️ **逐對讀過 7 對答案，實況是混合的**：**明確變差 1 條** —— `hr_maternity`（產假）after 開頭寫「資料中沒有明確載明產假的具體時長」，然後同一段又說「最長 14 星期有薪產假」，**自相矛盾**，而 before 乾淨地給了 14 星期＋40 星期服務條件＋2019-01-01 生效日。**可疑 1 條** —— `cpd_conduct_registration` 由誠實棄權變成作答，但答案講消防疏散與學生訓育，不是《教師專業操守指引》。**數字被攪混 1 條** —— `dig_teacher_digital_cpd_hours` 由「不少於 30 小時」變成「30 至 150 小時」再補一句「政策未載明具體課時數」。**看來相等或更好 3–4 條** —— `staff_grad_ratio`（多了職級劃分）、`fin_ac_grant`（多了 `$42,526` 與一個實算例子）、`bus_escort_list` 與 `hr_jobshare`（丟掉的是邊緣數字，答案反而更貼題）。
  - 🔴 **所以出貨建議收回：檢索指標報喜，答案層面未證實，而且有 2–3 條可指名的退化。** 要出貨必須先有**答案層面的評分器**（帶 rubric 的 LLM 判官對照 gold 段落，或人手覆核那 30 條）—— 即 S214 那套 grounded acceptance 的做法。**在此之前 `FEATURE_LEXICAL_RERANK` 不應在 Render 啟用**，即使 PR 合併亦然（預設關閉）。
  - **這一節的教訓值得記住**：`chunk_PASS +6` 是真的，但它量的是「答得到的段落有沒有入窗」，不是「合成器有沒有用對它」。兩者之間還有一層，而這一層本節第一次量到，量出來是混合結果。
- **未做（不要當已做）**：本節「量到但未修」的項目**一律不在此列舉**，全部由 `## Open Priorities` ①–⑤ 承載（那裡是唯一的家）。另有兩項不屬 OP 的：`regression:semantic`／`regression:grounded` 未跑；生產部署未做（Render 仍執行 `e9d14be`，而 `backend/src` 已有改動）。**已於本節完成、不要當未做**：`hr_appraisal` 標籤、兩個 registry ERROR、`closeout-status`（經 `npx` 可跑）、三個 PR（全部已合併）。

**S225（2026-09-14）** —— 逐項證據見 `dev/SESSION_LOG.md` S225 條。

- **機器閘**：`qc_report.py --self-test` ALL PASS（本節新增五條斷言）· `route_regression` **46/46** · `check_registry_drift --self-test` ALL PASS · PR #14 `build-gate` pass、`mergeable=CLEAN`。
- **紅測兩次，各自準確轉紅且不波及其他**：(a) 把判分打回 `.get("FAIL", 0)` 默認 → 「無 summary」與「summary 形狀不對」兩條轉紅，其餘一條都沒受影響；(b) 把選檔打回不過濾的 `sorted()[-1]` → 只有「live 選檔」那條轉紅。兩次紅測後皆以 `shasum -a 256` 核實檔案已還原至修補版。
- **封版閘前後對照**（重跑 `--check` 實測）：`EVAL_LATEST` **PASS → FAIL**（量度自 `2026-09-08_s220_route_first_before.json`，`FAIL 44/185`）· PASS **13 → 12** · ERROR **2 → 3** · releaseGate **5/15 → 4/15** · `PASS_EVAL_LATEST` **MET → NOT_MET** · **其餘二十項檢查零變動** · `overallStatus` 兩邊都是 `ERROR`。
- **起手探針 6/6 綠**：served `app.html` **3.3.8** · `/health` ok、commit `e9d14be` · `git diff --name-only e9d14be..origin/main -- backend` **為空** · Supabase **17,004**（service key `count=exact` 實讀）· `ZOMBIE` **0** · 本機落後 1 個 commit（`discovery-bot` ledger）已 fast-forward。
- **新證據（交接檔此前記為未發生）**：`discover_check` 與 `freshness_check` 已各自經歷**一次真實排程觸發**（`gh run list` 顯示 `event: schedule` 且 success，09-14 16:01／15:44 UTC），其中 discover 經 deploy key 推出 ledger commit `c48fcc5`，該 push 上的 `build-gate` 報綠。**看門狗與 pgvector 監察仍未經歷真實排程觸發。**
- **架構盤點覆核**：兩個讀碼 agent ＋ 一個獨立覆核 agent ＋ 一個流程監察 agent。覆核組十項數字化主張逐條重讀原碼全部 CONFIRMED，並揪出兩處主線遺漏（判官的 `trustedVaultLead` 常態繞過、輸入淨化整類未提），已補入 `dev/AUDIT.md`。
- **未做（不要當已做）**：185 題 gold **未重跑** · Recall@k 工具仍不可執行（語料快取指向已消失的 scratchpad）· `regression:semantic`／`regression:grounded` 本節**未跑** · 生產旗標與模型值仍無法由外部核實 · 七條前端未呼叫的端點是否死碼未查 · `agent-handoff-kit` CLI 未安裝，`closeout-status` 語義閘**未能執行**。

**S224（2026-09-14）** —— 逐項證據見 `dev/SESSION_LOG.md` S224 條。

- **機器閘**：`npm run check` 0 · `npm run build` 0 · `route_regression` **46/46 PASS** ＋成員斷言 · `execute_ingest --self-test` **28/28**（S223 為 13 條）· `qc_report --self-test` ALL PASS · `check_registry_drift --self-test`／`check_monitor_health --self-test` ALL PASS · 九個 workflow YAML 全部解析。
- **紅測三次，全部逐條對得上預期**：(a) 把 `git push origin HEAD:main` 注入回 `live_commit_push` → `step 6: does not push to main` 轉紅（27/28）；(b) 把凍結契約基準值回捲至 2.6.1/158 → `intact freeze contract passes` 轉紅；(c) 移走 `check_freeze_contract` 內的 guidelines 比較 → **新加那兩條** guidelines 紅案準確轉紅，其餘一條都冇受影響。
- **ruleset 實測擋得住**（唔係睇介面似乎生效）：推空 commit 落 `main` → `remote rejected`，兩條規則同時出聲 `Changes must be made through a pull request` ＋ `Required status check "build-gate" is expected`。API 逐項對證：目標 `~DEFAULT_BRANCH`、bypass 只有 `DeployKey(always)`、`build-gate` required、`strict=false`、PR 批准 0、`branches/main.protected=true`。
- **閘的兩半都在生產實測**：`3affb71`（改 backend／executor）→ `BUILD — 2 watched path(s)`，九步全跑全綠；`eb585a0`（純文件）→ `SKIP — none watched`，**5 秒**報 success。即每個 commit 都有確定結論，required check 不會卡 pending。
- **deploy key 實測推得到**：手動觸發 `qc_report` → `abe1fad..fe3b6ba  HEAD -> main`。
- **PR 流程連續兩次成功**：#10、#11 皆 `BLOCKED → CLEAN → MERGED`，分支自動清走。
- **凍結契約**：`--check` 後 `FREEZE_CONTRACT` FAIL → **PASS**，BLOCKER **1 → 0**，PASS 12 → 13，ERROR／WARN／INFO／NOT_MEASURED **一個都冇郁**（2／5／1／1）。
- **收工探針**：served `app.html` **3.3.8** · Supabase **17,004**（service key `count=exact` 實讀）· `ZOMBIE` **0** · `git diff --name-only e9d14be..HEAD -- backend` **為空**（線上執行碼 == HEAD）。
- **攔截了一個假綠**：`git diff main <branch>` 因 repo root 有個同名 0-byte 檔案而報 `ambiguous argument`，輸出那個 `0` 其實來自指令失敗而非比對結果。已用 `--` 重做，並改用「分支 tip vs 它自己的 squash commit」比對才刪分支。
- **未做（不要當已做）**：185 題 gold **未重跑**（Risks 3 的 p90 仍是舊數）· Recall@k 未量 · `regression:semantic`／`regression:grounded` 未跑 · SAG 換版的 eval before→after 仍然取不到 · 封版閘 **仍然不是綠**（見 Open Priorities ③）· 看門狗與 pgvector 監察仍未經歷真實排程觸發 · **入庫 PR 路徑未經一次真實入庫**（今日 approval queue 空，`build-gate` 只在文件與 backend commit 上驗過，未在 Option A 自己開的 PR 上驗過）。

**S223（2026-09-14）** —— 逐項證據見 `dev/SESSION_LOG.md` S223 條。

- **機器閘**：`npm run check` 0 · `npm run build` 0 · `route_regression` **46/46 PASS** ＋ `chi_hist_jss_ncs_2019` 成員斷言 PASS · 五個 JSON 全部解析 · `execute_ingest --self-test` **13/13** · `check_registry_drift --self-test` ALL PASS · `check_monitor_health --self-test` ALL PASS · 九個 workflow YAML 全部解析。
- **紅測（證明斷言會轉紅）**：把舊 locator 掛回去再跑 `execute_ingest --self-test` → **6/13 FAILED**，其中包括「live file: all 23 routes end on a real bracket」這條會即時偵測真實檔案的斷言。
- **生產寫入逐項對數**（兩次，皆由本節 AI 直接執行）：`sag_2025_11` 409 → 0 → **387**；`g24` 383 → **0**；`kgecg_2017` 108 → **0**；`g29` **107 未受影響**；全庫 17,517 → 17,112 → **17,004**（兩次差額分別為 −405 與 −108，與計劃完全相符）。顯示數字一律由 `live_total_count()` 讀真數再同步，未用加減推算。
- **生產端到端實測**：`/api/search/channel-b` 五條查詢全部 `ok` —— 命中 2026年8月版**新增**的「8.4.4 涉及危害國家安全的行為和活動」（p.253, score 0.654）、封面「2026 年8 月版」(p.3)、僱傭合約條文 (p.198/199)；`g24` 與 `kgecg_2017` 皆**不再出現在任何結果**；`g29` 正常承接幼稚園指引（p.44 score 0.717）。
- **瀏覽器實測**（三輪，每次 push 後都做）：載入線上 `app.html` —— 最終 `PLATFORM_VERSION` **3.3.8**、`GUIDELINES_REGISTRY.length` **170**、零重複 id、`g24`／`kgecg_2017` 皆不在瀏覽清單、片段數顯示 **17,004**、**零過期數字殘留**、**零 console error**。
- **量度方法設對照組**：`kgecg_2017` vs `g29` 的覆蓋率用同一抽取器 ＋ 自己對自己 0 miss 的對照組，再把落差視窗拆半分辨「次序」與「真缺失」。證據 `dev/source/eval_runs/2026-09-14_s223_kgecg_g29_overlap.json`。
- **未做（不要當已做）**：185 題 gold **未重跑**（Risks 3 的 p90 仍是舊計劃下的數字）· Recall@k 未量 · `regression:semantic`／`regression:grounded` 本節**未跑**（本節零檢索邏輯改動，但這不等於已驗證）· 合成側 grounded harness 狀態同 S220 · `qc_report.json` overall ERROR 未處理（S215 起）· **SAG 換版後未做 eval before→after**（S203 PLAN 有這一步，本節跳過了：舊語料已被覆寫，before 基線取不到）· 看門狗與 pgvector 監察仍未經歷真實排程觸發。
...
- **正確性閘（本節新工具 `backend/scripts/_s221_routedBaseline.ts`）**：DDL 前後各跑一次、**重用同一批查詢向量**，7 條題逐條比對 → **6/6 原本成功的 id 與 score 完全相同**，第 7 條（cpd「專業階梯」）由 **HTTP 500／0 列 → 200／40 列**，0 problems。
- **機器閘**：`npm run check` 0 · `route_regression` **46/46** · `regression:grounded` **48/48**。
- **生產實測**：`/api/search/channel-b` 三條 cpd 題各 3 次 = **9/9 全部 `ok`／8 結果**，1.48–2.42 秒（**首次 24.6 秒是 Render 冷啟動，不是查詢延遲**）。
- **未做（不要當已做）**：同批查詢的生產前後對照 —— 前值取自 S220 記錄（2.81／6.10／6.41 秒），非同一批題、非同一時段，故「快了多少」只有指示性。185 題 gold 未重跑，故 Risks 3 的 p90 仍是舊計劃下的數字。Recall@k 與合成側 grounded harness 狀態同 S220；`qc_report.json` overall ERROR 未處理（S215 起）。
...
8. 🟢 **【S225 發現並修好】封版閘的 `EVAL_LATEST` 假綠已收掉，但收掉之後那一格是紅的。** 舊行為：`latest_eval_run()`（`qc_report.py`）是 `sorted(glob("*.json"))[-1]`，不過濾揀中的是不是一次檢索評測；`check_eval_latest()` 再用 `s.get("FAIL", 0)` / `s.get("errors", 0)`，欄位不存在即當 0，於是「零失敗」恆真 → PASS。09-14 當日揀中 `2026-09-14_s223_kgecg_g29_overlap.json`（無 `summary` 欄），detail 逐字是 `None · PASS=None FAIL=0 … 共 None 條 query`。**修法**：新增 `is_gold_eval()`（要求 summary 具備 `PASS`／`FAIL`／`queries`），選檔改為由新到舊揀第一份合資格者，判分缺數據一律 `NOT_MEASURED`——即對齊同函式下半截 `EVAL_CHUNK_LAYER` 早已寫對的做法；detail 加印「量度自 <檔名>」令陳舊看得見。**實測**：`--self-test` ALL PASS（新增五條斷言），兩次針對性注入各自準確轉紅且不波及其他；重跑 `--check` 後 PASS 13 → 12、ERROR 2 → 3、releaseGate 5/15 → **4/15**，`PASS_EVAL_LATEST` 由 `MET` 轉 `NOT_MET`，其餘二十項檢查**零變動**。**餘下**：該格現時報 `FAIL`，量度自 2026-09-08 的 gold run（`FAIL 44/185`，且是旗標關閉的 before 基線）——**要令它轉綠只能靠改善檢索（現為 Open Priorities ②），不是靠再改 `qc_report.py`。**⚠️ **S228 更正**：原文寫「靠 ① 重跑 185 題」，但 ① 已於 S227 完成並移除，而且 S226 早已重跑過 gold —— 重跑本身不會令它轉綠，要轉綠必須真的把 FAIL 數壓下去。根因備註：`DOC_SYNC_CHECKLIST.md` 第 56 行早已寫明「新檢查不得預設報 0——量不到要報 `NOT_MEASURED`」，即這是既有規則沒被遵守，不是規則缺失。完整分析見 `dev/AUDIT.md` 乙①。

9. 🔴 **【S226 發現】封版閘那一格在報生產不執行的配置 —— 選檔永遠揀旗標關閉側。** `latest_eval_run()`（`qc_report.py:706`）是 `sorted(EVAL_RUNS.glob("*.json"), reverse=True)`，而同一日的 `..._before.json` 與 `..._after.json` 反向排序時 `b` 在 `a` 之後，**所以永遠揀 `before`**。現時 detail 逐字為「S219 route-first before (in-process, FEATURE_ROUTE_FIRST_SEARCH=0) · PASS=119 FAIL=47 … 量度自 `2026-09-15_s226_route_first_before.json`」，而 S226 已用行為探針核實生產行的是**開啟**側（184/184 逐條同判，見 Current Baseline 5）。**🟢 已修（Leonard 同節拍板選項 A）**：新增 `measures_production()`（`endpoint` 以 http(s) 開頭且非 localhost）與 `eval_source_kind()`；`latest_eval_run()` 在合格 gold run 中優先揀生產那一份，並在 detail 明寫「生產端點」或「非生產端點」，另列出未被採用的較新 run。**閘現時報 PASS 121／FAIL 44／errors 1（生產）**，不再報實驗室的旗標關閉側。7 條新斷言全綠，紅測（選檔打回 `runs[0]`）準確轉紅。**當初沒有逕自改的理由仍然成立並已記錄**：`/health` 不報 flag，「哪一份 run 代表生產」是個假設；選項 A 之所以可接受，是因為它不假設旗標值，只假設「打生產端點的 run 反映生產」，而這個假設由 S226 的行為探針（184/184）獨立支持。**同一節另一項已修**：`EVAL_CHUNK_LAYER` 長期 `NOT_MEASURED` 的原因不是沒重跑 gold，而是**兩支 185 題 harness 都沒有把片段層數字寫入 `summary`**（判分在 `check_eval_latest()` 的 `if "chunk_FAIL" in s`），每題的 `chunk_verdict` 其實一直算了出來、只是從未匯總；S226 把 `summarize()` 單一來源化後該格已有數（現報 FAIL）。**所以交接檔原本寫「做完 ① 就自然有」是錯的**，那是工具缺口，不是量度債。
...
- **Git state（S232 收工實測，2026-09-24）**: root 與 git root 皆為 `/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft` · 收工分支 **`s232/handoff`**（由 `main` `ef53da8` 開出，即 PR #35，待 Leonard 合併）· worktrees **1**（無平行工作區）。`origin/main` = **`ef53da8`**；線上執行碼 **`f712172`**（`backend/` 零落後）。S232 已刪本機 `s231/closeout`、`s232/alarm-mentions`、`s232/kg-ratio-footnote`（三者皆已 squash 合併）。仍殘留更早的已合併 squash 分支：`s228/expansion-knobs`、`s230/{judge-takeover,prereqs,vault-gate-signal}`、`s231/{entry-cleanup,judge-baseline,judge-comment,local-cors}` —— 可刪，刪前照下文「tip 對 squash commit 樹比對」核實。
...
**Recommended next step（S232 收工重生）：** **重跑 185 題 gold（生產端點，`_s213_run_gold.py`，零 LLM 呼叫）**，核實 S232 footnote 的兩條預期變化（`kg_sccc_ratio_confusable` 禁引 +1、`kg_teacher_pupil_ratio` 來源層 FAIL→PASS），並把 `EVAL_LATEST` 更新到 17,007 片段的語料。理由：S232 往生產加了一條會領先多條查詢的 footnote，現有基線（S226）已不描述生產；先量清楚才開路線乙，否則乙的量度會混入 footnote 的效應。**之後的主線是路線乙**（S202 reframe 的非提示式對象核對機制，先離線設計、用 D01／FT06 量，零生產改動）。⚠️ 換判官模型與改提示都已證修不了這一類（S202、S211、S230），不要再走。

**次序（不可倒轉）**：~~補基線~~（S231 完成）→ ~~GN10 個案修（甲）~~（S232 完成）→ **重跑 gold → 結構修（乙）** → 才談啟用 `FEATURE_VAULT_GATE_RAWVEC`。原因見 `## Validation / QC` S230 第六至八批：候選會令判官對多 52 條查詢變成承重件。**治理側第一件維護工作（S230 留下、S231／S232 未做）**：log 維護兩套觸發標準（`AGENTS.md` §4a 的 >400 行／>30 日 對 Kit 核心的 ≥11 條）要整合為單一定義，見 `## State Reconciliation Check` S230「Pack 衝突」。

② 🔴 **把片段層召回率拉高（缺口本體；基線與本節新數字見 `## Current Baseline` 7 與 `## Validation / QC` S228）。**

**S228 之後，已排除的方向增至七條**，前四條照舊（重新切片、調配額、詞面重排、改展開機制本身），新增三條：**(5) `EXPANSION_MIN_QUERY_CHARS`（floor）不出貨** —— 檢索兩層都改善（`floor=8` 片段層 +5、零退步），但它把展開詞整段拿走，**最高分中位跌 0.1296**，令 15–28 條題目跌穿 `VAULT_LEAD_SCORE` 0.70、17–27 條跌穿 `SPOTLIGHT_LEAD_SCORE` 0.60，答案層棄權由 8 升到 15（其中 10 條可答），判官 `net_after` +3.5／門檻 8 → `NO_MEASURABLE_GAIN`。**(6) 合併配置（floor ＋ balance）同樣不出貨**，理由同上（`net_after` +3.5）。**(7) `EXPANSION_BALANCE` 單獨啟用亦不出貨** —— 它幾乎不壓分（幅度只有 `floor` 的三十分之一、跌穿 0.60 者 0 條），檢索層 +4／+3、零退步，但答案層 `net_after` 只有 **+1.5**，棄權仍由 8 升到 11。⚠️ **這一條同時削弱了「全部怪門檻」這個讀法**：連幾乎不動分數尺的版本都只有 +1.5。所以本節的證據支持兩個並存的讀法，不要只取其一 —— **(i)** 下游兩個絕對分數閘確實吃掉了一部分得益（棄權上升、0.70 跌穿數可量）；**(ii)** 這個量級的檢索改動（片段層 +4 至 +7）本來就很難在 79–81 條的pairwise 判官上堆出 8 分淨值 —— 79 條之中判官判 **43 條 TIE**，即大部分題目的答案在它眼中根本無分別。**下一節要先決定信哪一個，因為兩者的下一步不同**：(i) 指向重校門檻，(ii) 指向「門檻 8 對這個量級的改動是否合適」，而後者屬 `dev/DOC_SYNC_CHECKLIST.md`「答案層評分改動」row ——**改門檻必須在跑之前決定並記低改前改後與理由，不可看完數字才改**。

**展開側兩個候選 S228 已全部量完並關閉；未試方向現時是 (b) 與 (c)，兩條都已有量過的成本或明確的先決條件：**

  **(a) ~~`EXPANSION_BALANCE` 單獨啟用~~ —— 🔴 S228 已量完並判定不出貨，此路已封。** 檢索層 +4 片段／+3 來源、零退步，壓分幅度只有 `floor` 的三十分之一（真正被重新配重的 57 條：平均 −0.0042、中位 −0.0021、最差 −0.0426），跌穿 0.60 的題目 0 條、跌穿 0.70 只 6 條 —— 但答案層判官 79 條、145 次呼叫，**`net_after` +1.5／門檻 8 → `NO_MEASURABLE_GAIN`**，棄權由 8 升到 11（3 條可答的失去答案）。**不要在未做 (c) 之前重跑這一個**。

  **(b) cross-encoder 重排 —— 上限已量到。** 166 條有簽名的題目中，**重排最多救得到 42 條**（片段層 59 → 101 為絕對上限），而 **65 條（39%）連寬池 top-40 都沒有，任何重排都救不到**。寬池就是現有的 over-fetch（`fetchCount = topK*5`），所以重排可以插進現有流程而不必再檢索一次；每條查詢要評 40 個候選，現時 routed p90 是 2,498ms，延遲預算要先算。⚠️ **具體模型／供應商未選，且按 AGENTS §0b 不得憑記憶寫 API** —— 動手前要先查官方文件並把端點、參數、回應路徑寫入 `dev/CODEBASE_CONTEXT.md` External Services。

  **(c) 【S228 新開】把下游的絕對分數門檻重新校，然後把 (a)／(b) 再量一次 —— 這是本節的發現直接指出來的方向。** 檢索層的得益是真的（合併配置片段層 +7、零退步、噪音底線 0），被吃掉的地方是 `VAULT_LEAD_SCORE = 0.70`（`searchChannelB.ts:1422`，合成 bypass）與 `SPOTLIGHT_LEAD_SCORE = 0.6`（`:1473`，overlay 放行）—— **兩個都是常數，都是在「查詢帶展開詞」的分數尺上校出來的**。兩條可走的路：①**重新校常數**（要先量出新尺上的等效分位，不可拍腦袋改數）；②**改為相對門檻**（例如取窗內分數的分位數而非絕對值），那樣任何動搖分數尺的改動都不會再撞這道閘 —— 但這是改合成閘的行為，屬另一個 row，要自己的 before→after。⚠️ **不要在未量門檻之前再跑一次展開側的 A/B**：結果會跟 S228 一樣，兩層檢索報喜、答案層被閘吃掉。

  🔴 **【S229 實測，2026-09-20】(c) 的方向要改：0.70 不是「被展開詞抬高了要調返低」，而是由頭到尾校錯了尺，而且現況是偏低不是偏高 —— 所以不要重校這個數，要換訊號。** 定 0.70 那支儀器是 `dev/source/judge_probe.py`，它 **embed 裸查詢兼打全庫**（`match_threshold 0.1, match_count 40`，無 `SOURCE_SET`）；而 0.70 套用在 `mainSearchLead.score`（`searchChannelB.ts:1242`），那個分數來自 **`expandQuery` 展開後 ＋ 路由收窄** 的搜尋。185 條 gold 之中 **97 條是路由題**，兩把尺不同；其餘 88 條無路由，兩尺重合。新探針 `backend/scripts/_s229_scaleOffset.ts`（生產配置 `FEATURE_ROUTE_FIRST_SEARCH=1`，用生產函式，零重寫）把同一批 24 條校準 query 分三把尺量：**A 裸＋全庫**（＝校準尺，重現 S195 原數：CLASS_B 上限 0.6321 對 0.632、CLASS_C 下限 0.6241 對 0.624，儀器自我驗證通過）、**B 裸＋路由**、**C 展開＋路由**（＝應用尺）。結果：路由本身**不抬分**（A→B 為 0 至 −0.067），抬分的是展開（B→C 最多 **+0.3226**）。**重疊由 0.0080 擴大到 0.0817，大十倍**；而 CLASS_B 敵意題在應用尺上有 **2 條 ≥ 0.70**，即合成閘被跳過。逐條開過段落核實（`_s229_leadDetail.ts`）：**「校巴司機最低工資係幾多」@ 0.7098 是真缺口** —— 窗內四個來源 `sch_bus_drivers_2026`／`sch_bus_parents_2026`／`sch_bus_operators_2026`／`g18` 共 31 片段，`工資` 與 `薪` 命中 **0**，全庫 `最低工資` 只有 10 條且全部是經濟／通識科課程文件，**判官被跳過而合成器手上零工資內容**，正是 0.70 當初要擋的 S177 那一類。**另一條「老師病假連續請幾耐先要交醫生紙」@ 0.7003 是探針標錯，不是缺陷** —— `g04` 原文有「常額教師如申請病假超逾兩天，必須出示有效的醫生證明書」，領先那一片就是答案片段，bypass 觸發是正確行為；`judge_probe.py` 把它列入 CLASS_B（語料無答案）是錯的。**兩個後果**：① `judge_probe.py` 本身要修（它不行生產路徑，違反共用經驗庫 `self-authored-probe-false-confidence` 方法 B 第 3 步），`dev/DOC_SYNC_CHECKLIST.md` 那個「跑 judge_probe」的 row 照現狀跑只會繼續報校準尺的數；② 兩個分佈在應用尺上重疊得更厲害，**同一張卡尾段寫明「兩分佈重疊時調 threshold 係死路，要換訊號或換判斷器」** —— 所以 (c) 的路①（重校常數）證據上不成立。**建議走第三條路（S229 新提，未實作、未量）**：`rawVec` 已經為每條查詢 embed 好（零額外成本），把 vault 閘改為測「領先片段對裸查詢向量的 cosine」，即把閘放回它被校準的那把尺上。**這是改合成閘行為，屬 `DOC_SYNC_CHECKLIST.md` 自己一個 row，要先定案再量 before→after，未得 Leonard 批准不得改。**

  🔴 **【S230 實測，2026-09-21】(c) 的兩條路都走過了，兩條都不通，而且不通的方式不同 —— 不要再當它是「未試」。** 路①（重校常數）已由 S229 否決。路②的具體做法（S229 提的「換訊號」：把 vault 閘改為測領先片段對裸查詢向量的餘弦）**本節已實作為 `FEATURE_VAULT_GATE_RAWVEC`（預設關閉）並量完**：bypass **5 → 0**，只有真缺口「校巴司機最低工資」一條是想要的結果，另外四條是三個正控加上預先定案明列必須保住的「老師病假」。**原因是結構性的、不是實作問題**：0.70 比整個校準集在校準尺上的最高分（0.6457）還高 0.07，S195 當初刻意這樣定（「no choice of number is safe… needs a better JUDGE, not a lower bar」），**所以換回校準尺等於移除 bypass**。而判官接手不了：`人工智能初探` 與 `資訊及通訊科技 課程指引` 兩條 `want: 能` 的裸名詞題，在三份記錄的凍結 run 全部判「否」。**旗標留在碼內預設關閉，作為這次量度的可重現路徑；不要在 Render 啟用它。** 逐批證據見 `## Validation / QC` S230 四批，artifacts 見 `dev/source/eval_runs/2026-09-21_s230_*`。

  🔴 **【S230 續跑，2026-09-22】185 題 gold 的 bypass 人口普查 —— 這道閘的作用面是 59/185（31.9%），不是幾條；(乙) 因此死亡，(甲) 被指出有短查詢偏差，並新開一條 (丙)。** 完整八點見 `## Validation / QC` S230 第五批。要記住的三件事：**①** 真正的傷害是 **5 條 `FALSE_BYPASS`**（語料本來答不到卻跳過判官），**五條的共同形狀是「問一個語料裡不存在的數額或版本」** —— `幼稚園 表現指標`／`藝術教育學習領域課程指引 2002 版`／`資優教育學苑 收生 智商分數要求`／`校車服務 每月收費標準`／`校董有冇人工`。**②** rawvec 閘攔到 5/5，但同時攔錯 7/10 條 `SOUND`，而那 7 條有六條是二三個 token 的短查詢（裸向量對短查詢結構上低分）——**所以這個訊號的偏差方向正好打在真實用戶的主要輸入形態上**。**③** 詞面零重疊規則只攔到 1/59、對五條 `FALSE_BYPASS` 全部失效（它們的重疊全部非零），24 條校準集看似可行只是巧合。**(丙) 新提，未實作未量**：五條的形狀是「問數額而語料無此數額」，所以要測的不是相似度也不是泛詞重疊，而是**「窗內有沒有被問的那一類數量」** —— 但注意 S229 對 `校巴司機最低工資` 做過的正是這個手工版本（`工資`／`薪` 命中 0），要先證明它在這五條上都成立才值得寫碼。⚠️ **不要用產生這個假設的同一批數據驗收它**（row 41 紀律）。

  🟢 **【S230 判官接手測試，2026-09-22 —— 本節最終結論，推翻上面兩段的「不出貨」】** 上面兩段（換訊號失敗、普查否決兩個規則）的共同盲點是：**它們都用 stub 判官量度，所以量到的是「失去 bypass」而不是「失去答案」**。用生產判官模型實測 53 次之後：被攔的七條 `SOUND` **7/7 判官照樣答**（其中六條是二三個 token 的短查詢，即第五批推論的短查詢誤傷**在答案層並未實現**），而五條 `FALSE_BYPASS` 有 **4 條正確轉為拒答**。**按預先定案，候選達標。**代價兩項、殘留缺口一項、未量成本一項（延遲），出貨先決條件四項 —— 全部逐條列在 `## Validation / QC` S230 第六批。**下一節不要再找新訊號；要做的是補完那四項先決條件。**

**那 65 條救不到的題目是另一件事**：它們要動的是候選生成（索引／路由／切法），不是重排。這是 ⑤ 與 ④ 的地盤。

**出貨閘不變**：用 `backend/scripts/_s227_rubricJudge.ts` 量答案層再決定。⚠️ 用它之前要知的三件事：(a) 判官模型必須 `JUDGE_MODEL=gpt-4.1`；(b) 依賴準則 3（數字）的判決自動標 `needs_human_review`（S228 有 22/81 條落入此類，比例不低，報結論時要連這個數一齊講）；(c) 跨次跑有變異（同一校準集 5/7 與 4/7），單題結論不可當定論。

🔴 **S228 新增的一條硬紀律（所有檢索改動都適用）** —— ⚠️ **S229 實測更正了它的適用範圍，照舊文照做會白做功夫**：原文寫「`VAULT_LEAD_SCORE` 0.70、`SPOTLIGHT_LEAD_SCORE` 0.60、`min_score` 全部是在『查詢帶展開詞』的分數尺上校出來的」。**只有 `VAULT_LEAD_SCORE` 與 `min_score` 是**；`SPOTLIGHT_LEAD_SCORE` 與兩個 `FOOTNOTE_*` 門檻**看的是裸查詢向量**（`searchChannelB.ts:1756` 的 `rawVec`，由 `:1770` 與 `:1821` 傳入兩個 overlay），所以**任何展開側改動都動不到它們**。證據：S228 十個 arm、共 1,667 筆 before／after 記錄，裸查詢字串在兩側逐筆同一個（`before_embedded`／`after_embedded`），0 筆例外。**修正後的紀律**：只有吃「展開後＋路由收窄」向量的閘（`VAULT_LEAD_SCORE`、`min_score`）才需要連同重校；把 spotlight／footnote 一齊算進去會做出一個不存在的因果。原文後半仍然成立 —— 動搖絕對分數而不重校，檢索兩層報喜而答案層變差，兩層檢索指標都不會顯示。

⑩ 🟢 **【S227 提出，S228 修好並重新定性】`hr_admin` 的「津貼」token 搶走幼稚園搬遷津貼查詢 —— 路由部分已修，但交接檔原本記載的病因是錯的。**

原記載說這是展開詞問題（裸查詢 0.5919 高過第 8 名 0.5879，加展開後跌到 0.4994）。S228 把展開完全關掉重量，**答案段落仍然入不到窗** —— 真正的閘是 `edbcm144_2026` 不在任何該查詢到達得到的 `SOURCE_SET`，只靠 spotlight overlay 露面，而 `SPOTLIGHT_MAX_LEADS = 1`，那唯一一格被 p.2 片段佔了。**已修**：`kg_admin` 由尾二提前到 `kg_admission` 之後、補「幼稚園＋津貼／搬遷」兩個 token、`edbcm144_2026` 入 `SOURCE_SETS.kg_admin`。blast radius 185 條只有 2 條改路由（皆為目標）、`route_regression` 63/63、第三條走 `kg_admin` 的 gold 零變動、off-gold 的 `幼稚園財務管理` 實窗核實已轉為幼稚園財務語料而 `學校財務管理` 不受影響。
...
⑨ 🟢 **【S227 提出，S228 已修】`backend/scripts/*.ts` 從來不在 `npm run check` 覆蓋範圍內。** 成因：`backend/tsconfig.json` 的 `include` 只有 `src/**/*.ts`，所以 `npx tsc -p` 對 `scripts/` 完全不生效，S227 第一次跑 typecheck 報「零錯」其實是空過。**修法採選項 (a)**：新增 `backend/tsconfig.scripts.json`（`extends` 主檔、`noEmit: true`、`rootDir: "."`，避開 `rootDir: src` 會爆的 outDir 錯），`npm run check` 改為串跑兩個 project，另留 `npm run check:scripts` 單獨用。**順帶修好四個一直潛伏的型別錯**：`groundedSynthesisProbe.ts` 的 filter callback 把元素型別窄成單一欄位（連帶令整段無法定型）、`groundedSynthesisRegression.ts` 在 `EvidenceChunk` 上讀它沒有宣告的 `id`／`content_type`（那兩個欄位執行期確實存在，因為判的是「揀中邊條」而非合成本身，故以測試自己的形狀讀取，不為遷就測試而放寬生產介面）。**紅測**：在 `scripts/` 注入一個型別錯，閘準確轉紅 1 條；還原後 exit 0。`regression:grounded` 48/48 PASS。

⑧ **【S226 新增】`closeout-status` 的 lifecycle 配對規則與本專案的交接密度衝突。** 實況：本節連跑 8 次，前三次各捉到一件真事（已完成項留在未了清單、三項過時的「未做」、§4 合規備註放錯段），**其後每次都指向另一對「依合約必須同時出現」的段落** —— 「量到但未修」的事實必須同時在 `Validation / QC`（證據）與 `Open Priorities`（下一步），最後一次甚至指向 S219 起的 cold zone（Risks 2）。**選項**：(a) 與上游確認該配對規則是否應排除 cold zone 與證據段；(b) 在本專案定義「必然重疊」白名單；(c) 接受它在本專案長期回報 blocked，並以 `Closeout outcome` 逐次記錄邊界（本節採此法）。**不應採用的做法**：靠改寫文字去通過它 —— 那會把交接檔改壞。

⑥ **【S216 提出，S219 修好其中一半】交接檔有 8 個 `ack` marker 未有專屬章節。** `durable-anchors`／`closeout-reconciled-state`／`task-understanding-summary`／`active-objective`／`sync-status` 五個 section marker，加 `user-intent`／`task-essence`／`success-criteria` 三個 field marker，現時集中放在檔頂並附註釋。**不影響 `doctor` 與 `closeout-status`**（前者只檢查 marker 存在，後者只讀四節加開場白），但這些章節對下個 agent 而言機械上讀不到 —— **這半邊仍未做**。（另一半的邊界標記範圍問題已於 S219 修好，不再是待辦；詳見該節 Last Session Record。）
...
1. UTC date: 2026-09-24
2. Session ID: S232。由頂層 dormant root「開工」redirect 入 Draft，起手探針全綠、零漂移。Leonard 逐步指示：GitHub 電郵太多 → 只留「待批准入庫候選」與最重要的電郵 → 警報 @mention（「OK」）→ GN10 走路線甲 → 寫入生產與兩次合成（「全做」）→ 收工。
3. Completed（撮要，逐項證據見 `## Validation / QC` S232 與 `dev/SESSION_LOG.md` S232 條）：
   - ✅ **GitHub 電郵收窄**：查明主要來源是 repository 層 push 電郵；Leonard 已改帳戶通知設定並清空四個 repo 的 push 電郵。
   - ✅ **警報 @mention（#33）**：看門狗與 build gate 開 issue 時會 @Leonard。
   - ✅ **GN10 路線甲上線（#34）**：攔下 S231 寫錯的 footnote 前提；以庫內原文寫成新 footnote 並寫入生產（17,006 → 17,007）；生產兩次合成都答 1:11；`footnote_lead_probe` 正控零損失。
   - ✅ **S231 收工 PR #32 已合併**（Leonard 親自按網頁）。
4. **本節 Supabase 寫入 1 行**（Leonard 批准，INSPECT before/after）；零 DDL、零 flag 改動、零 `backend/` 改動、零凍結合約改動；平台版本不變（v3.3.9）。#32、#33、#34 皆由 Leonard 在 GitHub 網頁合併。
...
1. UTC date: 2026-09-20
2. Session ID: S229（時分不可考 —— 該節從未收工）
3. 實測推翻 S228 交下來的 OP②(c)「重校 `VAULT_LEAD_SCORE`」：定 0.70 那支 `judge_probe.py` embed 裸查詢兼打全庫，而 0.70 套用在「展開後＋路由收窄」的分數上，兩把尺不同（185 條 gold 中 97 條是路由題）。建 `_s229_scaleOffset.ts` 與 `_s229_leadDetail.ts` 兩支探針。
4. ⚠️ **該節從未收工**：無 log 條目、無 `State Reconciliation Check` 條目、`Last Session Record` 停在 S228、開場白與鏡像檔亦停在 S228、四項改動未 commit，且**沒有存過任何 run 的原始輸出**。全部已由 S230 補齊並獨立覆核，詳見 `dev/SESSION_LOG.md` S229 補寫條目。

## Previous Session Record (S228)
...
3. Completed（撮要，逐項證據見 `dev/SESSION_LOG.md` S226 條與 `## Validation / QC` S226 各批）：
   - ✅ **185 題 gold 已重跑**（本機兩側 in-process、生產端點、三個 knob A/B 共六次全量跑），**片段層首次有數** —— 數值一律見 `## Current Baseline` 7，此處不複述。
   - ✅ **SAG 換版結案**：生產配置側只有 1 條退步（`hr_mpf`），且其舊 PASS 本來靠已退役的 `g24` 撐住、內容仍在庫 —— **不是丟內容，是短查詢排序**。
   - ✅ **生產旗標可由行為核實**：185 題生產 run 與本機旗標開啟側 **184/184 逐條同判**，`FEATURE_ROUTE_FIRST_SEARCH=1` 不再只靠 dashboard 記憶。
   - ✅ **答案鑰匙首次 185/185 零隔離**：重錨 1 條、改寫 1 條（`hr_appraisal` 原本量錯東西），機械根因（語料快取預設路徑指向已消失的 scratchpad）已修。
   - ✅ **封版閘 ERROR 4 → 2、PASS 12 → 14**：`EVAL_CHUNK_LAYER` 由假 `NOT_MEASURED` 變有數 · 選檔改為優先生產端點（Leonard 拍板選項 A）· `REGISTRY_SERIES` 假 ERROR 修好（閘漏傳第七個參數）· `REGISTRY_UNMANAGED` 由 Leonard 執行生產寫入修好。
   - ✅ **片段層機制診斷**：**不是切片問題**（`NO_CHUNK_HAS_IT` = 0）；61/63 是同源內部排序；16 條是 per-source 上限把答案本身趕走。
   - ✅ **三個檢索 knob 實測**：配額放寬＝一換一 · 族群配額不成立 · **詞面重排兩層同時改善**（三點劑量曲線，片段層在權重 0.05 停滯）。
   - ✅ **修好 `評核` 路由缺陷**：`hr_admin` 補詞、不動 curriculum 裸 token；回歸 46/46 → **50/50** 含對照組；before→after **blocking failures 0**。
4. **本節三次推翻自己的完整經過（含兩次量錯）記在 `dev/SESSION_LOG.md` S226 條四段 Fix Record**，此處不複述以免與 `## Open Priorities` 重疊。
5. **機器閘捉到收工初稿兩次 lifecycle 配對錯誤**（已完成項留在未了清單、建議下一步複述已完成內容），依其 Blocker 行逐次修正。**另一項事實更正**：交接檔長期記載「`agent-handoff-kit` CLI 本機未安裝」已過時 —— 本節經 `npx --yes @adamchanadam/agent-handoff-kit@latest` 實際跑得到。
...
1. UTC date: 2026-09-14
2. Session ID: `Claude_20260914_2030` — S225。由「開工」起手，起手探針 6/6 綠、零漂移；其後 Leonard 叫 `/ai-agent-patterns`，做完整 AI agent 架構盤點，再修好盤點揪出的一項封版閘假綠。**零 Supabase 寫入、零 `backend/src` 改動、零 flag 改動。**
3. Completed:
   - ✅ **完成 AI agent 架構盤點，產出 `dev/AUDIT.md`（239 行）。** 判定：全系統停在層 0 與層 1，**零工具呼叫**（全後端 `tools`／`tool_choice` grep 零命中），模型在任何一處都沒有揀工具或決定下一步的權力。**「可簡化」那一類落差是空的** —— 反模式十二條在架構層面一條都不中。問題全部在護欄與量度。
   - ✅ **修好 `EVAL_LATEST` 假綠並已合併（PR #14 → `40ae3cf`）。** 詳見 `## Risks / Blockers` 第 8 項。修法是把判分的上半截對齊同函式下半截早已寫對的做法；新增五條斷言並以兩次針對性注入證明會轉紅。
   - ✅ **把盤點接入治理**：`dev/AUDIT.md` 登記入 `PROJECT_INDEX.md` Directory Map 並註明它是凍結於 2026-09-14 的一次性評估、非 current state；`DOC_SYNC_CHECKLIST.md` 新增一條觸發（`backend/src` 的 LLM 呼叫／旗標／判官閘／護欄一改動即須覆核 AUDIT.md）；`PROJECT_INDEX.md` Local QC Commands 三行 Last verified 更新。
4. **本節修的是機制，不只是症狀** —— 但方向與 S224 相反：S224 發現的是「沒有規則指向那四個值」，本節發現的是**規則早就存在而沒被遵守**（`DOC_SYNC_CHECKLIST.md` 第 56 行白紙黑字寫著「新檢查不得預設報 0——量不到要報 `NOT_MEASURED`」）。所以本節**沒有新增任何治理規則**，只在三處記下這件事。
5. **本節有兩項發現是主線自己漏了、由獨立覆核 agent 揪出**：(a) 判官除了 fail-open，還有一條 `trustedVaultLead` 常態繞過（分數 ≥ 0.70 的 vault lead 連判官都不會叫）；(b) 整份報告原本一句都沒提輸入淨化。兩項都已補入 `dev/AUDIT.md`，並在該檔附錄逐項列明哪些判斷由主線親自核過、哪些只有子代理查過。
6. **本節攔截了自己一次疏忽**：更新 `PROJECT_INDEX.md` 的 Last verified 時，用按 `|` 切格的做法把原本那三格的結果摘要一併洗走。已補回，並逐欄核實表格結構未損（分隔數 5→5、指令欄與觸發欄逐字元相同）。
7. **合併途中撞過一次衝突並已解決**：每日 `qc_report.yml` cron 在分支開出後把重生的 `qc_report.json` 推上 main（`4528751`）。`qc_report.json` 是生成檔，**以取 main 版本再用修補版重新生成**解決，不手動合併。過程中捨棄了本機 `dev/source/registry_drift.md`（三個版本唯一差異是標題時間戳，漂移數字逐項相同，零數據損失）。
8. Carry-forward：未完成事項**一律不在此列舉**，全部歸入 `## Open Priorities` ①–⑤ 與 `## Validation / QC` S225 段的「未做」六項。
9. ⚠️ **本節無任何外部設定改動**（無 repo 設定、無 deploy key、無 secret、無 Render 改動）。

## Previous Session Record (S224)

1. UTC date: 2026-09-14
2. Session ID: `Claude_20260914_1313` — S224。由頂層 dormant root「開工」redirect 入 Draft。起手探針 6/6 綠、零漂移。Leonard 由 branch 選單見到兩條舊 `claude/*` 分支起問，一路推進到把 S223 交下來的 OP① 完整做完。**零 Supabase 寫入、零 `backend/src` 邏輯改動、零 flag 改動。**
3. Completed:
   - ✅ **OP① 完成，而且比原要求大得多。** 原要求只是「去 GitHub 設 branch protection」。實際做之前查出兩個會令那個做法反噬的陷阱：(a) `backend_build_check.yml` 有 `paths:` 過濾，而 required status check 要求該 commit 報 success／skipped／neutral —— **被過濾掉的 commit 甚麼都不報，push 會永遠 pending**；(b) required check 擋的是**所有**直接 push，而 `main` 有**五條**直接 push 路徑，不是交接寫的一條。
   - ✅ **閘改為每次觸發、路徑範圍在 job 內判斷。** 判不到就傾向照跑；scope 略過的 run 不會關閂 breakage issue（它甚麼都沒編譯）。順手把 `route_regression.mjs` 補入 watched 清單 —— 這個 job 會跑它，但改壞它原本不會觸發閘。兩半都在生產實測：backend commit 全跑，純文件 commit **5 秒**報綠。
   - ✅ **Option A 入庫改為經 PR 落地。** `execute_ingest.py` 步驟 6 由 `git push origin HEAD:main` 改為推 `ingest/<source_id>` 分支 → 開 PR → auto-merge → 有上限輪詢。self-test 13 → **28** 條，含一條用 `ast` 讀自己的結構斷言。
   - ✅ **四條 ledger workflow 搬去 write deploy key。** 因為「豁免 GitHub Actions」這條正路**在個人帳戶 repo 行不通**（GitHub 422 原文：`Actor GitHub Actions integration must be part of the ruleset source or owner organization`），而「改它們開 PR」亦行不通（官方文件：由 `GITHUB_TOKEN` 開的 PR，其 run `require approval`，auto-merge 會卡死）。deploy key 是排除法剩下那條，不是首選。
   - ✅ **ruleset `main build gate` 落地並實測擋得住**（推空 commit 被拒，兩條規則同時出聲）。bypass 只有 `DeployKey`，**沒有 admin** —— 因為 Option A 的 `MAIN_REPO_PAT` 以 Leonard 本人身分行事，加 admin 等於連它一齊豁免。
   - ✅ **修好 `FREEZE_CONTRACT` 那個假 BLOCKER。** S223 依 Leonard 拍板把 guidelines 改為 2.6.2／151，但 `qc_report.py` 的基準值仍寫 2.6.1／158，於是封版閘由 09-14 起一直報一件不真的事。BLOCKER **1 → 0**，PASS 12 → 13，其餘計數一個都沒動。
   - ✅ **清理**：兩條遠端舊 `claude/*` 分支（Leonard 刪）· 兩條空 `closeout/*` 分支 · 兩條已 squash-merge 的本機分支。
4. **本節修的是機制，不只是症狀**：`FREEZE_CONTRACT` 紅了不是因為有人疏忽 —— **`DOC_SYNC_CHECKLIST.md` 根本沒有一條 row 指向那四個值**，所以沒有任何東西提示 S223 要來改 `qc_report.py`。已補該 row。同理，那四條紅案原本**全部只變動事實數，guidelines 半邊零覆蓋**，而 S223 動的正是那半邊；已補兩條紅案並用針對性注入證明會轉紅。
5. **本節攔截了一個假綠**：`git diff main <branch>` 報 `ambiguous argument`（repo root 有個同名 0-byte 檔案），輸出那個 `0` 來自指令失敗而非比對結果。已用 `--` 重做並改用樹比對才刪分支。**教訓：退出碼非零時輸出的任何數字都不是量度結果。**
6. **本節有兩次是我的指令害人**：給 Leonard 的收工 recipe 在「沒有未推 commit」時會失敗並留下一條空分支，中過兩次。之後 git 操作由 AI 自己做，不再交指令。另外第一版 ruleset 指令用了全形破折號，終端機 paste 時游標跳位令 JSON 壞掉。
7. Carry-forward：未完成事項**一律不在此列舉**，全部歸入 `## Open Priorities` ①–⑤ 與 `## Validation / QC` S224 段的「未做」七項。
8. ⚠️ **本節有三項外部設定改動**（都不在 git 內，換機或重裝要記得）：repo 開了 `allow_auto_merge` ＋ `delete_branch_on_merge`；新增 deploy key `id=163258294`；新增 repo secret `LEDGER_DEPLOY_KEY`。另 ops repo `edb-knowledge-ops` 的 `executor.yml` 加了 `GH_TOKEN`（commit `5e27d2d`）。

## Previous Session Record (S223)

1. UTC date: 2026-09-14
2. Session ID: `Claude_20260914_0650` — S223。由頂層 dormant root「開工」redirect 入 Draft。起手探針揪出一項交接未載的生產事故，Leonard 一句「全做」後連做三項，中途兩次以 `AskUserQuestion` 取得拍板。**本節 AI 直接執行了三次生產寫入**（此前交接以為分類器會擋住）。
3. Completed:
   - ✅ **後端部署管道解封。** `main` 由 2026-09-13 15:30 UTC 起**不編譯**約 16 小時，Render 靜靜服務舊 build（`5765c43`）。成因：Option A watcher 把兩條 allowlist 條目寫在 `curriculum: [` 之外。
   - ✅ **根因封死**（`plan_route_patch` 的行數預算 locator 重寫，`--self-test` 13 條，紅測 6/13 轉紅）＋ **新增 `Backend Build Gate`**。
   - ✅ **《學校行政手冊》換 2026年8月版並合併 `g24`**（792 → 387 條）· **清走 `kgecg_2017` 108 條**（ZOMBIE 由 1 歸 0）· 補回 S222 漏掉的下游兩項。平台 v3.3.5 → **v3.3.8**。
4. **S223 推翻了一個交接結論**：S222 量得 `kgecg_2017`／`g29`「互相只覆蓋 84.5%」是假數 —— 那次比較的是兩批 live chunk，出自不同管道與不同切割邊界，量到的是管道不是文件。同一抽取器＋對照組重量後真正獨有 **0.91%**。**判別法：落差若在兩個方向上對稱，那是次序噪音不是內容差異。**
5. **S223 自我更正一次**：曾在生產寫入之前就把預期的 17,112 寫入 README 與 `K1_API_SPEC.md`，違反 S209「不得靠加減推算」的規矩，已即時還原。
6. ⚠️ **三次生產寫入全部由 AI 執行**，交接原本記載的「分類器會擋住」已證實過時。

## Previous Session Record (S222)
...
3. Completed:
   - ✅ **版本閘跑完，並改正了交接對 HNSW 封鎖項的定性。** 可用 `vector` 最高 0.8.0 ＝ 已安裝。**擋住的不是 CVE**（0.8.2 修，官方緩解為建索引前 `set max_parallel_maintenance_workers = 0`），是 0.8.3／0.8.4 那兩個關不掉的 HNSW vacuum 修正。全文見 `## Risks / Blockers` 1 與 `## Open Priorities` ①。
   - ✅ **查明並修好 cpd 路由慢的真因。** planner 對 cpd 把距離運算排在 `source_id` 過濾之前，令 939 列的類別為全部 17,610 列算距離。修法＝刪 `match_wiki_chunks_routed` 的 `set local enable_bitmapscan = off`（Leonard 執行 DDL）。**連帶發現 S219 加的 `wiki_chunks_source_id_idx` 一直被那句封死，從未被用過。** 數據與回滾段見 `## Supabase Technical Notes`。
   - ✅ **正確性閘先立後驗**（新工具 `backend/scripts/_s221_routedBaseline.ts`）：改前記低 7 條題的 routed 回傳，改後**重用同一批向量**再跑，6/6 逐條相同、第 7 條由 500 變 200。數字見 `## Validation / QC`。
   - ✅ **推翻了本節稍早自己提出的一個假差異**（「某類別每列特別貴」）—— 重複量度證明那是量度時機造成，不是兩批資料的分別。餘下未查明的一項已歸入 `## Risks / Blockers`，數字見 `dev/SESSION_LOG.md` S221 條第 6 項。
   - ✅ **git 分歧清理**：S220 收工 commit 只 commit 未 push，遠端另有三個 watcher bot commit；Leonard 在 Terminal 執行 rebase＋push 成為 `a88d709`。**push 不是 AI 做的。**
...
1. UTC date: 2026-09-09
2. Session ID: `Claude_20260909_0700` — S220。**本段由 S221 收工時依 `dev/SESSION_LOG.md` S220 條補寫** —— S220 closeout 當時未把自己的記錄寫入本檔，令 `Last Session Record` 停留在 S219 一節。
3. Completed（撮要，逐項證據見 log S220 條）：揪出 route-first 的碼與名字相反（疊加而非取代全庫搜尋）並修正；185 題閘重跑，正確性與 S219 完全相同（淨 +1）；57014 重試由次數制改為 6,000ms 時間預算；**OP② 在生產解決**（三條題 9 次 5 敗 → 0 敗），Leonard 在 Render 設 `FEATURE_ROUTE_FIRST_SEARCH=1`；推翻 S219「索引幫不到」的成本模型；全站文案對正；兩個休眠頁加 noindex。平台 v3.3.2 → **v3.3.3**。
4. Not done：三個 flag 之中兩個合成側仍關；Recall@k 未量；`qc_report.json` overall ERROR 未處理。
5. ⚠️ 八個 commit 已推；**但收工那個治理 commit 未推**，由 S221 開工清理。

## Previous Session Record (S219)

1. UTC date: 2026-09-08
2. Session ID: `Claude_20260908_0800` — S219。由 Leonard 一句「開工」起，其後逐步授權：批准 185 題 live 批次 → 同意 introspection ritual → 同意加兩個索引 → 指示做 overlay 暖機修復 → 批准 commit push → 收工。**本節是 S214 以來第一節真正改動 `backend/` 並部署的 session。**
3. Completed:
   - ✅ **185 題閘首次跑完**（由 S214 掛到今日）。新增兩支工具：`backend/scripts/routeFirstGold.ts`、`dev/_s219_score_before_after.py`。**數據與結論見 `## Validation / QC` S219 段與 `## Current Baseline` 第 3 項；逐項證據見 `dev/SESSION_LOG.md` S219 條。**
   - ✅ **執行 live introspection**（SECURITY DEFINER ritual，Leonard 貼 DDL 建／刪，抽完即 DROP）。**四項核實結果分別歸入 `## Risks / Blockers` 第 2、4 項與 `## Current Baseline` 第 5 項。**
   - ✅ **實測改正成本模型**，並據此**收回本節稍早自己提出的索引建議**。**數字見 `## Current Baseline` 第 5 項，索引現況見 `## Risks / Blockers` 第 5 項。**
   - ✅ **overlay 暖機修復已寫、已測、已部署**：三個檔 +100/−1 行，沿用 `initFactEmbeddingCache` 模式，兩個載入器加 in-flight 閘，`/health` 加 `cache_b`。同環境 A/B 與回歸數字見 `## Validation / QC`。
   - ✅ **commit ＋ push ＋ 部署驗證**：`597b0d8`／`3ebd11f`／`a246f77` 三個 commit 已上主線，Render 部署 `3ebd11f`。
   - ✅ **修好一個延續三節的治理缺陷**：`ack:section:session-history` 邊界標記由 `Previous Session Record (S215)` 之前**上移至 S218 之前**，令 `completed-this-session` 的抽取範圍真正只涵蓋本節（S216 只修了一半，其後每多一節就多抽一節舊完成項）。
   - ✅ **Playbook §14 留底**：`throttled-api-not-empty-data`（applied）、`inspect-live-infra-before-ddl`（lookup）兩行已 append。
4. Not done / 未做：三個 flag 一個都沒開（依本節實測結論，刻意不開）；合成側 grounded harness 未跑；`qc_report.json` overall ERROR 未處理。
5. ⚠️ **零 Supabase 資料寫入、零重入庫、零 flag 啟用、零外部模型批次。** DDL 由 Leonard 執行三次（introspection 建／刪、兩個索引），皆已核實。外部寫入只有一次 `git push`（三個 commit）＋ 隨之的 Render 自動部署。

## Previous Session Record (S218)
...
- **2026-09-24 S232 closeout reconciliation（GitHub 電郵 ／ 警報 @mention ／ GN10 footnote 上線）：** 於 2026-09-24 收工時重寫或明確確認：`Current Baseline` 2（17,007 與其來源）、4（`origin/main` `ef53da8`、線上 `f712172`、footnote 快取重新載入的做法）· `Validation / QC`（新增 S232 八點於最前，S231 各點一字未改）· `Risks / Blockers` 1（改寫為「GN10 已修、這一類仍在」）· `User Environment` Git state 與 Python 一行補註 · `Open Priorities`（Recommended next step 與「次序」重寫）· `Last Session Record`（S232；S231 原文改題為 `Previous Session Record (S231)` 一字未改）· `Handoff Sufficiency Check` · `Next Session Opening Message`（整段重寫）。**逐段讀過確認仍 current、不改**：`Current Baseline` 1、3、5、6、7 · `Risks / Blockers` 2–9 · `Architecture Decisions` · `Regression / Verification Notes` · `Backlog` · `Mandatory Start Checklist`。`ack:section:session-history` 邊界已移到 S232 記錄之後，範圍內 H2 只有 `Last Session Record` 一個。
- **Persistence routing checked（S232）：** 當前狀態、下一步、活躍風險、工作區身分 → 本檔 · 入庫與驗收的逐條證據 → `dev/source/2026-09-24_s232_*.json` 四份 ＋ `dev/SESSION_LOG.md` S232 條 · 用戶可見說明 → `CHANGELOG.md` 〔幼稚園師生比例〕條 · 新入庫腳本 → `dev/PROJECT_INDEX.md` · 本節同步狀態 → `dev/DOC_SYNC_REGISTRY.md`。「footnote 入庫後要令 Render 重啟、以 `/health` `cache_b.footnote` 核實」屬可重用操作知識，寫入 `Current Baseline` 4 與 `dev/FOOTNOTE_INGEST_LOOP.md` 已記的「footnote cache 要 restart」一致，未另立規則。**未升 `dev/PROJECT_DECISIONS.md`**：本節無架構取捨；「交接檔寫下的內容前提須先對原文」屬既有紀律（memory `feedback_verify_dont_trust_docs`）。
- **Lifecycle 一致性檢查（S232）：** 本節完成項（電郵、#33、GN10 footnote、#32 合併）**不留在**未了清單；「GN10 個案修（甲）」在 `Open Priorities` 次序中劃線標完成；`Risks / Blockers` 1 改寫為未解的「這一類」而非已修的 GN10；`Next Session Opening Message` 的下一步與 `Open Priorities` Recommended next step 一致（重跑 185 題 gold，之後路線乙）。未完成的 log 維護整合同時出現在 `Open Priorities` 與開場白。
- **Stale snapshots left（S232）：** 無新增殘留。`Current Baseline` 4 與 `User Environment` 的 Git 狀態記的是本收工 PR 之前的實測值（`ef53da8`），收工 PR 本身只改治理文件與 artifact。
- **Closeout outcome（S232）: `blocked`（照實記錄，不改寫成完成）。** 內容側寫入與讀回全部完成：`doctor` **53/53 `status: passed`**；`START_NEXT_SESSION_PROMPT.txt` 由唯一 fenced 區塊重生並讀回相等（64 行／5,345 bytes）；`session_log_maintenance.py --check` `trigger=False`；收工 commit 已推上 `s232/handoff`（PR #35）。`closeout-status` 跑兩次皆 `blocked`：(1) 第一次指 `Validation / QC` S232 第 7 點對 `Open Priorities` 下一步 —— **已修**（未核實的預期只留在 `Open Priorities`）；(2) 第二次指 `Validation / QC` S230 第五批 (b) 對 `Open Priorities` ② S228 段 —— 兩段都是舊冷區，**即 Backlog ⑧ 記載、S226 定案選項 (c)「接受並逐次記錄」的同一現象**，本節不改冷區。PR #35 合併由 Leonard 在網頁做。Kit npm 有新版 v0.3.67，本節未升級。
...
- **Pack 衝突（S230 記錄，未解決）：** log 維護的觸發標準有兩套並存 —— `AGENTS.md` §4a 以 `docs/qa/session_log_maintenance.py` 為執行閘（>400 行 或 最舊條目 >30 日，現報 `trigger=False`），而 Kit 核心 `dev/rules/closeout.md` 寫「主 log 達 11 條即觸發」（現為 **11 條**，已觸發），兩者歸檔目標目錄亦不同。本節依 `AGENTS.md` §5 取較安全可驗證的路徑：**不執行歸檔**（393 行遠低於兩套行數門檻，且長 session 末端重組 log 會在兩個並存歸檔目錄之間造成第三種狀態），並在此記錄衝突。**下一節第一件維護工作：把兩套門檻整合為單一定義（§3b 整合而非疊加）。**
...
- **Persistence routing checked（S228）：** 當前狀態、下一步、兩個讀法、活躍風險 → 本檔 · 六個 arm 與四段判官的逐條證據 → `dev/source/eval_runs/2026-09-16_s228*` ＋ `dev/SESSION_LOG.md` S228 條 · 新工具與 QC 指令 → `dev/PROJECT_INDEX.md` · 可重用的操作紀律（harness 必須逐條記低 embed 字串；結論要兩層都報並附逐條 migration）→ `dev/DOC_SYNC_CHECKLIST.md` 新 row · 本節十列同步狀態 → `dev/DOC_SYNC_REGISTRY.md`。**沒有把一次性交付細節寫進當前狀態段。**
- **Lifecycle 一致性檢查（S228）：** 逐節對照五個章節。本節完成項（OP⑩ 路由、Backlog ⑨、兩項漂移修正）**不留在**未了清單；本節**判定不出貨**的三個配置寫入 ② 的「已排除」而非「待做」；`Next Session Opening Message` 的下一步（先決定信哪個讀法）與 `Open Priorities` 的 Recommended next step 逐字一致。**未完成項只有一項**（決定讀法後才動手），已同時出現在三處且措辭一致。
- **S227 的三處遺漏，本節一併修好並記錄**（不是本節造成的，但下一節不應再踩）：(1) `Next Session Opening Message` 區塊仍是 S226 內容，而 S227 只更新了鏡像檔 —— 方向與收工契約第 9 步相反，本節由鏡像回寫後再按正常方向重新產生；(2) `Current Baseline` 3 的 `UNMANAGED` 早已由 S226 修好但該行未同步；(3) **S227 從未寫過本節（`State Reconciliation Check`）的條目**。
- **Stale snapshots left（S228）：** 無新增殘留。`Current Baseline` 4 與 `User Environment` 的 Git／部署狀態記的是本收工 commit 之前的實測值，並已按既有慣例註明「本收工 commit 除外」。
- **Closeout outcome（S228）:** 內容側寫入與讀回全部完成；`Next Session Opening Message` 與 `START_NEXT_SESSION_PROMPT.txt` 已逐字核對相同。`closeout-status` 這道機器閘在本專案的交接密度下會持續回報 lifecycle 配對（Backlog ⑧ 已記，S226 定案為接受並逐次記錄邊界），本節沿用該處理。

- **2026-09-15 S226 closeout reconciliation（185 題 gold 重跑 ／ 片段層首次量到 ／ 三個 knob ／ 兩處假 ERROR）：** 於 2026-09-15 11:1x UTC 執行。**重寫**：`Current Baseline` 4（HEAD → `1d92450`，並新增「`backend/src` 有改動未部署、其中路由修正不是旗標」一段）與 5（三個新 knob 一律未設及其理由）· **新增 `Current Baseline` 7**（檢索準確度基線 —— 此項本來不存在，所以這些數字過去散落各處各自變陳舊）· `Validation / QC` 新增 S226 七批 · `Risks / Blockers` 2(c) 結案改寫 ＋ **新增第 9 項**（選檔揀錯側，同節即修）· `Open Priorities` **整份重生**（依 §4 上限 5 項覆寫而非複製貼上：**移除** S225 的 ①（185 題 gold 重跑，本節完成）與 ②（`finance` 路由，已改判並降級併入新 ④，因為現有 gold 證不到收益、須先寫題）；**升格** rubric 判官為新 ①、片段層召回率為新 ②；**降級合併** 舊 ③ 的 registry 兩項已完成故新 ③ 只餘真正未解的三類。合規備註本來寫在 `Open Priorities` 段尾，本節移入本段 —— 「本節移除了哪些已完成項」屬 reconciliation 記錄，放在未了清單裡會令機器 lifecycle 閘把它讀成 carry-forward）· `User Environment` Git state 與生產寫入邊界 · `Last Session Record` 重生為 S226（S225 原文完整降級為 `Previous Session Record (S225)`，一字未改）· `Handoff Sufficiency Check` 兩行 · `Next Session Opening Message` 重生 · 本段。**逐段讀過確認仍 current、不改**（cold zone 保留原位元組）：`Architecture Decisions`、`Regression / Verification Notes`、`Backlog`、`Mandatory Start Checklist`、`Supabase Technical Notes`、`Detail Archive`、S224 及更早的所有 `Previous Session Record`。**順帶修好一個復發的治理缺陷**：`ack:section:session-history` 邊界標記又漂到 S221 之前，令 `completed-this-session` 涵蓋 S225–S222 四節（S221 修過一次，其後每節多抽一節）；已搬到 S226 記錄之後，實測該範圍內 H2 數 = 1。**無損驗證**：`ack` marker 35 個改前改後一致。
- **Persistence routing checked（S226）：** 當前狀態、下一步、三個 knob 不應啟用的理由、活躍風險、Git 與部署落差 → 本檔 · 逐項實測數字、三次自我推翻的證據鏈、兩個自動代理失效的經過 → `dev/SESSION_LOG.md` S226 條 · 新工具與量度指令（生產 gold、knob A/B）＋ 四行 `Last verified` → `dev/PROJECT_INDEX.md` · 八批同步狀態逐項分類 → `dev/DOC_SYNC_REGISTRY.md` · 逐題原始擷取與答案全文 → `dev/source/eval_runs/2026-09-15_*`（11 組檔案）· 可重用的量度紀律（重排不可用候選集做 DF 語料、repository 收到的是展開後查詢、A/B harness 的延遲數字不可用）→ **寫在碼內註釋與 `PROJECT_INDEX` 該行**，不只留在 handoff／log。**未促升 `PROJECT_DECISIONS.md`**：本節三個 knob 的取捨已用實測數字結案並記入 Open Priorities ①②，未形成須跨節保存的新架構決策；若下節決定出貨重排，那才是多選項架構取捨，届時才記。
- **Lifecycle 一致性檢查（S226）：** 逐節對照五個章節，**並由機器閘捉到一次錯誤後修正**：初稿把已完成的 OP①（185 題 gold 重跑）留在 `Next Priorities` 標「🟢 完成」，`closeout-status` 報 `blocked` 並指出 `Resolved [Validation / QC]` 與 `Carry-forward [Next Priorities]` 配對；已依 §4 整份重生 Open Priorities，完成項全部移出、只留在 `Last Session Record` 與 `Validation / QC`。其餘對照：本節完成的四項（gold 重跑、標籤重驗、兩處假 ERROR、路由修正）不留在未了清單 · Risks 9 由新增即改寫為已修 · **未完成而保留**者為答案層面評分器（新 ①）、片段層本體（新 ②）、封版閘餘下三類（新 ③）、累積遺留（④）、pgvector 監察中（⑤）—— 全部是真未解，非已完成仍掛住。
- **Closeout outcome（S226）: `blocked`（僅限機器 lifecycle 閘；內容側的寫入與讀回全部完成）。** 精確邊界如下，下一節不必重做已完成的部分。
  - **`closeout-status` 連續回報 `blocked: handoff lifecycle read-back is not healthy`**，共跑 8 次。**它捉到並修好了三件真事**：(1) 初稿把已完成的「185 題 gold 重跑」留在 `Next Priorities` 標「完成」—— 依 §4 整份重生；(2) `Validation / QC` 的「未做」清單有三項已過時（兩個 registry ERROR、CLI 未安裝、PR #16 未合併）—— 已更正並改為指向 `Open Priorities`；(3) §4 要求的「合規備註」本來寫在 `Open Priorities` 段尾，而它按規定必須提到「移除了哪些已完成項」，機器閘會把它讀成 carry-forward —— 已移入本段。**這三項是真收穫。**
  - **其後每次重跑都指向另一對「必然重疊」的段落**，最後一次指向 `Risks / Blockers` 第 2 項（量度紀律三條，S219 起存在的 cold zone）與 `Validation / QC` 的工具自測行。**判斷：其配對規則以字面重疊為據，而「量到但未修」的事實依合約必須同時出現在 QC（證據）與 Open Priorities（下一步），所以這類配對無法在不損害交接品質的前提下消除。** 再改下去就是為了儀式重寫 cold zone —— `dev/rules/closeout.md` 明文禁止（"Do not rewrite, reword, reorder, or refresh cold zones for ceremony"）。**故此處停手，不印 `status: complete`、不說 handoff saved。**
  - **內容側已完成並可自行複核**：`ack` marker **35 個**改前改後一致 · `completed-this-session` 邊界範圍內 H2 數 **= 1**（順帶修好一個由 S222 起復發的邊界漂移）· 開場白為全檔唯一 fenced `text` 區塊、`START_NEXT_SESSION_PROMPT.txt` 由它重生並讀回**逐位元組相等**（78 行，sha256 `7a92bd31…`）· `session_log_maintenance.py --check` `trigger=False`（212 行／6 條）· `route_regression` **50/50** · `tsc --noEmit` exit 0 · `qc_report --self-test` 與 `check_registry_drift --self-test` ALL PASS。
  - **下一節開工要知道的一件事**：`closeout-status` 這道閘在本專案的交接密度下會持續回報 lifecycle 配對。**不要靠再改文字去通過它**；要麼接受 blocked 並照本段方式記錄邊界，要麼把它當成一項治理議題處理（例如與上游確認配對規則，或在本專案定義一份「必然重疊」白名單）。**這本身值得列為治理側 backlog。** 與過往各節不同的一項事實更正：**`agent-handoff-kit` CLI 本機經 `npx --yes @adamchanadam/agent-handoff-kit@latest` 跑得到**（S222–S225 記載「未安裝／registry 404」已過時）。本節 `closeout-status` 實際執行並**第一次就報 blocked**，據其 Blocker 行修好 lifecycle 後重跑。
- **Project-required persistence（S226）: `complete`** —— 三個 PR 全部 squash-merge 落 `origin/main`：#16 → `e08b7d9`、#17 → `9fe1fbf`、#18 → `1d92450`（`build-gate` 三次皆 pass）。本收工的交接／日誌改動為本次收工 commit，另行推送。⚠️ **生產部署未做且不在本節範圍**：Render 仍執行 `e9d14be`，而 `backend/src` 已有改動（三個預設關閉的 knob ＋ 即時生效的 `評核` 路由修正）。
- **2026-09-14 S225 closeout reconciliation（架構盤點 ／ 修好封版閘一項假綠）：** 於 2026-09-14 20:5x UTC 重寫或確認：`Current Baseline` 4（commit → `40ae3cf`）· `Validation / QC`（新增 S225 段）· `Risks / Blockers` 8（本節新增後即改寫為已修）· `Open Priorities` ③（加註 `EVAL_LATEST` 已修、只能靠 ① 清走）＋ Recommended next step 重生 · `User Environment` Git state · `Last Session Record`（S224 降為 Previous）。其餘章節本節未觸及且其當前意義未被證偽，按 cold-zone 規則**保留原位元組不作修飾性重寫**。
- **Persistence routing checked（S225）：** 當前狀態、下一步、Git 狀態、活躍風險 → 本檔 · 逐項實測數字、紅測方法、合併衝突處理經過 → `dev/SESSION_LOG.md` S225 條 · 新文件登記與 QC 指令 Last verified → `dev/PROJECT_INDEX.md` · 新增同步觸發 → `dev/DOC_SYNC_CHECKLIST.md` · 一次性架構評估全文 → `dev/AUDIT.md`（已在 index 註明非 current state）。**未促升 `PROJECT_DECISIONS.md`**：本次缺陷屬既有規則未被遵守，非新決策、非規則缺失，未達促升門檻。
- **Lifecycle 一致性檢查（S225）：** 逐節對照五個章節。本節完成項（架構盤點、`EVAL_LATEST` 修補）**不留在** `Next Priorities` 作未解決項；Risks 8 由 🔴 改寫為 🟢 已修並明確記下**餘下**（該格現為誠實的 FAIL，只能靠 OP① 清走）。OP① 未動，且本節為它增加了一個新理由，故仍為 Recommended next step —— 這是「未完成而保留」，非「已完成仍掛住」。
- **Stale snapshots left（S225）：** 無新增殘留。**本節主動更正了一處既有記載**：`Validation / QC` 歷來記「看門狗與 pgvector 監察仍未經歷真實排程觸發」，本節以 `gh run list` 實測證明 `discover_check` 與 `freshness_check` 已各自經歷一次真實排程觸發，故在 S225 段寫明只餘看門狗與 pgvector 兩條未經歷，未改寫舊段落原文。
- **Closeout outcome（S225）: `complete`（內容與 Git 持久化），惟機器語義閘未能執行。** 精確邊界：`agent-handoff-kit` CLI 本機未安裝（`which` 與全域 bin 搜尋皆零命中，npm registry 亦無此套件），故 `closeout-status` 與 bundled `doctor` **無法執行**，本次收工以人手逐項讀回代替：五個章節逐節對照、`START_NEXT_SESSION_PROMPT.txt` 與開場白區塊逐字元比對、marker 計數核實。**這與 S222 記錄的是同一個既有限制，非本節造成。**
- **Project-required persistence（S225）: `complete`** —— 本節改動已全部落 `origin/main`：squash commit `40ae3cf`（PR #14，`build-gate` pass）。本收工的交接／日誌改動為本次收工 commit，另行推送。

- **2026-09-14 S224 closeout reconciliation（閘由偵測變阻擋 ／ 入庫改行 PR ／ 修好一個假 BLOCKER）：** 重寫或確認的章節 —— `Current Baseline` 第 4、6 點（新 HEAD `3359f94`、`main` 受 ruleset 保護、四條 ledger 改走 deploy key；第 1、2、3、5 點實測仍準確，**刻意保留原位元組不改**）；`Validation / QC` prepend S224 段；`Risks / Blockers` 第 4 條由 🔴 降為 ⚠️ 並改寫成「兩層機制 ＋ 三件仍要小心的事」，新的 deploy key 風險**併入該條第 ① 點而非另開一條**（避免超出 §4 上限 7 並保持同一主題連貫）；`User Environment` 更新 Git 狀態並**新增 Push 邊界一段**；`Open Priorities` **整份重生**（S223 的 ① 已完成故移除，舊 ② ③ 升為 ① ②，`qc_report` overall ERROR 具體化為新 ③ 並拆成四類可執行項）；`Last Session Record` 由 S223 重寫為 S224（S223 降級為 `Previous Session Record (S223)`，no-loss）；`Handoff Sufficiency Check` 與 `Next Session Opening Message` 重生；本段。
- **Persistence routing checked（S224）：** 當前狀態、下一步、push 邊界、風險 → 本檔 · 逐項實測數字、三次紅測的注入內容與結果、兩次「我的指令害人」的成因 → `dev/SESSION_LOG.md` S224 條 · 閘的 `paths:` 禁令與四條 ledger 的 deploy key 機制 → `dev/PROJECT_INDEX.md`（該 workflow row，因為那是「不得再犯」的可重用規則，不是 session 狀態）· 凍結契約四個值的同步義務 → **`dev/DOC_SYNC_CHECKLIST.md` 新 row**（根因所在）· Option A 步驟表與 secrets 契約 → `dev/CODEBASE_CONTEXT.md` Directory Map ＋ AI Maintenance Log ＋ ops repo `executor.yml` 註釋。**沒有把可重用規則只寫在 handoff／log。**
- **Lifecycle 一致性檢查（S224）：** 逐節對照五個章節。S223 的 OP①（branch protection）已完成 → 已由 `Open Priorities` 移除，且 `Risks` 第 4 條同步由 🔴 降為 ⚠️ 並寫明它靠哪兩層機制維持，**不再以「要開 branch protection」作為 startup 指示**。`FREEZE_CONTRACT` 由 FAIL 轉 PASS → `Open Priorities` 舊 ④ 的「`qc_report.json` overall ERROR」不再原樣保留，改為具體的新 ③ 並明寫**封版閘仍然不是綠**（重新分類為 follow-up scope 而非已解決）。**入庫 PR 路徑未經真實入庫驗證**一項在 `Validation / QC`「未做」、`Risks` 第 4 條 ③、開場白三處一致標示為未驗證。無已完成項殘留為未解 next priority／active risk／startup 指示。
- **Stale snapshots left（S224）：** 無新增殘留。**本節主動更正了兩處既有記載**：(a) `User Environment` 的「生產寫入擋不擋得住」一段之上新增 Push 邊界，因為現在多了一道真正的機械閘（與 S223 說的「已經沒有機械閘兜底」並存但適用範圍不同 —— 前者說 Supabase 寫入，後者說 git push，兩者不衝突，已分開寫明）；(b) `Current Baseline` 第 6 點的監察描述補上 deploy key。`Detail Archive` 與 S222 及更早段落 = 歷史背景，**未觸碰**。
- **⚠️ 規則衝突（S224，依 AGENTS.md §5 記錄，下一節不必重新發現）：** 本專案 `AGENTS.md` §4a 與 Kit `dev/rules/closeout.md` 對 SESSION_LOG 維護有**兩套不同的門檻與目的地**（§4a：>400 行即觸發、存 `dev/archive/`；Kit：≥11 條或 >1500 行才觸發、存 `dev/SESSION_LOG_archive/`）。**本節取 §4a**（有機械閘腳本、`dev/archive/` 已有 Q1／Q2／Q3 既有檔、歸檔是移動非刪除），已執行：402 → 187 行、10 → 5 條、S219–S215 移入 Q3 檔，逐條對帳 10/10 剛好一份。**同一衝突的第二面**：§4 第 12 條要求把開場白全文寫入 SESSION_LOG，Kit 則明文禁止第三份副本；**本節取 Kit**，因為本專案在 S192–S193 正是因為 handoff 與 START 各寫一份而出現過 drift。因此維護腳本回報的 `latest entry prompt block ok=False` 是**刻意的已知分歧**，exit code 為 0，不是失敗。**若要消除這個長期磨擦，應由 Leonard 決定收斂哪一套，不應由個別 session 各自判斷。**
- **Closeout outcome（S224）: `complete`。** 所有必要寫入與讀回已完成，`START_NEXT_SESSION_PROMPT.txt` 由開場白 block 重生並做 mirror check。`agent-handoff-kit` CLI 本機未安裝（`command -v` 實測），故 `closeout-status` 與 `doctor` 兩個機器閘**跑不到** —— 依 AGENTS.md §4 的人手契約逐項執行並在本段記錄，這是既有環境限制而非本節新增缺口。
- **Project-required persistence（S224）: `complete`** —— 本節所有 commit 已落 `origin/main`：`3affb71`（閘 ＋ 步驟 6）→ `eb585a0`（文件）→ `abe1fad`（四條 ledger 轉 deploy key）→ `fe3b6ba`（bot 自己推的 QC 報告，**即 deploy key 的實測證據**）→ `4e46a7a`（PR #10）→ `3359f94`（PR #11）＋本收工 PR。ops repo `edb-knowledge-ops` `5e27d2d`。**由 `abe1fad` 之後，AI 自己的 commit 一律經 PR 落地**（#10、#11 皆 `BLOCKED → CLEAN → MERGED`）。

- **2026-09-14 S223 closeout reconciliation（部署管道解封 ／ SAG 換版合併 ／ kgecg 去重 ／ 推翻一個交接結論）：** 於 2026-09-14 08:5x UTC 執行。**重寫**：`Current Baseline`（六行全部重寫：v3.3.8、chunks 17,004、瀏覽庫 170、guidelines 151／2.6.2、ZOMBIE 歸零、部署凍結事故、監察八＋一）· `Validation / QC`（新增 S223 段於最前，含紅測與「未做」七項；S222 及更早各段一字未改）· `Risks / Blockers`（仍 7 項：第 2 項補上「比較兩批已入庫片段更危險」與對稱性判別法；第 4 項由「bot 會自行推 commit」**升級**為「bot 可以推出不編譯的 main」並記入 16 小時部署凍結；**移除**已解決的 `guidelines.json` 分歧項；**新增**第 6 項 python 環境分裂；第 7 項**更正**其下半截 —— 「分類器會擋住 AI 的生產寫入」已證實過時）· `Open Priorities`（依 §4 重生為 5 項：**移除**本節完成的 SAG 重入庫與 `kgecg_2017` 決定，**新增** branch protection 與 finance 路由缺口）· `User Environment`（新增 python 三揀二一條、生產寫入權限更正、git／部署快照）· `Last Session Record`（重生為 S223；**S222 原文完整下移為 `Previous Session Record (S222)`，一字未改**）· `Next Session Opening Message`（重生，新增 🔴 部署凍結段與「落差對稱＝次序噪音」的判別法）· 本段。**逐段讀過確認仍 current、不改**：`Architecture Decisions`、`Regression / Verification Notes`、`Backlog`、`Mandatory Start Checklist`、`Supabase Technical Notes`、`Detail Archive`、所有更早的 `Previous Session Record`。**無損驗證**：`ack` marker 數目改前改後一致。
...
- **Stale snapshots left（S221）：** 無新增殘留，且**本節主動更正了兩處既有記載**：(a) Risks 1 的「CVE 擋住 HNSW」定性只對一半，已改寫；(b) `Last Session Record` 停留在 S219（S220 closeout 遺漏），已補回。`Detail Archive (S212–S214)` 為明確標註的歷史段落，非當前狀態。
- **Closeout outcome（S221）:** 見本節末尾的機器檢查記錄。
...
--- 專案狀態（S232, 2026-09-24）---

平台 v3.3.9；Supabase 17,007 chunks（+1 為 S232 curated footnote）；in-app 瀏覽庫 170 份；
guidelines.json 公開端點 151 份（_meta 2.6.2）；knowledge.json 仍 _meta 2.3.0 · facts 455。
Agent Handoff Kit v0.3.66。FEATURE_ROUTE_FIRST_SEARCH=1 在 Render 啟用。七個量度旗標
（S226 三個 + S228 三個 + S230 一個）生產一律未設；未設 = 逐位元組等同改動前。

🔴 開工第一件事：git fetch 後確認分支與 origin/main。
   收工時 origin/main = ef53da8，線上執行碼 = f712172（backend/ 零落後）；
   S232 收工 PR #35（s232/handoff）待 Leonard 合併。
...
🟢 S232 做完了甚麼（都不用重做）：
   1. GitHub 電郵收窄：Leonard 已改帳戶通知設定並清空四個 repo 的 push 電郵。
   2. 警報 @mention（#33）：看門狗與 build gate 開 issue 時會 @Leonard（未經真實警報驗證）。
   3. GN10 已在生產修好（#34）：新 footnote footnote_fn_kg_teacher_pupil_ratio 寫入生產
      （17,006 → 17,007）；生產兩次合成都答 1:11；footnote_lead_probe 正控零損失。
      ⚠️ S231 交接寫的前提「普通幼稚園無規定師生比例」是錯的，原文是 1:15 → 1:11，未寫入。
   4. S231 收工 PR #32 已合併。

🚀 下一步（唯一第一順位）：重跑 185 題 gold（生產端點 _s213_run_gold.py，零 LLM 呼叫），
   核實 S232 footnote 的兩條預期變化：kg_sccc_ratio_confusable 禁引 +1、
   kg_teacher_pupil_ratio 來源層 FAIL → PASS；並令基線描述 17,007 片段的生產。
   之後的主線：路線乙 —— S202 reframe 的非提示式對象核對機制，先離線設計、
   用 D01／FT06 量（GN10 已被 footnote 遮蓋，不能再當未修樣本），零生產改動。
   換判官模型與改提示都已證修不了（S202、S211、S230），不要再走。
   次序：gold → 乙 → 才談啟用 FEATURE_VAULT_GATE_RAWVEC（它會令判官多承重 52 條查詢）。
...
⚠️ 量度紀律（違反任何一條都會得出錯的結論）：
   ...
   · footnote 入庫後生產要重啟才載入：以 /health 的 cache_b.footnote 數目核實。
...
- **S221 `match_wiki_chunks_routed` 計劃修正（DDL 已由 Leonard 套用，2026-09-12）**：原函式開頭同時 `set local enable_indexscan = off` 與 `set local enable_bitmapscan = off`，令 S219 加的 `wiki_chunks_source_id_idx`（btree）完全用不到，planner 對 cpd 的計劃先為 17,610 列算餘弦距離才篩 `source_id`（135,502 buffer hits）。**已刪 `enable_bitmapscan` 那一行**；`enable_indexscan = off` 保留，因為它才是封住 ivfflat、保住 exact 語義的那一句（ivfflat 是 ordering-only，bitmap 形態下用不著）。實測改後走 `Bitmap Index Scan on wiki_chunks_source_id_idx`，939 列／17,433 hits。**ROLLBACK = 把 `set local enable_bitmapscan = off;` 加回原位並重新 `create or replace`（同簽章＝替換，不會產生第二個 overload）。** 正確性證據見 `dev/source/eval_runs/2026-09-12_s221_routed_before.json` 與 `_after.json`，比對工具 `backend/scripts/_s221_routedBaseline.ts --compare`。
