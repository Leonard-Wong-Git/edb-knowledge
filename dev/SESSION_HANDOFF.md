...
1. **🔴 資料質素 backlog（S137 已完整診斷、3 子任務、待 Leonard 決執行）** — (a) **SEN dedicated route**（routing-not-cutoff lever、~15 行 searchChannelB.ts、spec ready、low-risk win、可即做唔使等其他）；(b) **phys_sss_2007_2015 182 亂碼修法**：DROP-now（cb3_deprecate_stale.py、9,849→9,667、reversible）/ OCR 重抽（須先 `brew install tesseract tesseract-lang`）/ 後端 denylist——**根因 = CID-glyph、不可 decode 復原、勿當 S135 carry-decode 試**；(c) **g10/g19 SEN 真資料缺口 ingest**（registry verified 但 0 chunks；fetch→驗冇 mojibake→page-carry→索引、同 S135 history_jss backfill pattern）。
2. **🔵 Leonard 真機 verify pending（S136 遺留）**：手機「指引文件」tab（#guidelines mobile render）+ Channel B 政策搜尋 policychecker.wongfu.net（CORS 已修、應通）。
3. **下一階段方向（待 Leonard）**：Q4 對外契約收斂（見 #5）/ §8b rule 2 automation（見 #5）/ 39→148 guidelines 擴展。
4. **🔴 既有 deferred + low-priority backlog**：§E.10 admin-login client-side gate（**S131 ACCEPTED + DOCUMENTED**，conditional on cosmetic-gate design unchanged — 拆掉 client-side-only 前提即須 reopen）；57014 transient（retry 即恢復）；FAIL-A 注入 regression（record-only）；39→148 deferred；HKEAA；doc-debt；**stat_fact 21 chunks 仍 cite 2024/25 wording**（升 2025/26 = future；ROI≈0 因 Channel B filter `content_type!=="stat_fact"`，唔急）。
5. **Q4 對外契約收斂（deferred）+ §8b rule 2 automation（future）**：Q4 = Channel A→knowledge.json→Circular System、3 選項、未明示勿掂；§8b rule 2 = semantic-supersede KLA-title embedding similarity check 暫 process-level、automated sub-agent prompt 留 future。
...
## Last Session Record
1. UTC date: 2026-06-01
2. Session ID: Claude_20260601_1143 (Session 137 — 資料質素 backlog 診斷：phys mojibake 根因 + 「sen」短 query routing；READ-ONLY 0 mutation)
3. Completed:
   - ✅ **[Trigger]** Leonard `/workflow 全做` → 背景 workflow（3 agent：phys + shortquery + adversarial verify）+ inline sibling-audit + 直讀 searchChannelB.ts 診斷兩件 backlog。
   - 🔴 **[發現#1 — phys mojibake 唔係 S135 pattern，§G.2 doc-drift 又中]** 交接寫「同 S135、可即做」**證實為錯**。真根因 = CID-glyph（源 PDF Identity-H/無 ToUnicode CMap → PyMuPDF 抽 glyph 索引當 Unicode → valid-but-wrong CJK；物理/課程/科學 出現 **0** 次、U+FFFD=0）。**不可 decode 復原、必須 OCR 重抽**（S135 係 byte 雙重編碼可 carry-decode、唔同類）。182 chunks 全 vault_extract 100% 亂碼（adversarial verify 獨立 re-count=182、agree=true）。🚧 Tesseract 未裝 = OCR blocker。欄名 `text` 非 `content`。
   - 🟢 **[發現#2 — sibling audit]** 掃 65 page-carried PDF：**phys 係唯一 mojibake、孤立非 family-wide**（chem/bio/ict/history/geog_sss 全 clean）。
   - 🔴 **[發現#3 — 「sen」根因疊加]** ①routing gap（主）：「sen」配唔到 TOPIC_KEYWORDS → 全庫 raw 搜 → 落 phys 亂碼 @0.26；②phys 亂碼 = 落腳點；③g10/g19 真資料缺口（registry verified 但 0 chunks）。佐證①：大寫「SEN」/「特殊教育需要」已正確 surface role_facts_student @0.43-0.62（SENCO gold fact）+ g06 → 內容 retrievable、係 routing 問題。
   - ✅ **[修法設計 ready，未實施]** SEN dedicated route（routing-not-cutoff lever，~15 行 searchChannelB.ts）= TOPIC_KEYWORDS.sen（`\bsen\b/i`+特殊教育/融合教育/統籌主任/SENCO/特殊學校，置 curriculum 之前）+ SOURCE_SETS.sen=[g06,sag_2025_11,role_facts_student,role_facts_general,g10,g19] + QUERY_EXPANSIONS.sen。
   - ✅ **[出 PLAN → Leonard「收工」]** 兩執行 fork（phys 修法 DROP/OCR/denylist + g10/g19 ingest）Leonard 收工 → 全部 deferred。
4. Pending: SEN route 實施；phys 修法決策；g10/g19 ingest 決策；Leonard 真機 verify（Mobile + Channel B，上 session 遺留）
5. Next priorities (max 3):
   - **SEN dedicated route**（low-risk win、spec ready、可即做唔使等其他決策）
   - **phys 182 亂碼修法**（DROP-now / OCR〔須先裝 Tesseract〕/ denylist；勿當 S135 carry-decode 試）
   - **g10/g19 SEN 真資料缺口 ingest**（fetch→驗冇 mojibake→page-carry→索引）
6. Risks / blockers:
   - 🚧 **OCR blocker**：Tesseract 未裝；phys OCR 要 Leonard 先 `brew install tesseract tesseract-lang`。
   - 🔴 phys mojibake = glyph-index、**不可 decode 復原**（勿當 S135 carry-decode）；wiki_chunks 欄名 `text` 非 `content`。
   - 🔴 「sen」短英文 query 用戶仍見 phys 亂碼（未修；SEN route 或 phys 清理任一都解決呢 surface）。
   - 既有不變: 🔴 57014 transient (retry); FAIL-A (record-only); §E.10 (a) ACCEPTED conditional; q.html/A·AB code path dormant 勿清; Q4 deferred 未明示勿掂; Stage-2 closed; egress 每次自測。
7. **✅ 純診斷 session、0 code/data/Supabase mutation。** 兩件資料質素 backlog 已完整診斷（含 adversarial verify + sibling audit）、執行 PLAN ready；兩個執行 fork Leonard 收工待下次決。起手自測全 PASS（HEAD 356e810==origin/main / facts 455 / onrender warm / CORS policychecker ACAO / Supabase **9,849**）。無 commit（純診斷 + doc closeout）。
