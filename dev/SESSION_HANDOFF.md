...
## Open Priorities
> **🔄 S169（2026-06-15）①②③ 已 ship + push，待 QC agent 覆核：** ① 文件標註乾淨成品版「保留格式」（Word 用 JSZip 改原 docx XML 保留標題/編號/格式、建議融入正文無螢光無追蹤修訂）+「改動摘要」下載（commit `2150f59`，Node builder 32/0）；② onboarding 首訪自動彈 6 步導覽 + header「🎓 使用教學」重溫（commit `12f33fb`）；③ 平台介紹 in-app 使用手冊 + FAQ + 修正 stale 文件標註 channel 描述（commit `12f33fb`）。**全部純前端 app.html、headless 驗、零接觸 backend/Supabase/凍結合約。** **④ EDB 全入庫 = DEFERRED**（唯讀 freshness crawl 啟動後 Leonard 轉 QC，已停、零 Supabase 接觸；QC pass 後逐源 INSPECT + 逐源確認 live INSERT）。**版本未 bump（仍 3.1.0）；①②③(±④) 是否 = v3.2.0 待 Leonard 決定 + QC pass 後改 PLATFORM_VERSION/README/CHANGELOG。** **HEAD == origin/main `12f33fb`。**
...
4. Pending（待 QC pass 後）: **① QC agent 覆核 ①②③（headless fresh/bypass 快取）**；② 版本決定 ①②③(±④)=v3.2.0 → bump PLATFORM_VERSION/README/CHANGELOG；③ **④ EDB 全入庫**（deferred）逐源 INSPECT + 逐源確認 live INSERT。
...
7. commits（**已 push origin/main**）: `2150f59`(① app.html) → `12f33fb`(②③ app.html) → 本治理 commit。
