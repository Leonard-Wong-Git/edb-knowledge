...
## Open Priorities
> 產品方向：**全棧 Channel-B-only**。Channel A frozen @455。**S160：政策範本下載 + 文件修訂 兩個 tab 已 push + LIVE deployed（Pages+Render；live e2e PASS：14 域 / checklist-revise 真實報告 / CORS 通）。HEAD==origin/main（已 push）。** 主線 0 outstanding bug。

1. **#2 幼稚園 Phase 2 — KG live 入庫（DEFERRED，prep DONE，Leonard supervised）**：S160 嘗試 live INSERT 被 harness auto-classifier 拒（「除幼稚園外」+ prior「supervised fresh session」）→ 尊重、未做。prep 100% ready：`python3 dev/ingest_one_source.py kg_admin_guide_2026 && python3 dev/ingest_one_source.py kg_operation_manual_2026`（先 --dry-run；218+217=**435 chunks**、page-resolvable、兩源 live 0 rows、dry-run 重驗綠）。之後 KG route（searchChannelB.ts SOURCE_SETS+TOPIC_KEYWORDS）+ registry + **display-sync 7 點**（14,674→~15,109）+ routed smoke + KG 清單 pilot。⚠️ display drift byte-identical；INSPECT before。
2. **#3 學校版分校類 — 待 Leonard review docx（monitor）**：102 docx 喺 `dev/checklists/<域>/`（**現已 live、可由「政策範本」tab 下載**）。反饋要改格式/分流 → 調生成器（`gen_school_docx.js`/`gen_checklist_docx.js` `<type>` arg）或 tags 一鍵重生（**改 tags 後跑 `apply_school_types.py --check`**；改 docx 後 re-run `gen_templates_manifest.py` + `gen_checklists_bundle.py` 再 push）。
...
4. **既有 monitor**：IMC grouped 頁碼只對第一份 PDF 準 / `cgss_2024` routed rank 低 / 57014 cold-start / `stats.sources`=120 cosmetic-stale / S153 synthesis ~328 字 / Azure OpenAI fallback / 自動發現+freshness 週跑（detection-only）。
...
3. Completed（詳見 SESSION_LOG S160）:
   ...
   - ✅ **P2 政策範本下載 tab**（`fcccc34`，**LIVE deployed**）：app.html +TemplatesPanel +'templates' tab；`policy_templates.json` manifest（14 域 102 docx；`gen_templates_manifest.py` 生成）；連現有 live docx（`.nojekyll`，HEAD-200 驗）；browser-verify 102 連結 / 校類 filter（小學→26）/ fetch 200。
   - ✅ **P3 文件修訂 feature**（`bd99b91`，**LIVE deployed**）：backend `checklistRevise.ts`（embedding coverage + clause supplement，無 LLM）+ `/api/checklist-revise` + `/api/checklist-domains`；`checklists_bundle.json`（14 域 2944 items/1282 clauses，root，backend runtime 載入）；app.html +ReviewPanel +'review' tab +buildRevisedDocx。
   - ✅ **Deploy + live e2e**（Leonard「push 及上線」）：`git push` → Pages+Render auto-deploy。live：`/api/checklist-domains`=14 域、`/api/checklist-revise` 真實報告（safety docx→covered/partial/missing）、live app.html 含 15 新功能 markers、policy_templates.json=102、CORS echo policychecker。
4. Pending: **#2 KG ingestion**（Leonard supervised）；#3 docx review（現 live）；文件修訂 Phase 2.5（門檻 live-tune / LLM 覆核 / mobile shell）。
5. Next priorities: #2 KG ingestion → #3 review → 文件修訂 Phase 2.5。
6. Risks: 🟢 **HEAD==origin/main（已 push + LIVE deploy）**。Supabase 14,674 未動。覆蓋門檻係估算（tunable，建議 live 觀察真實文件再 tune）；`checklists_bundle.json` 公開於 root（EDB 衍生、benign）；新 backend route = additive（現有 route 零回歸）。
7. commits（**已 push origin/main + deployed**）: `fcccc34`(P2 政策範本)→`bd99b91`(P3 文件修訂)→收工 gov→deploy 後 gov。
   - QC：P2 browser-verify 全綠；P3 backend python e2e（match 198/10/0 vs unrelated 1/5/202、400 err）+ tsc check/build + frontend browser stub（render/filter/docx PK 8365B）+ **onrender live e2e（14 域 / 真實 revise 報告 / CORS）**。
