...
## Open Priorities
> 產品方向：**全棧 Channel-B-only**。Channel A frozen @455。S154 文件分析 Phase 1 + S159 Phase 2 已 ship。**主線 = #2 幼稚園 Phase 2 KG live 入庫（prep 已備，下個 session 執行）。** #3 學校版分校類 + #4 文件分析 Phase 2 = S159 完成。主線 **0 outstanding bug**。

1. **#2 幼稚園 Phase 2 — KG live 入庫（NEW 主線，prep DONE，待執行）**：S159 研究 + 抽取完成。**2 核心源已抽取待 INSERT**：`kg_admin_guide_2026`（幼稚園行政手冊2026.5，143頁，dry-run **218 chunks**）+ `kg_operation_manual_2026`（學前機構辦學手冊2026.5 v4.3，175頁，**217 chunks**）；共 **435 新 chunks**、U+FFFD=0、page-resolvable、`extract_*.txt` 已寫喺 `dev/vault/<id>/`、`ingest_one_source.py --dry-run` 驗過。**執行**：①`python3 dev/ingest_one_source.py <id>`（live INSERT，upsert-safe，先 --dry-run；key 喺 backend/.env）②加 KG route（searchChannelB.ts SOURCE_SETS+TOPIC_KEYWORDS）③registry entry ④**display-sync 7 點**（14,674→~15,109）⑤routed smoke + live count ⑥起 KG 清單 pilot（建議 conduct/governance 域，用新行政手冊，似 14 域 checklist build）。其餘 17 KG 源候選見 SESSION_LOG（qa/curriculum 等）。⚠️ display drift 係 recurring gotcha、byte-identical；live INSERT 前 INSPECT。
2. **#3 學校版分校類 — S159 完成、待 Leonard review（monitor）**：13 域 per-type docx **102 份**喺 `dev/checklists/<域>/`（generic + 小/中/特；placement/gifted 只小+中）。13 域 **348 carve-outs**（`school_types` field；`_school_type_tags.json`）。生成器 `dev/checklists/_work/gen_school_docx.js`+`gen_checklist_docx.js`（`<type>` arg）；`apply_school_types.py`（**改 tags 後必跑 --check 驗 locator**）/`gen_all.py`。已修全 14 域學校版「undefined」章節 bug。Leonard 科目→校類 ruling 已套（視藝=中小、家政/實驗室/工場=中+特）。**Review 反饋若要改格式/分流 → 調生成器或 tags 一鍵重生。** Caveat：混合 clause（safety 3+curriculum 1，核心通用 keep-shared）小學版見零星設施句 = 已接受（徹底乾淨要 per-type 文字變體，較大工程）。
3. **#4 文件分析 Phase 2 — S159 SHIPPED LIVE（follow-up = Phase 2.5）**：標註版原文 docx 下載 + 校類選擇器已 live（`b503784`，onrender e2e PASS）。校類整合 = advice-level（notes/標註按校類）。**Phase 2.5 follow-up**：深度 source-level 校類 filter（matches 按校類過濾，需 source→school_type map，較大）。
3. **S158 ph_pri re-ingest monitor**：`ph_pri_guide_2025` 已完整重抽（315 chunk、物理頁 1-262 page-resolvable）；curriculum 8 條人文科引用用**物理頁碼**（非印刷「5-1」頁標）—同站其他源一致。舊抽取封頂 bug = 一次性已修；其他源若疑抽漏，對真 PDF 頁數核實（教訓：extract 可被 `--pages` 靜默封頂）。
4. **自動發現 + freshness 週跑（detection-only）**：`discover_sources.py`（週一 10:00 UTC）/ freshness（週一 09:00）續 diff。
5. **既有 deferred / monitor-only**：IMC grouped 源（`imc_governance_supplements`/`imc_election_guides`）連續頁碼只對第一份 PDF 準 / `cgss_2024` routed rank 低 / 57014 cold-start / `stats.sources`=120 cosmetic-stale（live ~216）/ S153 synthesis ~328 字略過 300 soft cap / Azure OpenAI fallback（llmClient+embeddingClient swap）。
...
2. Session ID: Claude_20260613_S159 (S159)
3. Completed（詳見 SESSION_LOG S159）:
   - ✅ **#3 學校版分校類 mass-gen** — 13 域 per-type(小/中/特) docx **102 份**（dev/checklists/<域>/）+ 修全 14 域學校版 docx「undefined」章節 bug（gen_school_docx 讀 ch.section_no→ch.si+checklist sections[si-1].name；S156/S157 QC 漏覆蓋編號）+ 13 域 **348 校類 carve-outs**（school_types field；9 域背景 Workflow tagging + safety/curriculum facility carve-outs）+ Leonard 科目→校類 ruling（視藝=中小、家政/實驗室/工場=中+特）。生成器/腳本 dev/checklists/_work/（gen_*.js <type> arg / apply_school_types.py / gen_all.py）。
   - ✅ **#4 文件分析 Phase 2 SHIPPED LIVE** — 標註版原文 docx 下載（client-side docx UMD，不上載）+ 校類選擇器（analyzeDocument.ts +school_type+text；app.html）。onrender e2e PASS。
   - ✅ **#2 KG 研究 + 入庫 prep** — 19 源 WebFetch 核實；2 核心源抽取（435 chunks 待 INSERT，dev/vault/）。live 入庫留下個 session。
4. Pending（主線 = Open Priorities #1）: **#2 KG live 入庫**（435 chunks ready：INSERT + route + registry + display-sync 7 點 + KG 清單 pilot）；#3 Leonard docx review；#4 Phase 2.5（深度 source-level 校類 filter）。
5. Next priorities: #2 KG live 入庫 → KG 清單 pilot → #3 review 反饋。
6. Risks: 🟢 0 outstanding bug。Product/Supabase(14,674)/live 本 session #3 零接觸；#4 已 deploy live(additive)。入庫紀律：display drift recurring gotcha 要 byte-identical、live INSERT 前 INSPECT。
7. commits: `b1d4f5c`(#3 docx)→`97c0356`(#3 gov)→`b503784`(#4 live)→收工 gov commit（全 push）。
   - 〔S158 摘要：ph_pri_guide_2025 完整重抽 146→315(總14,674) + curriculum 人文科 8/9 re-anchor + 培訓證書清理 + #3 地基(profiling+4/13 tagging)。詳見 SESSION_LOG S158。〕
