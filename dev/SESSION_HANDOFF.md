...
1. **文件標註 Phase 2 / 2.5（接 S161 Phase 1）**：Phase 2 = PDF inline highlight（pdf-lib + pdf.js 座標，難）；Phase 2.5 = per-segment `detectQueryCategory` 自動偵測範疇取代手動 multi-select（現 auto-detect 用 domain 描述子 embedding 相似度、cap 2）。**先等 Leonard 試用 Phase 1 反饋再定** —— 重點睇真實學校 docx 嘅段落↔XML mapping 命中率（v1 段落級 highlight，碎 run/表格多嘅檔可能多入附錄）+ partial/missing 門檻噪音。
...
3. **#2 KG 收尾**：`kg_admin` route 已 live（S161 起手探針確認，S160 嘅 Render stuck 已自解）。剩 +幼稚園清單 pilot（用新 2 源 kg_admin_guide_2026 / kg_operation_manual_2026 起 KG checklist→docx→入政策範本 manifest，似 14 域 pipeline）。
