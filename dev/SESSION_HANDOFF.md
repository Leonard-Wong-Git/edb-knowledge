...
1. **⚠️ 站搬遷 GitHub Pages → Cloudflare Pages（最優先，進行中）**：**根因確定** = Leonard 將 repo set private，**GitHub Pages 免費 plan 唔支援 private repo → 自動 unpublish 全站 404**（DNS/CNAME 全對，純 plan 限制；GitHub Pages 全球 operational）。**Leonard 決定：保持 private + 搬 host（Cloudflare Pages，免費支援 private repo + 保留域名）。** 步驟：CF dashboard→Pages→Connect Git(private repo)→build preset None/output `/`→加 custom domain policychecker.wongfu.net→去**註冊商(orderbox-dns)** DNS 面板改 `policychecker` CNAME 由 leonard-wong-git.github.io→`<project>.pages.dev`。**Repo 唔使改**（靜態，CF 直 serve；.nojekyll/CNAME 留住無妨）。**Backend CORS 已 baseline-allow policychecker.wongfu.net**（env.ts BASELINE_CORS_ORIGINS）→ 切域名即 work；想 *.pages.dev preview 試搜尋則 Render env CORS_ORIGIN 加該 origin（免改 code）。搞掂後驗 app.html v3.0.0 + 搜尋 + 標註 + 0 console err。Render backend（P2/P3）一直 LIVE 不受影響。
