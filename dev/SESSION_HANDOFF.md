...
1. **文件標註相關性收緊 — Leonard 揀 A，已實施 LIVE（`9b1f6a5`）**：auto-detect 改揀**單一最相關範疇**（was 2）+ 門檻 0.30→0.38（`detectRelevantDomains`）+ auto 模式 missing cap 8（was 25）。實證：小學數學課程 doc auto-detect 只出 `curriculum` 課程管理、**零 SEN/跨範疇**。✅ **跨校類殘留 — S162 FIXED（待 push/部署）**：`_school_type_profiles.json` 逐域 `applies_to` → bundle 加 **domain-level `school_types`** → backend `okType` precedence（clause tag → domain scope → all）+ `detectRelevantDomains` 加 sel 過濾。untagged clause 唔再跨校類漏出（live e2e A-D + regression 全 PASS）；一併修正 placement/gifted/sen/cpd/qa/school_governance/kg_admission 同類洩漏。
...
2. ✅ **dead code cleanup — S162 DONE（待 push/部署）**：移除 `AnalyzePanel`/`ReviewPanel`/`buildAnnotatedDocx`/`buildRevisedDocx`（app.html 4345→3715 行）；保留 live `REVISE_SCHOOL_OPTS`/`TEMPLATE_TYPE_FILTERS`/`TemplatesPanel`/AnnotatePanel；`REVISE_STATUS_META`/`REVISE_BACKEND_URL`/`ANALYZE_SCHOOL_LABELS` 變孤兒小 const、無害保留。browser-verify 5 tab + 0 console error。chip task_5dc04973 done。
