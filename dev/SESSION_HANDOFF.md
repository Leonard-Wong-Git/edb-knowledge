...
## Current Baseline
1. **Version / git**: v2.3.0（knowledge 凍結 @2.3.0、guidelines @2.5.0，無 bump）；git `main`=`origin/main` HEAD = **S159 governance commit（deliverables `b1d4f5c` 之後；已 push）**；起手自行 verify HEAD==origin/main + Supabase **14,674**。**S159（2026-06-13）：#3 學校版分校類 mass-gen — 13 域 per-type(小/中/特) docx 102 份入 `dev/checklists/<域>/`；修全 14 域學校版 docx「undefined」章節 bug；13 域 348 校類 carve-outs（school_types field）；Leonard 科目→校類 ruling 已套。Product/Supabase/live 零接觸。****S157：checklist QC 全清（128 verify_issues、0 pending）。S158（2026-06-13）：(1) `ph_pri_guide_2025` 完整重抽入 Supabase 146→315 chunk（舊抽取封頂 80/262 頁、缺 ch3-6 正文；DELETE 146 舊 + INSERT 315 新）→ 總 14,505→**14,674**、公開 Channel B 搜尋實測命中 ch5/ch6；(2) curriculum 人文科 8/9 verify-issue 引用由 EDB 通告 re-anchor 去真指引 `ph_pri_guide_2025` 真物理頁（p8/11/19/123/127/192/196/245；idx7 培訓 刪），2 份 curriculum docx 重生；(3) 刪 人文科 idx7 + 科學科 4 條 30hr/15hr 培訓證書 items（行政公布非校本政策要求）；(4) display sync 14,505→14,674 × 7 處。**
...
## Open Priorities
> 產品方向：**全棧 Channel-B-only**。Channel A frozen @455。S154 文件分析 Phase 1 已 ship。**主線 = #3 學校版分校類（S159 mass-gen 完成，待 Leonard review）+ #4 文件分析 Phase 2。** 主線 **0 outstanding bug**。

1. **#3 學校版分校類（per-type 獨立可下載文件）— S159 MASS-GEN 完成、待 Leonard review**：13 域 per-type docx 共 **102 份**已生成喺 `dev/checklists/<域>/`（generic + 小/中/特；placement/gifted 只小+中）。tagging：13 域 **348 carve-outs**（`school_types` field 入 checklist/clauses.json，`_school_type_tags.json` 記錄）。生成器 = `dev/checklists/_work/gen_school_docx.js` + `gen_checklist_docx.js`（`<type>` CLI arg、按 school_types filter、無 field=共用）；流程腳本 `apply_school_types.py`（--check/--apply，改 tags 後必跑驗 locator）+ `gen_all.py`（mass-gen driver）。**已修全 14 域學校版 docx 之前「undefined」章節 bug**。Leonard **科目→校類 ruling** 已套：視藝(一般)=中小、視藝酸類=中、家政/科技與生活/實驗室/工場=中+特（safety/curriculum facility carve-outs）。**Remaining**：①Leonard 開 docx review 格式/分流，有改再調生成器或 tags；②**幼稚園 Phase 2**（kg_admission 要補 KG 專屬源先 per-type；KG governance 屬另一法律框架）；③混合 clause（safety 3+curriculum 1，核心通用故 keep-shared）小學版仍見零星設施句 = 已接受 caveat，徹底乾淨要 per-type 文字變體（較大工程）。
2. **#4 文件分析 Phase 2（掃描學校文件→修改補充→可下載標註）**：Leonard 要用 agent team 做。接 S154 Phase 1（上載→比對指引→逐段報告）。補充內容**要校類正確 → 食 #3 校類模型**，故排 #3 之後。
...
2. Session ID: Claude_20260613_S158 (S158)
3. Completed:
   - ✅ **[LIVE Supabase 重抽] `ph_pri_guide_2025` 完整重抽** — 揭發舊抽取封頂 80/262 頁、缺 ch4.7課業/ch5評估/ch6學與教資源 正文（agent 二輪疑「庫缺」→ 親 fetch 真 PDF 證實 262 頁、prev extract `--pages` 封頂）。流程：fetch_extract 全抽（262頁/190k字/U+FFFD=0）→ dry-run 315 chunk → pre-flight id 集（old 146/new 315/overlap 0）→ **INSERT 315 → DELETE 146 stale** → 驗 ph_pri=315、總 **14,674** → all_chunks.json cache 刷新 → live 搜尋實測命中 ch5 p193。
   - ✅ **[curriculum 人文科 re-anchor 8/9]** — 9 條 verify-issue 引用原指 EDB 通告 `edbc12_2025_ph_pri`（無頁/approx）；2 輪 agent（先 80頁誤判 5/9，重抽後完整 262 頁）→ 8 連真指引真物理頁（idx0→p8/1→127/2→123/3→192/4→245生成式AI/5→19/6→11/8→196）；idx7 培訓=刪。docx rels 驗連結正確。
   - ✅ **[培訓證書清理]** 刪 人文科 idx7 + 科學科 4 條（30hr/15hr 證書 items，行政公布非政策要求）；clause9 表格 4→1 行（留一般培訓）、clause29 trim 核心句；兩 docx 0「培訓證書」；清單 634→629。
   - ✅ **[display sync]** 14,505→14,674 × 7 處（assertion 防誤改：3 _meta.stats + app/index/K1_API_SPEC/README）。
   - ✅ **[#3 地基]** 14 域校類 profiling（`_school_type_profiles.json`）+ per-type tagging **4/13**（`_school_type_tags.json`）— 餘 9 域撞 session 上限未完。
   - ✅ **Git commit `6a37e9c` + push**（15 檔）。
4. Pending: **#3 resume**（9 域 tagging + 生成器 filter + pilot + mass-generate，見 Open Priorities #1）；#4 文件分析 Phase 2。
5. Next priorities: #3 per-type 文件（小/中/特）→ #4 Phase 2。
6. Risks: 🟢 0 outstanding bug。ph_pri 物理頁碼非印刷頁標（與全站一致）。#3 tagging 9 域 resume 用 Workflow wf_d9e8cf6a-6d5。
7. commits: HEAD = `6a37e9c`（S158 closeout，已 push）。
