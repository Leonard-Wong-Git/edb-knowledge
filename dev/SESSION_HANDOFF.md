...
2. Session ID: Claude_20260615_S169 — 「開工」→ AskUserQuestion 選「1-4」＝ ①②③④ 全做 batch。①②③ ship+push+headless 驗；④ 啟動唯讀 freshness crawl 後 Leonard 轉交 QC agent → ④ deferred、crawl 已停。
3. Completed（詳見 SESSION_LOG S169）:
   - ✅ **①**（`2150f59`，app.html）文件標註乾淨成品版「保留格式」+「改動摘要」：新 `buildCleanOriginalDocx`（Word 用 JSZip 改原 `document.xml`，原段落不動保留標題/編號/格式，建議插普通「（建議補充）」段落、**無 w:highlight/無 w:ins**，說明入附錄）；`handleDownloadClean` 按 `isDocx&&fileBuffer` 路由（PDF/貼文字維持 from-text）；新 `buildChangeSummaryDocx` + 「📋 下載改動摘要」掣。抽共用 `partitionCleanFindings`。**QC：Node 抽真源 builder 單元測試 32/0 PASS（docx 8.5.0/jszip 3.10.1）。**
   - ✅ **②**（`12f33fb`，app.html）onboarding：`Onboarding` component + `TOUR_STEPS` 6 步，首訪 `localStorage`（`k1_tour_done_v1`）自動彈、header「🎓 使用教學」重溫。QC：headless dump 驗首訪自動彈 + babel boot。
   - ✅ **③**（`12f33fb`，app.html）使用手冊：`PlatformIntroPanel` in-app 手冊（5 功能 details + FAQ）+ tour 完成 CTA 指向；**doc-drift 修正** 文件標註 channel 描述（stale 螢光/追蹤修訂 → 乾淨成品版）。QC：about-tab dump 驗手冊+FAQ render + regression 確認 stale 文案消失。
4. Pending: ✅ QC 覆核完成（PASS-with-flags，no blocker）；✅ ⑤ dead-code cleanup 完成（`967dd7d`，resolve QC flag 2）；✅ 版本決定（Leonard：等埋 ④ 一齊 = v3.2.0）。**淨低：① ④ EDB 全入庫（deferred）逐源 INSPECT + 逐源確認 live INSERT；② ④ 完成後一齊 bump v3.2.0（PLATFORM_VERSION/README/CHANGELOG）；③ Leonard 真機/真檔收貨 ①②③（封版前）。**
5. Next priorities: 見 Open Priorities 頂部 S169 區塊。
6. Risks: 🟢 HEAD==origin/main `12f33fb`（已 push）。🟢 ①②③ 純前端、Node 32/0 + headless boot 驗、零接觸 backend/Supabase 15,127/凍結合約（_meta 2.3.0·facts 455·guidelines 152）。⚠️ ①②③ 未經真機/真檔端到端收貨（下載需先有 analyze report；建議 QC 上載真 Word 驗保留格式 + 表格內段落命中）。⚠️ mobile onboarding 未做（desktop 已有）。⚠️ 版本未 bump（live 功能領先版本號，待版本決定）。⚠️ Render free-tier cold-start ~50s。Supabase 15,127 零接觸。
7. commits（**已 push origin/main**）: `2150f59`(① app.html) → `12f33fb`(②③ app.html) → `ffc8b97`(治理) → `20fd2a7`(QC 記錄) → `967dd7d`(⑤ dead-code cleanup −464)。
   - QC：Node builder 單元測試 32/0 PASS（buildCleanOriginalDocx 保留 pStyle/標題/正文·零螢光·零 w:ins·建議融入·附錄 before sectPr·單一 body·edge；buildChangeSummaryDocx 分段計數；buildCleanDocx from-text regression）；headless Chrome `--dump-dom`（fresh user-data-dir）驗 app.html boot（tabs render=babel OK）+ 首訪 tour 自動彈 + about-tab 手冊 render + stale channel 文案 regression 消失。**未做：真機/真檔端到端、真表格 docx、跨 Word viewer 顯示——留 QC agent / Leonard 收貨。**
