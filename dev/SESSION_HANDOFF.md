...
1. **Optional content refresh remainder（low ROI 收窄）**：S128 揭 14 sources EDB-content-updated 入面 — **3 PDF marker-bearing S129 done + 4 stat xlsx vault S130 done**；剩 **5 HTML index**（stat_edb_figures / arts_curr_docs / ph_pri_curr / edbc197_2024_ph_pri / moral_civic_curr）catalogue-level fetch（無頁、低 retrieval ROI 結構天花板）+ **stat_fact 21 chunks 仍 cite 2024/25 wording**（S130 vault-only refresh scope；stat_fact 升 2025/26 屬 future backlog — 需 build_stat_facts.py 4 builder rewrite + stat_facts.json rebuild + separate Supabase replace path）。唔急。
...
3. **🔴 既有 deferred + batch ranking polish backlog**：§E.10 admin-login client-side gate（**S131 ACCEPTED + DOCUMENTED**，conditional on cosmetic-gate design unchanged — 拆掉 client-side-only 前提即須 reopen）；57014 transient（retry 即恢復）；FAIL-A 注入 regression（record-only）；P2/P3（39→148 deferred）；Mobile UI P2；HKEAA；doc-debt；batch ranking polish ~15-17 sources（S122-S125c 累計）。
