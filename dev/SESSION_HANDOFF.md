...
🔴 **S228 新增的一條硬紀律（所有檢索改動都適用）**：`VAULT_LEAD_SCORE` 0.70、`SPOTLIGHT_LEAD_SCORE` 0.60、`min_score` 全部是在「查詢帶展開詞」的分數尺上校出來的。**任何會動搖絕對分數的檢索改動，都必須連同這些門檻一起重新校**，否則檢索兩層都報喜而答案層變差 —— 而兩層檢索指標都不會顯示這件事。S228 就是這樣把一個 +7 的改動量成不出貨。
