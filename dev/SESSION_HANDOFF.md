...
## Current Baseline
1. **Version / git**: v2.3.0（knowledge 凍結 @2.3.0、guidelines @2.5.0，無 bump）；git `main`=`origin/main` HEAD = **`6a37e9c`（S158 closeout）**；起手自行 verify HEAD==origin/main + Supabase **14,674**。**S157：checklist QC 全清（128 verify_issues、0 pending）。S158（2026-06-13）：(1) `ph_pri_guide_2025` 完整重抽入 Supabase 146→315 chunk（舊抽取封頂 80/262 頁、缺 ch3-6 正文；DELETE 146 舊 + INSERT 315 新）→ 總 14,505→**14,674**、公開 Channel B 搜尋實測命中 ch5/ch6；(2) curriculum 人文科 8/9 verify-issue 引用由 EDB 通告 re-anchor 去真指引 `ph_pri_guide_2025` 真物理頁（p8/11/19/123/127/192/196/245；idx7 培訓 刪），2 份 curriculum docx 重生；(3) 刪 人文科 idx7 + 科學科 4 條 30hr/15hr 培訓證書 items（行政公布非校本政策要求）；(4) display sync 14,505→14,674 × 7 處。**
...
## Open Priorities
> 產品方向：**全棧 Channel-B-only**。Channel A frozen @455。S154 文件分析 Phase 1 已 ship。**S158 主線新方向 = #3 學校版分校類（Leonard 定案）+ #4 文件分析 Phase 2。** 主線 **0 outstanding bug**。

1. **#3 學校版分校類（per-type 獨立可下載文件）— IN PROGRESS（S158 開）**：Leonard 定案「每域 4 份獨立文件（幼/小/中/特），但有共用」。模型 = 每條要求標 `applies_to` 校類 → 生成器按校類 filter → 各校類獨立 docx（共用條 + 該類專屬）。**幼稚園 Phase 2**（要補 KG 專屬源；KG governance 等屬另一法律框架）；Phase 1 = 小/中/特。**進度**：14 域 profiling 完成（`dev/checklists/_school_type_profiles.json`，9/14 most-shared；幼稚園只適用 8/14 域）；per-type tagging **4/13 完成**（`dev/checklists/_school_type_tags.json`：school_governance/activity/safety/gov_admin，揪好 item/clause exceptions）。**Resume**：①跑餘 9 域 tagging（placement/sen/curriculum/gifted/conduct/qa_inspection/hr_admin/student_support/cpd；Workflow resumeFromRunId `wf_d9e8cf6a-6d5` → cached 4 + 跑 9）②改 `gen_checklist_docx.js`+`gen_school_docx.js` 加校類 filter（item/clause 加 `school_types` field、無 field=共用）③pilot 一域（建議 placement，小/中 split 清楚）畀 Leonard 睇格式 ④mass-generate 小/中/特（~37 files）。
