...
1. **CB-3 B-2 生產落地（GATED — 等 Leonard 睇 B-1 結果 + 明示，next 問 Leonard）**：B-1 已量度（39 標記源 100% chunk 帶頁、+1017、74 無標記源 byte-identical PASS、inert 未動 Supabase）。B-2＝re-embed 39 源變動 chunk（OpenAI）+ blast-radius dry-run + 授權後 Supabase DELETE-by-source_id 替換 + 重傳 + live smoke（§E.7/§E.13/§5、不可逆，bare「確認」非 go）。ceiling：B 救唔到無標記高流量源（sag/g06/g26）→ 需 Option C。
2. **Phase 1 Channel-B-only 渲染 Leonard browser-verify（GitHub Pages，next 問 Leonard）**：靜態 QC 全 PASS，渲染未驗（CDN SPA 依 §D.7 交 Leonard）。確認 OK / 要否調整。
2b. **Option C（CB-3 全覆蓋，B 之後 Leonard 排）**：74 無標記源（含 sag_2025_11/g06/g04/g26/g25 高流量、Leonard 測試 query 命中）重抽取頁標記 + 重 build + 重傳；達北極星全覆蓋唯一路。HTML-landing 源永無 `#page=N`（結構天花板）。
...
3. Completed:
   ...
   - ✅ **[PERSIST]** SESSION_LOG S119+CB3 block+verbatim、SESSION_HANDOFF baseline/OP重生/本 record、PROJECT_MASTER_SPEC §F/§B/§A/§C.4/§E.13/§D、CODEBASE_CONTEXT+Maintenance Log、DOC_SYNC、auto-memory；commit+push 指定檔（Phase1 + build_wiki_index.py inert）。
4. Pending:
   - Phase 1 渲染待 Leonard browser-verify（GitHub Pages）。**CB-3 B-2 生產落地 GATED**（re-embed 39 源 + Supabase DELETE-replace + 重傳 + live smoke）＝等 Leonard 睇 B-1 結果 + 明示，不可逆 §E.7/§E.13/§5、bare「確認」非 go。Option C（74 無標記源，B 之後）。Q4 deferred；Stage-2 closed。
5. Next priorities (max 3 — 詳見 Open Priorities)：
   - 問 Leonard：(a) Phase 1 browser-verify OK？ (b) 睇 B-1 結果→授權 CB-3 B-2 生產？定先 Option C？
...
7. **Session 進行中（非 closeout — Leonard 未表示「收工」）**：Phase 1 promote+QC+commit+push 完成；CB-3 Phase 2 診斷 + Option B B-1 完成（量度 PASS、build_wiki_index.py commit、inert）。**CB-3 B-2 生產 mutation GATED — 等 Leonard 睇 B-1 結果 + 明示授權**（bare「確認」非 go）；Phase 1 渲染待 browser-verify。
