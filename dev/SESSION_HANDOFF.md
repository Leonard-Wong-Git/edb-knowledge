...
2. Session ID: Claude_20260621_1200 (S174) — 「開工」探針全綠 → Leonard 指出 EDB 文件附件表格底細字（footnote）藏住正文無講嘅實質要求、值得做 source → 規劃+試找例子（agent fan-out 確認跨 13 文件 + 三流失機制：扁平埋藏/頁數截斷/摘要式抽取）→ Leonard /loop 自主「全部入庫 + 敵意 agent 攻擊到 98% 出報告」→ harvest 33 → staging → sim iterate（揭 overfit、held-out 校）→ 報告 → Leonard 批 A live 入庫（INSPECT+INSERT 15,363）→ live 複測揭路由盲點 → Leonard /loop=繼續 → 修 searchFootnotes 路由獨立 overlay → live 100% → 報告 → Leonard「A 收工」。
3. Completed（詳見 SESSION_LOG S174）:
   - ✅ **附件細字 footnote 入庫機制**：全庫 209 掃 footnote（1104→精煉 61→triage）→ 33 條策展入 Channel B（`content_type=footnote_curated`、15,330→**15,363**、INSPECT before/after、`id=footnote_*` 可逆、re-embed text+keywords）。
   - ✅ **路由獨立檢索修法**（`8f2cace`）：診斷=`searchWiki` RPC 後 `sourceIds` post-filter 丟 footnote + ivfflat probes=8 recall 盲點 → `wikiRepository.searchFootnotes`（exact-cosine overlay）+ `searchChannelB` footnote pass（強配對 ≥0.45 lead 入合成窗、best-effort try/catch）。WikiContentType +footnote_curated。
   - ✅ **敵意準確度 62%→75.8%→live 100%**（held-out 33 全新口語 query）；synthesize 證原 hallucination（代課批准亂作「30日」）修正為真「6個月/大多數校董」。
   - ✅ display-sync 15,363 ×8 + CHANGELOG（無 PLATFORM_VERSION bump、凍結合約 `_meta` 2.3.0·facts 455·guidelines 158 零接觸）；headless app.html 驗綠。
4. Pending: 見 Open Priorities 🔜 NEXT（① footnote 擴充 ~28 候選〔待 Leonard〕；既有 monitors）。
5. Next priorities: footnote 擴充(待定) → 文件標註精準度 monitor → EDB monitor-driven 入庫。
6. Risks: 🟢 HEAD==origin/main `9b3d8f9`（已 push、tree clean、0 outstanding bug）。🟢 凍結合約零接觸、Supabase 15,363。⚠️ footnote 用 backend in-memory cache —— re-ingest footnote 後要 **restart Render** 先 reload（`invalidateWikiCache`）。⚠️ Render free-tier cold-start ~50s。⚠️ OpenAI node-fetch（S173）Premature close 復發 → Azure fallback。
7. commits（已 push origin/main）: `8f2cace`(footnote overlay backend) → `9b3d8f9`(display-sync+staging+CHANGELOG) + 本 closeout commit。live Supabase INSERT 33 + upsert（Leonard 批 A、INSPECT 齊）。
