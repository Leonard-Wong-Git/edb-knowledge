...
3. Knowledge state: **455 Channel A facts** (三層同步 ✅ byte-identical；dedup 792→455 commit `711f911`), **0 candidates in queue**, **Supabase 10,133 chunks**（S125 post-batch-4；net -120 來自 10 batch-4 sources；DELETE 537 INSERT 417）。Vault: 120 sources 提取完成；39 marker via Option B + 3 pilot + 10 batch-1 + 10 batch-2 + 10 batch-3 + 10 batch-4 = **82 sources Supabase page-carried**；whole-vault page-resolvable **~76.0%**（~7,706/10,133）；剩 **21 marker-less PDFs**（batch-5~6）+ 9 結構天花板。**指引數字 4 層（148 app 內庫 / 39 公開 guidelines.json / 151 source_registry / 120 vault-extracted）見 PROJECT_MASTER_SPEC §B.1 釐清框 — 39 是否擴到 148 = OPEN DECISION，未收斂**。
...
## Open Priorities
> 產品方向：**搜尋介面 Channel-B-only**（S119 定，A/AB dormant；Q4 契約 deferred）。北極星＝合理＋指引＋**頁數**（CB-3 不可 defer）。Stage-2 closed 勿復活。**driver 4 輪 verified（S122~S125）；剩 21 marker-less PDFs**（batch-5~6）+ 9 結構天花板。**S125 新教訓 §8b promote candidate**：audit pre-flight 必 cross-check index 既有 stale-superseded 版本（S123/S125 兩度 surface）。
1. **broader Option C batch-5**（10 sources 候選：g29/g15/g24/edbc12_2025_ph_pri/pri_science×4/pri_science_cert_course_list/sci_jss_framework_2025）：pre-flight 全新 — §3 HIGH-risk Leonard 明示 go → pre-flight agent team（feasibility URL probe + **audit supersede chain + cross-check index stale-superseded S125 lesson** + monitor chunk delta）→ Gate 1 vault `--write`（先 +10 entries to repage_pdfs.py PILOT_LEGACY/PILOT_OUT）→ Gate 2 `--execute --skip-local` → QC + smoke。
2. **broader Option C batch-6**（11 sources 剩，視 batch-5 selection 重列）：edbcm183_2023_values_edu / sec_curr_guide_2017_booklet_6a / pe_sss_2023 + ~8 個。
3. **Freshness workflow triage（NEW S125）**：(a) 本地跑 `python3 dev/source/check_freshness.py --dry-run` 識別 N 條 fail URL；(b) 修 source_registry.json URL drift；(c) 改 `check_freshness.py` 失敗 threshold（建議 errors >5 才 exit 1、或改用 GitHub Issue 而非 fail email）。SESSION_HANDOFF Regression Notes #2 同步更新。Chronic 5 連 fail since 2026-04-30、非 blocker、ops noise。
4. **Audit cross-check rule promote (§8b S125)**：下次 batch-5 audit agent 必跑 cross-check 落 wiki_chunks index 是否有被 batch-5 候選 supersede 嘅 stale 版本仍 indexed（e.g. econ_sss_supp_2015 vs econ_sss_supp_2025 S125 觸發 ranking miss）；若有 = audit DROP candidate / schedule deprecation。
5. **batch ranking polish backlog（低優先）**：S122 batch-1 → tech_kla/ls_jss/chi_hist；S123 batch-2 → music_sss_2024；S124 batch-3 → 3 non-surface；S125 batch-4 → 6 non-surface（含 1 stale-superseded competition）。共 ~13 sources ranking/topic-routing 競爭。
...
7. **Q4 對外契約收斂（deferred）**：Channel A→knowledge.json→Circular System；3 選項待 CB-3 成熟；未明示勿掂。

> ✅ **S125 完成**：CB-3 Option C broader batch-4（10 sources）page-carry 生產 live；Supabase 10,253→10,133；whole-vault page-resolvable ~73.0%→~76.0%；82/113 sources marker-bearing；Freshness workflow chronic fail triaged (deferred to next session); audit cross-check §8b lesson surfaced。
...
2. Session ID: Claude_20260526_0737 (Session 125)
3. Completed:
   - ✅ **[Batch-4 PILOT entries +10 + Gate 1 + Gate 2 EXECUTE]** 10 sources（ict_sss_2021 / chi_hist_jss_ncs_2019 / chi_hist_jss_bilingual_2019 / econ_sss_2025 / econ_sss_supp_2025 / geog_sss_supp_2022 / geog_sss_summary_2022 / geog_sss_update_brief / arts_kla_guide_2017 / music_national_anthem_2024）：CHANGE step 0 `dev/vault/repage_pdfs.py` PILOT_LEGACY/PILOT_OUT 各 +10 batch-4 entries（純 dict extension、size 33→43、import verify）→ Gate 1 dry-run + EXECUTE 10/10 PASS（pages 65/28/19/84/41/13/9/14/106/4 total 383；content sanity 102.5% slight + marker overhead；backup `dev/init_backup/20260526_073931_UTC/cb3c_pilot_legacy/`；git scope 1 M + 10 D + 10 ?? 守）→ Gate 2 dry-run no anomaly（10 sources -14~-32% canonical normalization、無 outlier）→ Gate 2 EXECUTE 10/10 OK（Phase 1b embed 417 → wiki_index.json auto-backup `dev/init_backup/20260526_091916_UTC/` → DELETE 537 / INSERT 417 / net -120 / Supabase 10,253→**10,133**、per-source `del/ins/now` 全對齊；Phase 3 SKIPPED `--skip-local` §E.14 紀律）。
   - ✅ **[QC post-execute 4 PASS]** (1) Supabase total = 10,133 via Range header exact match prediction (2) INVARIANT 5 spot-check g01=32/sag_2025_11=383/chem_sss_2007_2018=172/eng_lit_guide_2023=633/music_sss_2024=69 全 unchanged (3) backend `/health` ok (4) Gate 1 markers==pages 全對 + raw REST inspect econ_sss_2025 chunk text 含 `=== Page N ===` markers verified live。
   - ✅ **[Live smoke 4/10 surface + 6/10 ranking competition non-regression]** ⭐ chi_hist_jss_ncs_2019 p=4/5 (0.572/0.569) / geog_sss_supp_2022 p=1 TOP-1 (0.559) / geog_sss_update_brief p=14 (0.546) / econ_sss_2025 p=6/9 (0.543/0.534 via econ_sss_supp_2025 query)。6 non-surface = ranking 競爭非 regression（ict_sss_2021 now=81 1× 57014 transient 換 query 恢復 / chi_hist_jss_bilingual_2019 chi_pri/chi_jss 撞 / **econ_sss_supp_2025 撞 econ_sss_supp_2015 superseded-in-index** = S123 audit miss pattern S125 復發 / geog_sss_summary_2022 generic / arts_kla_guide_2017 va_p1_s6 dominate / music_national_anthem_2024 music_p1_s6 dominate）。Live smoke parser self-fix（field `page` 非 `page_number`）。Supabase wiki_chunks 無 `page` column（42703）= backend `searchChannelB.ts` 從 `text` content extract markers organize response，infrastructure intact verified。
   - ✅ **[Mid-session Freshness workflow chronic-fail triage (read-only)]** Leonard 貼 GitHub Actions failure email → 揭發 `.github/workflows/freshness_check.yml` 5 連 fail since 2026-04-30（run #6-#10 schedule）。Root cause = `dev/source/check_freshness.py:141-142 if errors > 0: sys.exit(1)` + EDB intermittent + 15s timeout 即觸發 GitHub email。SESSION_HANDOFF Regression Notes #2「check_freshness.py Errors:0 Checked:145 ✅」係 stale baseline。Artifact 401 需 GitHub token、Leonard 揀 batch-4 EXECUTE 優先、freshness 列下次 session priority。**非 batch-4 觸發**（Supabase 未掂 + workflow 只 HEAD probe）。
   - ✅ **[§8b lesson promote candidate (S125)]** Audit cross-check rule: pre-flight audit 之前只 check batch 自己 supersede chain，未 cross-check 落 index 既有 stale-superseded 版本（e.g. econ_sss_supp_2015 vs econ_sss_supp_2025 = S125 ranking miss 真因；S123 music_sss_2015/va_sss_2015/ethics_relig_2007_2019 同 pattern）。Recurrence-prone 兩 session 觸發、multi-agent collab、單 batch 不可 fix → §8b promote threshold 達標，列下次 batch-5 必跑。
   - ✅ **[PERSIST]** SESSION_LOG S125 entry + DOC_SYNC matrix + verbatim handoff；SESSION_HANDOFF baseline #1/#3 + Open Priorities regen（demote S124）+ 本 record；CODEBASE_CONTEXT External Services + AI Maintenance Log +S125；HANDOFF_PACKAGE §2 chunks 10,253→10,133；commit+push 指定檔。
4. Pending：
   - **broader Option C batch-5（10 候選 sources）+ batch-6（11 剩）**：等 Leonard 排步伐；batch-5 audit agent 必跑 cross-check index stale-superseded rule（S125 §8b lesson）。
   - **Freshness workflow triage**（chronic fail、非 blocker、ops noise）。
   - PROJECT_MASTER_SPEC §D.16 batch-4 verified note + §8b audit cross-check rule codify（deferred to batch-5 closeout 一次過）。
5. Next priorities (max 3)：
   - **broader Option C batch-5 排**（pre-flight 全新 + S125 §8b cross-check rule）
   - **Freshness workflow triage**（5 連 chronic、ops cleanup）
   - 🔴 §E.10 admin-login client-side gate（OPEN）/ batch ranking polish backlog（13 sources）
6. Risks / blockers:
   - **§8b promote candidate (S125)**：audit cross-check 既有 index stale-superseded（S123/S125 兩度 surface、recurrence-prone）；下次 batch-5 必跑。
   - **§E.14 driver reuse 4th-validation**：`cb3_b2_pagecarry_migrate.py` 4 batch（40 sources）0 incident；pipeline production-ready confirmed。
   - **Freshness workflow chronic fail (NEW S125)**：低 blast radius（ops noise + SESSION_HANDOFF Regression #2 stale）；triage 列 priority #2。
   - local `wiki_index.json` vs Supabase 82 源 diverge（S125 後 72→82；Supabase query-authoritative；reconcile 低優先 backlog）。
   - 既有 risks：🔴 §E.10；🔴 57014 transient（retry 即恢復、本 session ict_sss_2021 撞 1 次 retry 恢復）；🔴 FAIL-A（record-only）；Q4 deferred；Stage-2 closed 勿復活；egress 每次自測；EDB PDF 永遠用 `url_primary`（§E.12）；路徑空格雙引號。
7. **Session 進行中**（待 Leonard「收工」或繼續其他 OP；§4 closeout 視 Leonard 表示而觸發；§4a check 待 closeout）— S125 batch-4 完成生產 live + freshness triaged + commit/push 指定檔 pending。
