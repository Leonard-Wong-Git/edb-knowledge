...
## Current Baseline
1. Version: **v2.3.0**；git `main`=`origin/main` HEAD = **S120 closeout commit（commit *後* HEAD 自然推進＝正常非 desync，§G.2；下次起手自行 verify，應 ≥ S120 closeout commit）**。**S120：CB-3 Option C pilot 完成生產 live** — 3 個高流量 marker-less PDF 源（sag_2025_11 + g06 + g26）re-extract with `=== Page N ===` markers + per-source Supabase replace (738→814 rows, total **10,606 → 10,682**)；新增 `dev/vault/repage_pdfs.py`（PyMuPDF page-by-page，§5.a-compliant backup convention）+ 沿用 driver `cb3_b2_pagecarry_migrate.py --only` 完整 reuse §E.14 pattern；4 QC + 5 live-smoke query 全 PASS（g26 q=「幼稚園收生」p=2/3/4 0.67-0.70、sag q=「學校行政手冊 校本管理」TOP-1 p=1 0.657、B-2 既有源 0 regression）；經 2 條 §3 deviation 安全修正（diff scan 自我糾正 char drop 比錯；Option A backup 移出 vault 避 rglob ghost）。Whole-vault page-resolvable trajectory：13.2% (pre-B) → 23.7% (post-B-2) → **32.2% (post-pilot C)**。剩 Option C broader（**61 marker-less PDFs**，pipeline 已 generalize-ready）+ 9 結構天花板（4 HTML + 5 xlsx）救唔到。**S119 closed：搜尋介面 Channel-B-only Phase 1 + Option B 全完成生產 live + Leonard browser-verify PASS**。**S118 closed：Stage-2 adaptive combo non-viable、PLAN-1b 4 dedicated route promote** — 詳見 Previous Session Records。**S117 closed：masking-defect FIXED**（`channel_b_status` discriminator）。**S116 closed：`backend/supabase/schema.sql` 改 text 變體 + plpgsql volatile + probes=8（生產 live，Leonard Dashboard 套用）**。⚠️ probes=8 *live* 仍未獨立 introspect；Supabase free-tier probes=8 偶發 `57014` statement-timeout 已知 transient（生產可用性、retry 即恢復）。Q4（Channel A→`knowledge.json`→Circular System 對外契約）deferred 獨立 track；Stage-2 closed-as-non-viable 勿復活。local `wiki_index.json` vs Supabase 對 39+3=42 源 diverge（Supabase query-authoritative；reconcile 低優先 backlog）。sag_2025_11 PDF EDB 已從 2025-11 更新到 2026-05；source_id 仍 `sag_2025_11`（freshness metadata backlog，非 blocker）。
...
3. Knowledge state: **455 Channel A facts** (三層同步 ✅ byte-identical；2026-05-16 dedup 由 792 → 455，移除 275 條跨角色完全重複 + 合併 36 組相近事實，commit `711f911`，reversible log `dev/DEDUP_LOG_2026-05-16.md`；早前 Session 102 已 1,001 → 792), **0 candidates in queue**, **Supabase 10,682 chunks**（S120 post-pilot-C，pre 10,606；+76 net 來自 3 pilot sources page-carry replace）。Vault: 120 sources 提取完成（local extract 文件 113，3 pilot 已整合至單 repaged 檔；39 marker via Option B + 3 marker via Option C pilot = 42 sources Supabase page-carried；剩 61 marker-less PDFs + 9 結構天花板）。**指引數字 4 層（148 app 內庫 / 39 公開 guidelines.json / 151 source_registry / 120 vault-extracted）見 PROJECT_MASTER_SPEC §B.1 釐清框 — 39 是否擴到 148 = OPEN DECISION，未收斂**。
...
1. **Option C broader（61 marker-less PDFs，問 Leonard 排步伐）**：pilot 3 sources 已 done + 生產 live + pipeline 驗證 generalize-ready（driver `cb3_b2_pagecarry_migrate.py` + `dev/vault/repage_pdfs.py` 兩條都 reusable，擴 PILOT_LEGACY/PILOT_OUT dict 即可）；建議分批（例：10 sources/批 × 6-7 批）每批仍 §3 HIGH-risk Leonard 明示 go；總 cost ~$0.05、總 +800-1500 chunks。**結構天花板**：4 HTML + 5 xlsx（共 9 源）救唔到（HTML 永無 `#page=N`、xlsx 無頁概念）→ CB-3 北極星全覆蓋上限 ≈ 88%（27+3 marker + 64 marker-less PDF / 113 vault）。
2. **CB-3 收尾 backlog（低優先，非生產影響）**：(a) local `wiki_index.json` ↔ Supabase reconcile（42 源 diverge，Supabase query-authoritative）；(b) build_wiki_index hash-dedup vs live 語料不齊（latent corpus-consistency）；(c) sag_2025_11 freshness metadata（2025-11→2026-05；EDB 已更新 PDF，source_id/title 仲舊；對外 contract 唔變、純 internal naming）；(d) g06 vs pri_curr_guide_2024 near-duplicate ranking polish（SOURCE_ALIASES dedup 或 boost）。
...
> ✅ **S120 完成**：CB-3 Option C **pilot 3 sources（sag_2025_11/g06/g26）page-carry 生產 live + 北極星端到端 verified**（g26 q=「幼稚園收生」p=2/3/4 0.67-0.70；sag q=「學校行政手冊 校本管理」TOP-1 p=1 0.657）。Pipeline generalize-ready 等 broader scope Leonard 排。
