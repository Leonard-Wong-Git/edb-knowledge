...
3. Knowledge state: **455 Channel A facts** (三層同步 ✅ byte-identical；dedup 792→455 commit `711f911`), **0 candidates in queue**, **Supabase 9,713 chunks**（**S132 Phase 3b** post-page-carry of 6 stale Vanilla-preserved sources via Leonard rule「未明寫『取代』→ KEEP + 照做 page-carry」audit by 6-parallel agent-team；DELETE 743 INSERT 646 net -169；prior baseline 9,882 post-S130）。Vault: 120 sources 提取完成；39 marker via Option B + 3 pilot + 10×5 batch-1~5 + 2 batch-6 page-carry + 3 batch-7 + **6 S132 Phase 3b** = **100 sources Supabase page-carried**；2 sources deprecated（pe_sss_2007_2015 / sci_jss_supp_2017，vault legacy 保留作 historical record）；0 stale Vanilla preserved（**S132 Phase 3b cleared all 6**）。Whole-vault page-resolvable **~85.0%**（~8,265/9,713）；CB-3 final ceiling **~88% 達成**。**指引數字 4 層（148 app 內庫 / 39 公開 guidelines.json / 151 source_registry / 120 vault-extracted）見 PROJECT_MASTER_SPEC §B.1 釐清框 — 39 是否擴到 148 = OPEN DECISION，未收斂**。
...
## Open Priorities
> 產品方向：**搜尋介面 Channel-B-only**（S119 定，A/AB dormant；Q4 契約 deferred）。北極星＝合理＋指引＋**頁數**（CB-3 不可 defer）。Stage-2 closed 勿復活。**CB-3 達 final ceiling ~88%**（**100/113 marker-bearing post-S132 Phase 3b**（6 stale Vanilla-preserved sources page-carried per Leonard rule「未明寫『取代』→ KEEP + 照做 page-carry」、agent-team 6-parallel audit verified 2 HIGH-conf KEEP_BOTH + 4 UNCLEAR-lean-KEEP）+ 2 deprecated + 9 結構天花板 + 2 others = 113）。**driver `cb3_b2_pagecarry_migrate.py` 10 輪 verified（S122~S132、65 sources 0 final incident, S132 §3 CHANGE divergence textbook execute NUL-byte defensive guard codified §8 regression）+ `cb3_deprecate_stale.py` 0 incident**。**S132：PolicyChecker brand launch + Phase 3b 6-stale page-carry closed**（custom domain `policychecker.wongfu.net` live + OG/iframe/embed-sample + 5-round brand unification + 6 sources DELETE 743 INSERT 646 net -169；Supabase 9,882→**9,713**）。
1. **Phase 3a batch ranking polish ~15-18 sources**（S122-S125c+S130 累計 + Phase 3b live smoke 未進一步揭新 ranking issue）：g29 KGECG-TC-2017 dominance / tech_kla / chi_hist / ls_jss / arts ranking competition / econ_sss_supp competition / 等。Case-by-case 診斷 dedicated route or query expansion or per-source quota。Fresh session 較佳（each source needs individual judgment）。
2. **Phase 3c 5 HTML catalogue-level refresh（low ROI 結構天花板）**：stat_edb_figures (vault mojibake fix) / arts_curr_docs / ph_pri_curr / edbc197_2024_ph_pri / moral_civic_curr。Catalogue-level 無頁；retrieval ROI 結構天花板、不能提高 traceability。唔急。
3. **🔴 既有 deferred + low-priority backlog**：§E.10 admin-login client-side gate（**S131 ACCEPTED + DOCUMENTED**，conditional on cosmetic-gate design unchanged — 拆掉 client-side-only 前提即須 reopen）；57014 transient（retry 即恢復）；FAIL-A 注入 regression（record-only）；P2/P3（39→148 deferred）；Mobile UI P2；HKEAA；doc-debt；**stat_fact 21 chunks 仍 cite 2024/25 wording**（stat_fact 升 2025/26 = future backlog；user-facing ROI ≈ 0 due to Channel B filter `content_type!=="stat_fact"`，唔急）。
4. **Q4 對外契約收斂（deferred）**：Channel A→knowledge.json→Circular System；3 選項；未明示勿掂。
5. **§8b rule 2 automation tooling（future implementation）**：semantic-supersede detection KLA-title embedding similarity check 暫 process-level apply；automated sub-agent prompt 留 future。
...
3. Completed:
   - ✅ **[Phase 2 brand launch]** custom domain `policychecker.wongfu.net` live (CNAME → leonard-wong-git.github.io / TTL 7200 / GitHub Pages HTTPS Let's Encrypt) + 4 HTML head OG/Twitter meta + 4-size PNG favicon variants (sips 32/180/192/512) + og-image PNG 1.7MB → JPG 151KB 1200×630 (WhatsApp 600KB limit fix) + multi-origin CORS allowlist (env.ts getCorsOrigins + server.ts setCorsHeaders(req,res) echo-Origin echo-or-fallback / 13 callsites updated / typecheck+build PASS) + `embed-sample.html` 公開 iframe demo for school IT/CMS
   - ✅ **[5-round brand unification]** 4 HTML titles → 純功能 (EDB 學校政策搜尋平台 / 搜尋工作室 / EDB 快速問答 / 校本採購指引) + hero mark/name K1/知識平台 → EDB/政策核對 + body "K1 知識平台" → "香港學校政策搜尋平台" + logo dot K1→EDB + sub 香港學校管治 → 政策核對 + description meta + OG site_name surface "PolicyChecker · 政策核對"；學段「K1-K3」/「K1 至 S6」references preserved per EDB standard
   - ✅ **[Phase 3b 6-stale audit]** agent-team 6-parallel Explore subagent_type 讀 6 superseder PDF prefaces → 2 HIGH-conf KEEP_BOTH (va_p1_s6_2024 explicit「請參閱」+ econ_supp_2025 explicit「同時參閱」) + 4 UNCLEAR-leaning-KEEP (ethics_relig_2024 / music_sss_2024 / econ_sss_2025 / bafs_2007_2020 no explicit「取代」found) → 0/6 DROP per Leonard conservative rule
   - ✅ **[Phase 3b execute §3 CHANGE divergence + clean recovery]** First execute halted mid-bafs source on Postgres `22P05 \\u0000 cannot be converted to text` (1 NUL byte at byte offset 95212 in bafs_sss_2007_2015_repaged.txt = PDF extraction artifact between Chinese sentences); partial state: 5 untouched + bafs 122 DELETEd + 50/93 INSERTed → STOP+report Leonard + 3-option AskUserQuestion → Leonard picks Option B (strip NUL + driver patch + re-execute = durable §8 regression fix) → Python strip NUL from bafs file (140632→140631 bytes) + cb3_b2 build_rows() defensive `ch.replace(\\x00, "")` before hash compute (1-line additive, behavior unchanged for clean chunks) → re-execute --execute --skip-local 6/6 OK
   - ✅ **[Phase 3b QC 3 PASS]** Supabase total via Range header **9,713 exact match prediction** (9,882 - 743 + 646 net -169) / per-source `del/ins/now` aligned (bafs 50→93 / econ_2007 147→112 / econ_supp 39→31 / ethics 166→137 / music 161→129 / va 180→144) / INVARIANT 7 spot-check 0 touched (g01=32 sag=383 chem=172 eng_lit=633 music_p1_s6=85 va_p1_s6=71 arts_kla=116) / live smoke 6/6 sources direct-Supabase sample chunk shows `=== Page N ===` markers ✓
   - ✅ **[PERSIST + commit chain origin/main]** `c6dab15` Phase 2 brand launch + `d2a7cac` OG image PNG→JPG WhatsApp + `2c0fde1` brand round-1 + `d86dfe5` round-2 app UI + `d10d12f` round-3 Group A hero + logo + `062fb88` Phase 3b 6-stale page-carry. driver 10th-validation S122-S132 = 65 sources end-to-end PASS, 0 final incident (NUL byte halt cleanly recovered via Option B durable fix).
4. Pending:
   - closeout commit (本 entry + governance docs)
5. Next priorities (max 3):
   - Phase 3a batch ranking polish ~15-18 sources (case-by-case)
   - Phase 3c 5 HTML catalogue-level refresh (low ROI)
   - 既有 deferred backlog (§E.10 conditional ACCEPTED / 57014 / FAIL-A / Q4 deferred / etc)
6. Risks / blockers:
   - **§3 CHANGE divergence stop-report-recover textbook execute 2nd time S122-S132** (S130 ct_filter divergence + S132 NUL byte divergence); lesson: dry-run+execute can surface runtime assumption errors mid-flight, halt-report-recover discipline → 0 final incident, partial state cleanly recovered
   - **NUL-byte PDF extraction artifact recurrence-prone**: 1 in 65 sources tested so far; codified as `ch.replace(\\x00,"")` defensive guard in cb3_b2 build_rows; future PDFs auto-clean (§8 regression lesson codified)
   - 既有 risks: 🔴 Supabase free-tier 57014 transient (retry 即恢復); FAIL-A 注入 regression (record-only); §3c FAIL-A/B record-only; q.html/A·AB code path dormant 勿清; Q4 deferred 未明示勿掂; Stage-2 closed; egress 每次自測; EDB PDF 永遠用 `url_primary` (§E.12); 路徑空格雙引號; Testing/ 喺 Draft git 外
   - 注意: Phase 3b 後 6 stale sources 唔再 in stale-Vanilla-preserved 狀態、CB-3 final ceiling 仍 ~88% (113 sources total, 100 marker-bearing, 2 deprecated, 9 結構天花板, 2 others)
7. **Session CLOSED 2026-05-28 (Leonard「收工」)** — Phase 2 brand launch live + Phase 3b 6-stale page-carry production live + §3 CHANGE divergence clean recovery + driver 10th-validation 65 sources + NUL-byte defensive guard codified. HEAD chain origin/main: `c6dab15`→`d2a7cac`→`2c0fde1`→`d86dfe5`→`d10d12f`→`062fb88` + closeout commit pending. §4a no trigger (224 lines < 400, 2 entries S130/S131 within 30d → 3 entries after S132 add). 下次起手＝問 Leonard 揀 Phase 3a batch ranking polish ~15-18 sources / Phase 3c 5 HTML catalogue-level refresh / 既有 deferred backlog。Phase 3b 完成 + brand 統一 + custom domain live = 大 milestone。
