...
## Open Priorities
> 產品方向：**全棧 Channel-B-only**。Channel A frozen @455。**S160：政策範本下載 + 文件修訂 兩個 tab staged（未 deploy）。⚠️ HEAD = `bd99b91` LOCAL，比 `origin/main`(`c47f604`) 多 2 commit 未 push（P2 `fcccc34` + P3 `bd99b91`）。** 主線 0 outstanding bug。

1. **Deploy P2+P3（待 Leonard review → 一鍵 push）**：政策範本下載 tab（`fcccc34`）+ 文件修訂 feature（`bd99b91`，含新 backend `/api/checklist-revise` + `/api/checklist-domains`）。Review 後 `cd "…/Draft" && git push origin main` = GitHub Pages（前端）+ Render（backend）**同時** deploy。前端 `REVISE_BACKEND_URL` 自動 onrender；deploy 後 smoke `curl https://edb-knowledge.onrender.com/api/checklist-domains`。詳 `dev/SCHOOL_DOCX_DOWNLOAD_PLAN.md` + `dev/CHECKLIST_REVISE_FEATURE.md`。
2. **#2 幼稚園 Phase 2 — KG live 入庫（DEFERRED，prep DONE，Leonard supervised）**：S160 嘗試 live INSERT 被 harness auto-classifier 拒（「除幼稚園外」+ prior「supervised fresh session」）→ 尊重、未做。prep 100% ready：`python3 dev/ingest_one_source.py kg_admin_guide_2026 && python3 dev/ingest_one_source.py kg_operation_manual_2026`（先 --dry-run；218+217=**435 chunks**、page-resolvable、兩源 live 0 rows、dry-run 重驗綠）。之後 KG route（searchChannelB.ts SOURCE_SETS+TOPIC_KEYWORDS）+ registry + **display-sync 7 點**（14,674→~15,109）+ routed smoke + KG 清單 pilot。⚠️ display drift byte-identical；INSPECT before。
3. **#3 學校版分校類 — 待 Leonard review docx（monitor）**：102 docx 喺 `dev/checklists/<域>/`（**現可由「政策範本」tab 下載**）。反饋要改格式/分流 → 調生成器（`gen_school_docx.js`/`gen_checklist_docx.js` `<type>` arg）或 tags 一鍵重生（**改 tags 後跑 `apply_school_types.py --check`**；改 docx 後 re-run `gen_templates_manifest.py` + `gen_checklists_bundle.py`）。
4. **文件修訂 / 文件分析 follow-up（Phase 2.5）**：覆蓋門檻 `COVERED=0.50`/`PARTIAL=0.42`（checklistRevise.ts，tunable）；大域 >`MAX_ITEMS=220` 項截斷（UI 有提示）；可加 LLM 覆核 borderline；mobile shell 未有入口（同 文件分析/政策範本）。#4 文件分析 source-level 校類 filter 同樣 pending。
5. **既有 monitor**：IMC grouped 頁碼只對第一份 PDF 準 / `cgss_2024` routed rank 低 / 57014 cold-start / `stats.sources`=120 cosmetic-stale / S153 synthesis ~328 字 / Azure OpenAI fallback / 自動發現+freshness 週跑（detection-only）。
...
3. Completed（詳見 SESSION_LOG S160）:
   - ⛔→**DEFERRED #2 KG live 入庫**：嘗試 live INSERT 被 harness auto-classifier 拒（理由「除幼稚園外」+ prior「supervised fresh session」）→ 尊重拒絕、唔做 overnight；prep 100% ready（dry-run 重驗 218+217=435、page-resolvable、兩源 live 0 rows）。
   - ✅ **P2 政策範本下載 tab**（`fcccc34`，**staged 未 deploy**）：app.html +TemplatesPanel +'templates' tab；`policy_templates.json` manifest（14 域 102 docx；`gen_templates_manifest.py` 生成）；連現有 live docx（`.nojekyll`，HEAD-200 驗）；browser-verify 102 連結 / 校類 filter（小學→26）/ fetch 200。
   - ✅ **P3 文件修訂 feature**（`bd99b91`，**staged 未 deploy**）：backend `checklistRevise.ts`（embedding coverage + clause supplement，無 LLM）+ `/api/checklist-revise` + `/api/checklist-domains`；`checklists_bundle.json`（14 域 2944 items/1282 clauses，root，backend runtime 載入）；app.html +ReviewPanel +'review' tab +buildRevisedDocx。
4. Pending: **Deploy P2+P3**（Leonard review → `git push`）；**#2 KG ingestion**（Leonard supervised）；#3 docx review；文件修訂 Phase 2.5（門檻 tune / LLM 覆核 / mobile）。
5. Next priorities: Leonard review+deploy P2/P3 → #2 KG ingestion → #3 review。
6. Risks: ⚠️ **HEAD `bd99b91` LOCAL 比 `origin/main`(`c47f604`) 多 2 commit 未 push**（P2/P3 staged）。Supabase 14,674 / live product **零接觸**（無 deploy）。覆蓋門檻係估算（tunable）；`checklists_bundle.json` 公開於 root（EDB 衍生、benign）。
7. commits（**LOCAL，未 push**）: `fcccc34`(P2 政策範本)→`bd99b91`(P3 文件修訂)→收工 gov commit。origin/main 仍 `c47f604`。
   - QC：P2 browser-verify 全綠；P3 backend python e2e（match 198/10/0 vs unrelated 1/5/202、400 err）+ tsc check/build + frontend browser stub（render/filter/docx PK 8365B）。
