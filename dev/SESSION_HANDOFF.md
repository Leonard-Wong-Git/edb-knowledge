...
- **未做（不要當已做）**：`EVAL_LATEST` 仍讀**旗標關閉**側（選檔缺陷，見 Risks 8 與 Open Priorities ③）· Recall@k 完整指標仍未跑（`_s213_eval_metrics.py` 要語料快取，指向已消失的 scratchpad；本節的 recall@8 是由 run 檔的 `rank_of_expected_chunk` 直接算，未經該模組的 occupancy／per-domain 指標）· gold 標籤自 S223 換版後**未重新核實**（5 條以 `g24` 為預期或替代來源，其中 `cpd_mainland_promotion_tour` 的 `verified_by.chunk_id` 指向已刪語料）· `regression:semantic`／`regression:grounded` 未跑 · 六項人手驗證仍未記錄 · `agent-handoff-kit` CLI 未安裝，`closeout-status` 語義閘未能執行。
...
**Recommended next step（S226 收工重生）：** **③(b) 兩個 registry ERROR** —— ①（185 題 gold 重跑）本節已完成，②、③ 的其餘各項現時都需要你先拍板或先做標註，而 (b) 那兩個 ERROR 是純機械、無須決策、做完直接把封版閘的 `NO_ERROR_FAIL` 往前推一格的一項。

① 🟢 **【S226 完成】185 題 gold 已重跑，兩節量度債已還清；剩下的是它揭出的三件事。** 實測數字見 `## Current Baseline` 7 與 `## Validation / QC` S226 段。結論三句：(a) **旗標值得開**——來源層 119 → 122、片段層 recall@8 0.325 → 0.349、routed p90 由 S220 的 8,335ms 降至 2,498ms、路由失敗由 9 條降至 0；(b) **SAG 換版沒有丟內容**——生產配置側只有 1 條 verdict 退步（`hr_mpf`），而該條的舊 PASS 本來就是靠已退役的 `g24` 撐住，主要預期來源兩次都不在前八，內容仍在庫內（新版 SAG 23 條含「公積金」）；(c) **真缺口在片段層與短查詢**——166 題有段落簽名者 recall@8 只有 0.349，64 題拿對文件卻拿錯段落；`plain`（1–3 詞）題 FAIL 24/77，而 `natural_sentence` 只有 6/43。**餘下三項（都不是重跑能解決的）**：① -1 gold 標籤自 S223 換版後未重新核實（要先重建語料快取，`_s213_recon.py`）；① -2 `_s213_eval_metrics.py` 的完整指標（occupancy／per-domain／MRR）仍未跑，同樣卡在該快取；① -3 19 條無答案題有 1 條分數 0.7575 超過棄權門檻 0.75、另有 1 個 forbidden hit。
...
③ **封版閘現時 5/15（S226 由 4/15 推上，但那 +1 是量度覆蓋率、不是質素）。** 逐類現況：**(a) `EVAL_CHUNK_LAYER` 已有數**（S226 修好 `summarize()` 缺口，NOT_MEASURED → FAIL，`NO_UNMEASURED` 轉 MET）——要它轉綠須把 chunk_FAIL 由 107 降下來，屬 ①(c) 那條真缺口，非閘的問題。**(b) 兩個 ERROR 仍在**：`REGISTRY_SERIES`（1 個來源 `stat_enrolment_report`，13 個分年檔／528 條片段不受任何 registry 監察）與 `REGISTRY_UNMANAGED`（`stat_integrated` 2 條，id 錯配）。**純機械、無須決策，是現時最值得做的一項。** **(c) 兩個 WARN 沒有 waiver**：`BODY_LENGTH_FLOOR`（3）、`MOJIBAKE`（58）——兩者都已逐條讀過並判定可接受，只欠在 `release_gate.json` 寫下 owner／理由／接受期限。**(d) 六項人手驗證從未記錄**（核心搜尋案例、答案事實 spot-check、文件標註流程、mobile/desktop 分流、私隱與上載、冷啟動與錯誤狀態）。**(e) 【S226 新增，須 Leonard 拍板】`EVAL_LATEST` 選檔揀錯側**（詳見 Risks 9）：三個選項——**A**（建議）在合格 gold run 中優先揀 `endpoint` 以 http 開頭者，即生產端點那一份，detail 標明所選檔與日期，只有 in-process run 時照舊揀最新並寫明「非生產端點」；**B** 由 `release_gate.json` 宣告生產旗標狀態、選 label 相符者（多一個人手維護的真相來源）；**C** 改命名慣例令生產 run 排最後（語意藏在檔名，下一位不會知道）。實施須跟 `DOC_SYNC_CHECKLIST.md` row 56。
