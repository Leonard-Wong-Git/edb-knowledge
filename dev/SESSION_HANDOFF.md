...
- **第六批（Leonard「全做」授權的四項；兩個新 knob 都預設關閉）**：
  ...
  - 🟡 **`REGISTRY_UNMANAGED` 的修正工具已備但未執行** —— `dev/_s226_fix_unmanaged_source.py`（預設 dry-run、有 `--execute` 與 `--rollback`、以 id 精確鎖定 2 行、附守恆核對）。dry-run 通過。**生產寫入被 auto mode 分類器擋住（`Modify Shared Resources`），要 Leonard 自己跑。**
