...
2. Session ID: Claude_20260614_S163 (S163) — Leonard「ABC」= 做晒 3 個 NEXT（C 核 KG QC / A Phase 2.5 / B Phase 2）+ 規劃版本/首頁/平台簡介；全權自主、慳 token、中斷則 22:40 續。
3. Completed（詳見 SESSION_LOG S163）:
   - ✅ **起手核實全綠**：HEAD `8f18f5a`==origin/main；Supabase **15,109**；`/api/checklist-domains`=15 域含 kg_operation；範本下載 live 見幼稚園營運卡。
   - ✅ **C 核 KG QC**（`ef43517`）：`QC_VERIFY_ISSUES.md` 17 flags 全修（`_qc_fix.py` exact-match+assert：刪虛構鋪墊句／補漏義務／修錯引用）。**深挖揭發並修 S162 結構 bug**：kg_operation/clauses.json 用非標準 schema `section_no/name`（14 既有域＋兩 docx 生成器＋backend 全期望 canonical `si/section_name`）→ bundle `si:None` 令 backend supplement linkage（`c.si===sectionIdx+1`）對 kg_operation **全失效**（388/388 items 拎唔到 clause→修後恢復）+ 學校版 docx 章節名空白（似 S159 bug）。正規化 schema、重生 4 docx + bundle + manifest。
   - ✅ **A 文件標註 Phase 2.5**（`d71ae1e`）：`detectDomainsPerSegment`（per-segment argmax 路由）取代 whole-doc `detectRelevantDomains`；`AUTO_DETECT_MAX_DOMAINS=3`、`SECONDARY_MIN_SEGMENTS=2`。真 OpenAI 驗：maths→curriculum 單域（legacy 誤判 qa_inspection）、SEN+gifted→兩域、primary scoping 排除 kg_operation、empty→[]。前端已 render domains[]，零 UI 改。Render 驗 SEN doc→['sen'] 21 findings。
   - ✅ **版本+首頁+平台介紹**（`f510ee8`）：`PLATFORM_VERSION='3.0.0'` decouple（凍結 knowledge `_meta.version` 2.3.0 不動）；app.html 平台介紹 channels 改 5 卡（政策搜尋/文件標註/範本下載/指引文件庫/通告分析）+ hero + 版本徽章；index.html +文件標註+範本下載 卡 + v3.0 eyebrow + meta；CHANGELOG v3.0.0 entry。browser-verify desktop（5 卡、v3.0.0、0 err）+ index static checks。
