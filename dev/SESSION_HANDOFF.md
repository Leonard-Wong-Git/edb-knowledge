...
3. **Knowledge state**: **455** Channel A facts（三層同步 byte-identical）、0 queue；Supabase **12,290** chunks（S146 +1,683：batch1 g33/g35/g36/g37/g09/g14/g17 + batch2 apl_curr_docs/g07/g23/nat_sec_edu；**S147 +13** = `mce_framework_2008` 德育公民課程架構2008，**雲端 vision OCR** 入庫〔掃描影像 PDF、新工具 `dev/ocr_extract.py`〕+ 加 SOURCE_SETS.curriculum allowlist）；指引 4 層（161 app/152 公開/203 registry/120 vault）；CB-3 補入庫進行中（見 `dev/INGEST_GAP_2026-06-06.md`）；Phase 3 全完成。
...
1. **Channel B 補入庫 — 餘下可選**（已入 12 源；仍 39 registry 源未入，但只 ~14 有內容可加，分類見 `dev/INGEST_GAP_2026-06-06.md` 批次2 段）：~~(a) `mce_framework_2008`~~ = **S147 DONE**（OCR 入庫 +13）；(b) `g38` 音樂指引 = CID 字體亂碼 **needs OCR**（可用新 `dev/ocr_extract.py` 雲端 vision OCR，同 mce 一樣）；(c) `g13` 中學課程指引(SECG) / `g16` 訓育工作指引 = 真 guide 但分章碎檔、要 multi-PDF 砌；(d) `chi_edu_curr_docs` 全本 CLEKLAG（g09 已有節錄，待決定）；(e) `gifted_policy_docs` 待 Leonard 正確 link；(f) g17 深化；(g) `phys_sss_2007_2015`（2015 舊版＋**文字層未驗**、做之前先試抽，可能又要 OCR）。**其餘 ~24 源 = dup/已覆蓋/deprecated/舊版/示例，建議唔做。** 工具：文字層 PDF/HTML 用 `dev/fetch_extract.py`、**掃描影像/CID 亂碼 PDF 用 `dev/ocr_extract.py`（雲端 vision OCR、S147 新增）**，再 `dev/ingest_one_source.py`（含 NUL strip；per-source 安全；勿跑 full `wiki_index.json` upload）。**入庫前必先試抽驗文字層；新源入庫後必加 backend `SOURCE_SETS` allowlist（S135 coupling，否則 routed search 唔 surface）。** dup/gap 按內容（url/PDF 檔名）對賬。
