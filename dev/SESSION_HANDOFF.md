...
## Open Priorities
> 產品方向：**搜尋介面 Channel-B-only**（S119 定，A/AB dormant；Q4 契約 deferred）。北極星＝合理＋指引＋**頁數**（CB-3 不可 defer）。Stage-2 closed 勿復活。**CB-3 達 final ceiling ~88%**（94/113 marker-bearing + 2 deprecated + 6 Vanilla-preserved stale + 9 結構天花板 + 2 others = 113）。**driver 6 輪 verified（S122~S125c、52 sources 0 incident）+ NEW `cb3_deprecate_stale.py` 0 incident first-use**。**S127：§8b 3-rule + governance doc full update closed**（PROJECT_MASTER_SPEC §D.16/§D.19/§G.2/§G.3 全 codified、純 markdown 改 0 code/data mutation）。
...
5. **§8b rule 2 automation tooling（future implementation）**：semantic-supersede detection 嘅 KLA-title embedding similarity check 暫 process-level apply（每 batch audit agent 必 raise candidate pair）；automated sub-agent prompt 留 future batch / governance session 寫。
