...
**Recommended next step（S226 收工重生）：** **片段層** —— 現時唯一一個「量到、未動」的大缺口：Chunk Recall@5 **0.345**，64 題拿對文件卻拿錯段落，而兩側基線都是今日的，改切片或排序有現成 before 可比；基線每過一次語料變動就失效，所以拖一節就要再付一次重跑成本。**若想要一件無須決策的短工**，改做 ③(b) 兩個 registry ERROR。
