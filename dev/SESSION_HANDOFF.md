...
## Current Baseline
1. Version: **v1.3.1** (K1 EDB Knowledge Platform) — local version bump prepared; backend split-role compatibility bridge included
...
4. Release / merge status: **v1.3.1 prepared locally, not yet pushed in this session**。`knowledge.json` 已由 `v1.3.0` bump 到 `v1.3.1`；backend package 已由 `0.1.0` bump 到 `0.1.1`，並加入 split-role compatibility bridge。Repo: `Leonard-Wong-Git/edb-knowledge`. Live URL: https://leonard-wong-git.github.io/edb-knowledge/k1-dashboard.html.
...
## Open Priorities
1. **[發布]** commit + push `v1.3.1`（含 backend split-role compatibility bridge）
...
2. Session ID: Codex_20260408_0955
3. Completed:
   - ✅ 更新 `dev/CODEBASE_CONTEXT.md` directory map：`K1_API_SPEC.md` 已在 repo root，不在 `dev/`
   - ✅ 確認 public `knowledge.json` 已是 split-role schema，並 bump 至 `v1.3.1`
   - ✅ 升級 backend `types/knowledge.ts`：同時接受 `department_head`、`subject_head`、`panel_chair`
   - ✅ 升級 backend `knowledgeSelector.ts`：加入 bridge logic，可同時處理舊 merged role 與新 split-role schema
   - ✅ backend package version bump：`0.1.0` → `0.1.1`
   - ✅ `cd backend && npm run check` 通過
   - ✅ `cd backend && npm run build` 通過
4. Pending: commit + push `v1.3.1`；EDB 側更新取值邏輯；backend regression test；決定 `dev/knowledge/role_facts.json` 是否也升級
5. Next priorities (max 3): (1) commit + push `v1.3.1` (2) 通知 EDB 側更新 subject_head+panel_chair 取值邏輯 (3) backend semantic quality regression
6. Risks / blockers: EDB Circular System 仍使用舊 department_head 取值直至更新；`dev/knowledge/role_facts.json` 與 public `knowledge.json` 仍不同步；backend 仍是未追蹤目錄，需小心只 stage 必要檔案
