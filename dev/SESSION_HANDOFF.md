...
3. Knowledge state: **455 Channel A facts** (三層同步 ✅ byte-identical；dedup 792→455 commit `711f911`), **0 candidates in queue**, **Supabase 10,117 chunks**（S125 post-batch-5；S125 total DELETE 1,289 INSERT 1,153 net -136 across 20 batch-4+batch-5 sources）。Vault: 120 sources 提取完成；39 marker via Option B + 3 pilot + 10 batch-1 + 10 batch-2 + 10 batch-3 + 10 batch-4 + 10 batch-5 = **92 sources Supabase page-carried**；whole-vault page-resolvable **~80.0%**（~8,094/10,117）；剩 **~10 marker-less PDFs**（batch-6：7 stale-superseded for DEPRECATION track + 3 truly orphan small）+ 9 結構天花板。**指引數字 4 層（148 app 內庫 / 39 公開 guidelines.json / 151 source_registry / 120 vault-extracted）見 PROJECT_MASTER_SPEC §B.1 釐清框 — 39 是否擴到 148 = OPEN DECISION，未收斂**。
...
## Open Priorities
> 產品方向：**搜尋介面 Channel-B-only**（S119 定，A/AB dormant；Q4 契約 deferred）。北極星＝合理＋指引＋**頁數**（CB-3 不可 defer）。Stage-2 closed 勿復活。**driver 5 輪 verified（S122~S125b、50 sources 0 incident）；剩 ~10 marker-less PDFs**（batch-6 deprecation-mixed）+ 9 結構天花板。**S125b 2 新教訓 §8b promote candidates**：(1) audit cross-check stale-superseded（first-live-applied S125b 揭 1,010 chunks）(2) NEW semantic-supersede detection（g24/sag pattern）。
1. **broader Option C batch-6 = DEPRECATION-mixed**（not pure page-carry）：7 stale-superseded DROP candidate（va_sss_2015 180 / ethics_relig_sss_2007_2019 166 / music_sss_2015 161 / econ_sss_2007_2015 147 / econ_sss_supp_2015 39 / bafs_sss_2007_2015 122 / pe_sss_2007_2015 119 / sci_jss_supp_2017 76）— Vanilla 全保留 / Hybrid 部分 DROP / Aggressive 全 DROP，需 Leonard 拍板。3 orphan small (g15 / edbcm98_2024_pri_science / 另選) 走 page-carry path。**Driver 限制**：cb3_b2_pagecarry_migrate.py 只 page-carry、DROP-only 需新 script 或 Leonard Dashboard SQL。
2. **Freshness workflow triage**（chronic ops cleanup）：(a) 本地跑 `python3 dev/source/check_freshness.py --dry-run` 識別 N 條 fail URL；(b) 修 source_registry.json URL drift；(c) 改 script 失敗 threshold（建議 errors >5 才 exit 1 / 用 GitHub Issue 而非 fail email）。SESSION_HANDOFF Regression Notes #2 同步更新「stale baseline」。Chronic 5 連 fail since 2026-04-30、非 blocker、ops noise。
3. **§8b rule promotion (S125b 2 lessons)**：(1) audit cross-check stale-superseded（first applied S125b、codify PROJECT_MASTER_SPEC §D.16 + §8 lessons）(2) NEW semantic-supersede detection（g24/sag、tech_kla/pri_curr、music_sss_2024/music_p1_s6 同 pattern；audit agent 加 KLA-title embedding similarity check）。
4. **batch ranking polish backlog（低優先）**：S122-S125b 累計 ~17 sources ranking competition（含 stale-superseded 真因 + semantic-supersede 真因）。
...
6. **Q4 對外契約收斂（deferred）**：Channel A→knowledge.json→Circular System；3 選項待 CB-3 成熟；未明示勿掂。
7. **Governance doc full update**（PROJECT_MASTER_SPEC §D.16 batch-4/5 verified + §8b 2 new rules codify）：建議 batch-6 closeout 一次過做。

> ✅ **S125 完成（雙批）**：CB-3 Option C broader batch-4 + batch-5（共 20 sources）page-carry 生產 live；Supabase 10,253→10,133→10,117；whole-vault page-resolvable ~73.0%→~80.0%；92/113 sources marker-bearing；driver 5th-validation；§8b audit cross-check first live application 揭 8 stale-superseded 1,010 chunks；§8b NEW semantic-supersede lesson surfaced；Freshness workflow chronic fail triaged。
...
2. Session ID: Claude_20260526_0737 (Session 125 — batch-4 + batch-5 雙批)
3. Completed:
   - ✅ **[Batch-4 完整 cycle]** 10 sources page-carry：CHANGE step 0 repage_pdfs.py +10 → Gate 1 dry-run+EXECUTE 10/10 PASS（pages 383 / backup `dev/init_backup/20260526_073931_UTC/`）→ Gate 2 dry-run no anomaly → Gate 2 EXECUTE 10/10 OK（DELETE 537 INSERT 417 net -120 / Supabase 10,253→10,133 / wiki_index auto-backup `dev/init_backup/20260526_091916_UTC/`）→ QC 4/4 PASS + smoke 4/10 direct surface + 6/10 ranking competition non-regression + commit `e703910` push origin/main。
   - ✅ **[Batch-5 完整 cycle Vanilla strategy]** 10 sources (g24/g29/sci_jss_framework_2025/pe_sss_2023/edbcm183_2023_values_edu/sec_curr_guide_2017_booklet_6a/edbcm58_2024_pri_science/pri_science_cert_course_list/edbcm57_2024_pri_science/edbcm243_2024_pri_science) page-carry：CHANGE step 0 repage_pdfs.py +10 batch-5 entries（size 43→53）→ Gate 1 dry-run+EXECUTE 10/10 PASS（pages 616 / backup `dev/init_backup/20260526_124023_UTC/` / content sanity 101.4-103.8%）→ Gate 2 dry-run 揭 g24 300→383 +28% content RECOVERY 撞 legacy 300 cap（同 S122/S123 pattern、Monitor 模型 update need）→ Gate 2 EXECUTE 10/10 OK（DELETE 752 INSERT 736 net **-16** vs predict -188 / Supabase 10,133→10,117 / wiki_index auto-backup `dev/init_backup/20260526_133107_UTC/` / per-source del/ins/now 全對齊）→ QC PASS + INVARIANT 6 spot-check 0 touched + smoke 5/10 surface (3 direct + 2 bonus)。
   ...
   - ✅ **[§8b audit cross-check FIRST LIVE APPLICATION (S125b)]** 揭發 8 stale-superseded 仲 in index 共 1,010 chunks (~10% Supabase)：va_sss_2015 (180) / ethics_relig_sss_2007_2019 (166) / music_sss_2015 (161) / econ_sss_2007_2015 (147) / econ_sss_supp_2015 (39) / bafs_sss_2007_2015 (122) / pe_sss_2007_2015 (119) / sci_jss_supp_2017 (76)。Leonard 揀 Vanilla 保 §A.2 #1 traceability、deprecation 推 batch-6 評估。
   - ✅ **[§8b NEW semantic-supersede lesson (S125b)]** g24 vs sag_2025_11 = registry `supersedes=[]` 但實質 same-domain elder vs newer consolidated（同 KLA + same naming pattern + title overlap）；S122 tech_kla/S123 music_sss_2024 同 pattern；audit cross-check 之前只用 registry supersede field、未 catch semantic-level。Recurrence-prone → promote candidate（audit agent 加 KLA-title embedding similarity check）。
   - ✅ **[Mid-session Freshness workflow chronic-fail triage]** 5 連 fail since 2026-04-30 / root cause check_freshness.py errors>0 exit 1 + EDB intermittent；非 batch 觸發；Regression #2 stale 確認；列下次 priority。
   - ✅ **[PERSIST]** SESSION_LOG S125 entry（batch-4 main + batch-5 sub-block + DOC_SYNC matrix + verbatim handoff rewrite covering 雙批）；SESSION_HANDOFF baseline #1/#3 + Open Priorities regen + 本 record；CODEBASE_CONTEXT +S125 entry；HANDOFF_PACKAGE §2 chunks 10,117。
4. Pending：
   - **broader Option C batch-6 = DEPRECATION-mixed**：7 stale-superseded DROP candidate + 3 orphan small；需 Leonard 拍板 strategy；driver 限制：DROP-only 需新 script。
   - **Freshness workflow triage**（chronic fail、低 blast radius、ops noise）。
   - **§8b 2-rule promotion**：(1) audit cross-check stale-superseded（first live S125b）(2) NEW semantic-supersede detection。
   - PROJECT_MASTER_SPEC §D.16 batch-4/5 verified note + §8b 2 new rules codify（建議 batch-6 closeout 一次過）。
5. Next priorities (max 3)：
   - **broader Option C batch-6 deprecation strategy 排**（需 Leonard 拍板）
   - **Freshness workflow triage**（5 連 chronic、ops cleanup）
   - **§8b 2-rule codify + governance doc full update** OR 🔴 §E.10 admin-login client-side gate
6. Risks / blockers:
   - **2 §8b promote candidates pending Leonard decision**：(1) audit cross-check stale-superseded (live-validated S125b、~10% Supabase) (2) NEW semantic-supersede detection (g24/sag pattern)；下次 batch-6 + governance doc update 時 codify。
   - **§E.14 driver reuse 5th-validation**：50 sources 0 incident；pipeline production-ready；batch-6 deprecation 部分需新 script。
   ...
   - **Freshness workflow chronic fail (NEW S125)**：低 blast radius（ops noise + Regression #2 stale）；triage 列 priority #2。
   - local `wiki_index.json` vs Supabase 92 源 diverge（S125 後 72→92；Supabase query-authoritative；reconcile 低優先 backlog）。
   - 既有 risks：🔴 §E.10；🔴 57014 transient（S125 ict_sss_2021 + g24 各 1 次 retry 恢復）；🔴 FAIL-A（record-only）；Q4 deferred；Stage-2 closed 勿復活；egress 每次自測；EDB PDF 永遠用 `url_primary`（§E.12）；路徑空格雙引號。
7. **Session 進行中**（待 Leonard「收工」或繼續其他 OP）— S125 batch-4 + batch-5 共 20 sources 完成生產 live + freshness triaged + 2 §8b lessons surfaced + commit/push batch-5 closeout pending。
