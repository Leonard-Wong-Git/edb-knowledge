...
- **第六批（Leonard「全做」授權的四項；兩個新 knob 都預設關閉）**：
  ...
  - 🔴 **新發現一個路由缺陷（不在授權四項內，未動）**：`評核` 單獨就路由去 **curriculum**（實測 `detectQueryCategory('教師評核') → curriculum`），所以「教師評核」被送去課程語料，那裡沒有 SAG，答案永遠取不到。與已知的 `氣體` → safety 同族（backlog ④(d)）。`hr_appraisal` 這題的來源層 FAIL 就是這個成因，不是標籤問題。
...
**Recommended next step（S226 收工重生）：** **決定 `FEATURE_LEXICAL_RERANK` 出不出貨** —— 它是本節唯一兩層同時改善的改動（來源層 122→129、7 修好 0 退步；片段層 58→64），而且順手修好 `hr_mpf`（SAG 換版唯一退步）與 `cur_secondary_central_curriculum`（route-first 兩條退步之一），零額外資料庫成本。出貨前要做的三件事：(a) 權重穩健性 —— 0.02 的第二次 A/B（S226 已跑，見下）；(b) 讀準那 +7 的成分 —— 其中一部分只是「正確文件到了」而非答得到的段落，可信的是片段層 +6；(c) 合成側影響未量 —— 本節兩側都 `synthesize:false`，而重排會改變餵給合成器與判官閘的窗口內容。**(c) 是唯一真正未做的前置條件。**
