...
1. **文件標註：建議相關性收緊（Leonard 未拍板，最immediate）**：S161 Leonard 真檔（小學數學課程）試用揭發 —— 課程/一般文件會出**跨範疇/跨校類**建議（如數學 doc 出 SEN 三層支援、幼稚園課程架構、增補基金買點字機），因 auto-detect 揀多範疇 + 將該範疇成份 checklist 缺漏全當建議塞落。已向 Leonard 出 4 選項（A 收緊對焦：auto-detect 1 域+嚴格校類 filter+提高門檻 / B 保留廣泛 completeness / C 預設收緊+可展開 / D 課程文件只出指引比對、暫停清單建議）**未揀**。⚠ 校類 filter 漏洞：無 school_types tag 嘅 clause（如幼稚園課程指引衍生）即使揀咗小學都會漏出 → 要更好 clause school-type tagging 或 source-aware filter。下次先問 Leonard 揀邊個方向。
