...
## Open Priorities
> 產品方向：**搜尋介面 Channel-B-only**（S119 定，A/AB dormant；Q4 契約 deferred）。北極星＝合理＋指引＋**頁數**（CB-3 不可 defer）。Stage-2 closed 勿復活。**CB-3 達 final ceiling ~88%**（94/113 marker-bearing + 2 deprecated + 6 Vanilla-preserved stale + 9 結構天花板 + 2 others = 113）。**driver 6 輪 verified（S122~S125c、52 sources 0 incident）+ NEW `cb3_deprecate_stale.py` 0 incident first-use**。**S128：S126 follow-up trio (a)+(b)+(c) closed**（g28 URL drift fix `9122964` + freshness persist `9f5c514` Checked 147 Changes 22 Errors 0 + g29/g24 sanity check confirmed non-drift baseline 本身 stale；§E.12 pattern 第二次 ops 應用）。**S127：§8b 3-rule + governance doc full update closed**（PROJECT_MASTER_SPEC §D.16/§D.19/§G.2/§G.3 全 codified、純 markdown 改 0 code/data mutation）。
1. **Future batch-7 (optional)**：6 stale Vanilla-preserved sources case-by-case re-evaluate（va_sss_2015 180 / ethics_relig_sss_2007_2019 166 / music_sss_2015 161 / econ_sss_2007_2015 147 / econ_sss_supp_2015 39 / bafs_sss_2007_2015 122 = 815 chunks 仲 in index）；如果 ranking polish 後仍構成顯著競爭、可考慮再 Hybrid deprecate；唔急。
2. **🔴 既有 deferred + batch ranking polish backlog**：§E.10 admin-login client-side gate（OPEN）；57014 transient（retry 即恢復）；FAIL-A 注入 regression（record-only）；P2/P3（39→148 deferred）；Mobile UI P2；HKEAA；doc-debt；batch ranking polish ~15-17 sources（S122-S125c 累計）。
3. **Q4 對外契約收斂（deferred）**：Channel A→knowledge.json→Circular System；3 選項；未明示勿掂。
4. **§8b rule 2 automation tooling（future implementation）**：semantic-supersede detection 嘅 KLA-title embedding similarity check 暫 process-level apply（每 batch audit agent 必 raise candidate pair）；automated sub-agent prompt 留 future batch / governance session 寫。
5. **可能 future content refresh**：S128 揭 14 sources 內容已 update on EDB live（stat_kg/pri/sec/special / arts_curr_docs / ph_pri_curr / edbc197 / moral_civic_curr / arts_kla_guide_2017 / music_p1_s6_2024 / va_p1_s6_2024 / stat_edb_figures）；若要對齊 vault txt 抑或 Supabase chunks 內容 → 屬另一個 batch task（re-fetch + re-extract + page-carry）；非北極星阻塞、唔急。
