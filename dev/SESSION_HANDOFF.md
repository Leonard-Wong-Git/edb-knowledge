...
## Open Priorities
> 產品方向：**全棧 Channel-B-only**。Channel A frozen @455。**S162（4123 排序）：④ 幼稚園清單 pilot（新 kg_operation 域 → 15 域）+ ① 跨校類 filter（domain-level school_types）+ ② dead-code cleanup 全 DONE & pushed（`ec01e1b`→`5dde30f`→`51b6df2`）；③ 文件標註 Phase 2 留下次。S161：「文件標註」合併主線 Phase 1 LIVE。Supabase 15,109、HEAD==origin/main。** 主線 0 outstanding bug。

1. **③ 文件標註 Phase 2 / 2.5（4123 餘下，大 feature，留 fresh session）**：Phase 2 = PDF inline highlight（pdf-lib + pdf.js 座標，handoff 標「難」）；Phase 2.5 = per-segment `detectQueryCategory` 自動偵測範疇取代手動 multi-select。重用既有 annotateDocument/analyzeDocument。
2. **KG pilot 收尾（S162 餘下）**：(a) Leonard review `dev/checklists/kg_operation/QC_VERIFY_ISSUES.md`（16 軟性 fabricated/distorted-number 覆核待辦，已修法團校董會→校董會、13/20 章覆核清）；(b) 部署後 live 驗「範本下載」見幼稚園營運卡 + `/api/checklist-domains` 含 kg_operation。改 docx/checklist re-run `gen_checklists_bundle.py`+`gen_templates_manifest.py`。
3. **#3 學校版分校類 — 待 Leonard review docx（monitor，live）**：102 docx 由「範本下載」tab 下載。改格式/分流 → 調生成器/tags（改 tags 跑 `apply_school_types.py --check`；改 docx re-run 兩 generator 再 push）。
4. **既有 monitor + follow-up**：`kg_admin`「幼稚園質素」→qa_inspection（minor）/ 文件標註門檻 COVERED=0.50/PARTIAL=0.42 + AUTO_DETECT_THRESHOLD=0.38 + auto missing cap 8 tunable / IMC 頁碼 / cgss rank 低 / 57014 free-tier / stats.sources=120 cosmetic / Azure fallback / 週跑。
...
2. Session ID: Claude_20260614_S162 (S162) — 全權自主（Leonard 答「4123」= 按 handoff NEXT 排序做 ④①②③）
3. Completed（詳見 SESSION_LOG S162）:
   - ✅ **起手核實全綠**：HEAD `c0cffd3`==origin/main；Supabase **15,109**；`/api/annotate-document` live 探針 auto-detect 單域。
   - ✅ **④ 幼稚園清單 pilot**（`ec01e1b`）：新範疇 `kg_operation`（幼稚園營運）行勻 14-域 pipeline（chunk 2 KG 源入 all_chunks → distill Workflow 414 items → mech-verify 388 → 272 碎 section 整合 20 章 → rewrite Workflow 162 clauses）→ **15 域**；4 docx（學校版+清單 × 通用/幼稚園）入「範本下載」（106 docx）；`gen_*` +幼稚園/kindergarten；app.html +幼稚園 filter；**backend 零 code 改**（bundle-driven）。17 覆核 issues：修法團校董會→校董會，16 軟性入 `kg_operation/QC_VERIFY_ISSUES.md`。
   - ✅ **① 跨校類 filter**（`5dde30f`）：bundle 加 domain-level `school_types`（`_school_type_profiles.json` applies_to）+ backend `okType` precedence（clause→domain→all）+ `detectRelevantDomains` 加 sel。untagged clause 唔再跨校類漏（live A-D + regression PASS；一併修 6 既有型別專屬域）。
   - ✅ **② dead-code cleanup**（`51b6df2`）：刪 AnalyzePanel/ReviewPanel/buildAnnotatedDocx/buildRevisedDocx（app.html 4345→3715，−630；browser-verify 5 tab 0 err）。
4. Pending: **③ 文件標註 Phase 2（PDF inline highlight）/ Phase 2.5（per-segment auto-detect）= 大 feature 留下次**；KG pilot 收尾（Leonard 核 QC_VERIFY_ISSUES + 部署後 live 驗）；#3 docx review。
5. Next priorities: 見 Open Priorities（③ Phase 2 → KG pilot 收尾 → #3 review）。
6. Risks: 🟢 HEAD==origin/main（已 push）。⚠️ KG pilot docx = DRAFT 草擬本，16 軟性覆核 issues（fabricated 鋪墊句 / distorted-number）未修、watermark「概以原文為準」，待 Leonard 核 `QC_VERIFY_ISSUES.md`。⚠️ KG rewrite Workflow 中途撞 account session limit（resumeFromRunId 救回）。Supabase 15,109 零接觸。
7. commits（**已 push origin/main**）: `ec01e1b`(④)→`5dde30f`(①)→`51b6df2`(②)→收尾 gov。
   - QC：backend tsc/check+build exit 0；真 OpenAI 本機 live e2e（15 域 / KG auto-detect 單域 / ① A-D + regression）；browser-verify（範本下載 幼稚園 filter+卡、② 5 tab、全 0 console error）；docx 4/4 well-formed。
