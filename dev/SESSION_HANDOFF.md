...
2. **Option C — CB-3 北極星全覆蓋（next 問 Leonard 排）**：74 無標記源（含 sag_2025_11/g06/g04/g26/g25 高流量、Leonard 測試 query 命中）重抽取頁標記 + 外科式 replace（可重用 `dev/cb3_b2_pagecarry_migrate.py` driver，必守 §E.14 完整 reuse pattern）。HTML-landing 源永無 `#page=N`（結構天花板）。Option B（39 源）已完成生產 live。
2b. **CB-3 收尾 backlog（低優先，非生產影響）**：local `wiki_index.json` vs Supabase 對 39 源 reconcile；build_wiki_index hash-dedup vs live 語料不齊（latent corpus-consistency）。
...
> ✅ **S119 完成**：(1) Channel-B-only 搜尋 surface Phase 1 promote（5 前端 surface 移除 A/AB；檔 dormant；契約零接觸；§3 正常流程；QC PASS；渲染待 browser-verify）。(2) **CB-3 Option B 全做完生產 live**：診斷→B-1 page-carry→B-2 外科 per-source replace 39 源（25 首輪+14 復原；stat 409 incident 修 dedup 復原，§8 固化 §E.14）；live smoke 頁碼出+相關度健康。剩 Option C（74 無標記源）。
...
4. Pending:
   - Phase 1 渲染待 Leonard browser-verify（GitHub Pages）。**Option C**（74 無標記源含 sag/g06/g26 高流量，達北極星全覆蓋）— Leonard 排。local↔Supabase reconcile + build_wiki_index hash-dedup latent（低優先 backlog）。Q4 deferred；Stage-2 closed。
5. Next priorities (max 3 — 詳見 Open Priorities)：
   - 問 Leonard：(a) Phase 1 browser-verify OK？ (b) CB-3 推唔推 Option C（74 無標記源）？
...
6. Risks / blockers:
   ...
   - Phase 1 渲染未 browser-verify（CDN SPA 依 §D.7 交 Leonard；勿宣稱 browser-tested）。檔案 dormant 非刪（q.html/A·AB/backend endpoint 可逆勿清）。
...
7. **Session 進行中（非 closeout — Leonard 未表示「收工」）**：Phase 1 promote+QC+commit+push 完成；CB-3 Option B（B-1+B-2，含 incident 復原）全完成生產 live + commit+push。待 Leonard：Phase 1 browser-verify + 排 Option C。
