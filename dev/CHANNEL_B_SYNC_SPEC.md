...
| Q4 Phase 2 — K1 端 build | ⏳ 待 review 後另開 | 實作 §3/§4 兩個唯讀端點 + sync key（additive、reversible；見 §9） |
| Q4 Phase 2 — 下游 build | ⏳ 跨 repo、Leonard 主導 | 下游自家索引 + 同步迴圈（§5 informational） |

**點解揀 Incremental sync** — 依下游 profile（**online 長駐 service ＋ 自有向量庫 ＋ 近乎即時 freshness**）：export 快照 ❌ 與近即時衝突；pure query-time API ⚠️ 每 query 綁 onrender free-tier（冷啟 ~30s + 10 req/min/IP + 無 SLA）= 長駐 consumer 嘅脆點；**incremental sync ✅✅** 下游拉細 delta、維護自家 index、查詢本地做（唔依賴 K1 free-tier），正正用到下游「自有向量庫」。
...
─────────────────────                        ──────────────────────────────────────────
  本地 Channel B index   ── ① GET manifest ──▶  anon REST select=id,source_id,hash
  （自家 pgvector 等）    ◀── 細 manifest ─────   （+ source_aliases、ingest guard）
...
```json
{
  ...
  "chunks": [
    { "id": "vault_stat_enrolment_2012_ebc2728957012532", "source_id": "stat_enrolment_2012", "hash": "ebc2728957012532" }
  ],
  "next_cursor": null
}
```
...
- **近乎即時**：縮短 poll 間隔（建議 2–5 分鐘）；惟**有效新鮮度 = max(poll 間隔, manifest cache TTL)**——poll 快過 cache TTL 不會更 fresh。
...
- **Downstream-scoped read key**：header `X-Sync-Key` 比對 env `CHANNEL_B_SYNC_KEY`（**唔落 dev/* 檔，存 onrender env / OS secret；錯誤路徑亦不可 log key / header 值**）。
...
- **限流（不可豁免）**：sync 端點**不套** public POST 10/min（嗰個會夾死正常 sync），但**必須有自己嘅限流**：建議 60 req/min **＋ 每日 chunk/bytes 預算上限**（防全庫 exfil——60/min 仍可 ~22 個 batch 拉走全庫；故需 daily budget，非淨 req/min）。
...
- **兩條路（決策點 #4，下游揀）**：
  1. **對齊 3-small（建議）**：食 K1 送嘅 `embedding`（chunk 零重嵌），query 用 `text-embedding-3-small`。最省最快。
  2. **下游用第二 model**：`include_embedding:false` 慳頻寬、用 `text` 自家全量重嵌，query 同一自家 model。
...
1. `GET /api/channel-b/manifest` — anon REST `select=id,source_id,hash`；server 快取 + ETag（涵蓋 version+model+排序 id-set）；`ingest_in_progress` guard；`source_aliases`；`include_statistical`；分頁 + 自有 `57014` retry/Range。
...
*v0.2 draft — 2026-06-05 S144（過一輪對抗審核 hardening）。契約開放 Leonard review/iterate；§11 決策定咗 + 你 GO 後另開 K1-side endpoint build task。*
