...
| 產生知識庫品質報告 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && set -a && . backend/.env && set +a && python3 dev/source/qc_report.py --check` | 封版前；改入庫／切片邏輯之後 | 2026-09-15（S226）— overall **ERROR**、**BLOCKER 0**、releaseGate **5/15** 達標（由 4/15）。S226 令 `EVAL_CHUNK_LAYER` 由 `NOT_MEASURED` 轉 **FAIL**（chunk PASS 54／FAIL 112），`NOT_MEASURED` 項數 1 → 0、`NO_UNMEASURED` 轉 MET、ERROR 3 → 4，`qc_report.json` 逐行 diff 只有 12 加 13 減、其餘檢查零變動。⚠️ 那 +1 是量度覆蓋率不是質素。`EVAL_LATEST` 現量度自 `2026-09-15_s226_route_first_before.json`（FAIL 47）——**選檔仍揀旗標關閉側，見 HANDOFF Risks 9** |
...
| 檢索準確度量度 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && python3 dev/_s213_eval_metrics.py --run <run.json> --gold dev/_s213_gold_all.json` | 任何檢索或排序改動的 before→after | 2026-09-04 (S213) — Source R@1 0.364 / Chunk R@5 0.273。⚠️ **S226 未能執行**：此模組要語料快取 `S213_STORE_CACHE`，預設路徑指向已消失的 session scratchpad，須先 `python3 dev/_s213_recon.py --cache <path>` 重建（全語料唯讀拉取）。S226 的 chunk recall@8（0.349）是繞過本模組、直接由 run 檔的 `rank_of_expected_chunk` 算出，不含 occupancy／per-domain／MRR |
...
| Gold set 量度工具自檢 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && for f in _s213_eval_metrics _s213_run_gold _s213_validate_gold _s213_build_gold_staffing _s213_fix_extract_headers _s213_title_backfill; do python3 dev/$f.py --self-test; done` | 改任何量度邏輯 | 2026-09-15 (S226) — 全部 ALL PASS。S226 在 `_s219_score_before_after.py` 新增 5 條（含來源層／片段層兩條守恆、一條「無斷言題目不得抬高 chunk 分母」），並以注入 `chunk_FAIL` 硬寫 0 證明 3 條會轉紅，還原後 `shasum -a 256` 相同 |
