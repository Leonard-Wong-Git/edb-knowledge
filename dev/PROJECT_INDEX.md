...
| `backend/` | Node.js/TypeScript backend（Channel B 搜尋 API、Render 部署、OpenAI node-fetch + Node 22.x；`backend/.env` = Supabase service key，勿入 git）。**Render 環境變數：`OPENAI_MODEL`（合成器，現為 `gpt-4o-mini`，只可在 dashboard 確認）、`JUDGE_MODEL`（S211 新增，判斷閘專用，程式預設 `gpt-4.1-mini`，Render 無須設定）。、**S226 新增三個量度旗標，生產一律不設**：`MAX_PER_SOURCE`（未設＝現行公式 `max(2, ceil(top_k/3))`；實測放寬到 8 是一換一）、`FEATURE_LEXICAL_RERANK` ＋ `LEXICAL_RERANK_WEIGHT`（詞面重排已 over-fetch 的 40 條候選，預設權重 0.05；**實測兩層同時改善，是目前唯一值得考慮出貨的一項**）、`FAMILY_QUOTA`（把姊妹文件併為一個配額桶；實測不成立）**。**`GET /health` 自 S211 起回報 `commit`（Render 注入嘅 `RENDER_GIT_COMMIT` 前 7 位，本機為 `local`）同 `started_at`——部署是否落地由此判斷，不必再開 dashboard | backend / 部署任務 |
...
| `dev/source/judge_acceptance.py` + `judge_acceptance_cases.json`（frozen 35）+ `judge_transplant_fresh_s202.json`（fresh held-out 10）+ `judge_prompts/`（v3 shipped / v4a·v4b candidates）+ `judge_runs/` | anti-confab judge 驗收 harness。`--cases`/`--cache` override 可獨立計分 held-out 集（frozen 不污染）；**judge 係 LLM 非決定性，任何 verdict 要 ≥3 runs**；**必須明示 `JUDGE_MODEL=gpt-4.1-mini`**（S211 起判官走自己的 `JUDGE_MODEL`，Render 無條目、程式預設即生產值；harness 預設 `gpt-4o-mini` 是舊配置 —— 舊文「量前 dashboard reconfirm `OPENAI_MODEL`」S231 更正為此）。現行基線：`judge_runs/2026-09-24_s231_shipped_41mini.json`（**單次**，未達 ≥3 runs）。findings＝`JUDGE_PROMPT_FINDINGS.md` | judge / synthesis-gate 改動 |
...
| `dev/ingest_s232_kg_ratio_footnote.py` | **S232 最新的單條 curated footnote 入庫範本**（GN10 幼稚園師生比例）。`--self-test` 不寫入：以一條已存 footnote 的向量鎖定 embed 格式（text＋空格＋keywords，cos 1.0000）、並對查詢變體與對照題量餘弦；`--execute` 做 INSPECT before → upsert → INSPECT after。需 `openai`（`python3.13` 或 `/usr/local/bin/python3`）。入庫後生產要重啟才載入，以 `/health` `cache_b.footnote` 核實 | 補一條 curated footnote 時照抄；**寫入內容前先對庫內原文** |
...
| 路由回歸 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && node dev/source/route_regression.mjs` | 改 `TOPIC_KEYWORDS`、`SOURCE_SETS` / 新增路由 | 2026-09-16 (S228) — **63/63 PASS** ＋ 兩條 `SOURCE_MEMBERSHIP` 斷言全綠。S228 新增 9 條（2 條幼稚園搬遷津貼改道 ＋ 7 條對照組，含「學校搬遷津貼」仍歸 `hr_admin`，證明沒有認裸「搬遷津貼」）；改碼前紅測準確紅 3 條 |
| 展開旗標不變式 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s226_knobAB.ts --set EXPANSION_BALANCE=0.5 --ids fin_dls --label smoke` | 改 `expandQuery()` 或三個 `EXPANSION_*` 旗標 | 2026-09-16 (S228) — 開跑前印兩行 `invariant OK`（`MAX_PER_SOURCE` ＋ `EXPANSION_*`），任何一條不成立即拋錯不花一分錢。含一條證明會紅的斷言：balance 0.35 的重複次數必須少於 0.5。**S228 一度另建 `_s228_expansionAB.ts`，實測 `--set` 逐位元組複製得到同一輸入後已退役**（AGENTS §3b）|
| 重排上限探針自檢 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx scripts/_s228_rerankCeiling.ts --self-test` | 引用「cross-encoder 救得到幾多條」這個數字之前 | 2026-09-16 (S228) — 4 條 ALL PASS（含一條守住「本檔不得自行判分」）|
| **門檻分數尺偏移（S229 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s229_scaleOffset.ts <out.json>` | 要引用或改動 `VAULT_LEAD_SCORE` / `min_score` 之前；任何「某個門檻校得啱唔啱」的問題 | 2026-09-20 (S229) — 把 `judge_probe.py` 那 24 條校準 query 分三把尺量：**A 裸＋全庫**（校準尺）／**B 裸＋路由**／**C 展開＋路由**（應用尺），三把都由生產 `searchChannelB` 產生，零重寫。**自我驗證**：A 重現 S195 原數（CLASS_B 上限 0.6321 vs 0.632、CLASS_C 下限 0.6241 vs 0.624）。唯讀、`synthesize:false`、34 個 embedding 呼叫。**必須設 `FEATURE_ROUTE_FIRST_SEARCH=1`**（檔內已寫死，理由同 `_s226_knobAB.ts:139`）|
| **判官 bypass 逐條落點** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s229_leadDetail.ts` | 要斷言「某條查詢的合成閘有無被跳過」之前 | 2026-09-20 (S229) — 印全窗 ＋ 標出 forced prefix 與 `mainSearchLead`，再判 bypass 有無觸發。**存在理由**：`_s229_scaleOffset.ts` 量的是「窗內最高分 vault_extract」，而閘測的是 `mainSearchLead`（overlay 之前、`searchChannelB.ts:1748`），過 0.70 是必要而非充分條件 |
| **合成閘訊號 A/B（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s230_gateSignalAB.ts --self-test` ／ 實跑：同一指令改為 `... scripts/_s230_gateSignalAB.ts <out.json>` | 要改動 vault judge-bypass 的**條件或訊號**之前（門檻數字本身的問題走 `_s229_scaleOffset.ts`） | 2026-09-21 (S230) — 24 條校準 query × 旗標關／開兩個 arm，**bypass 有無發生是觀察出來的**：stub 判官記錄自己有無被問，所以不必像 `_s229_leadDetail.ts` 那樣由窗形推斷 `mainSearchLead`。閘自己會按主鍵讀回領先片段的向量，探針攔那一個 URL 就得到閘實際測的是哪一片。`--self-test` 11 條含一條紅測（預先定案的期望必須能令 run 變紅）。唯讀、無 LLM、每條 query 兩次搜尋 |
| **合成窗詞面分離度（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s230_leadOverlap.ts <gateSignalAB_artifact.json> [--window] [--out <o.json>]` | 要用「詞面有無命中」作為任何閘的訊號之前 | 2026-09-21 (S230) — 用 `textBigrams.ts` 既有的自校準 informative-bigram 機制量查詢與領先片段（預設）或**全窗 top 5**（`--window`）的重疊。**分離度只在「量得到」的案例之間計算**，並另報量不到的條數 —— 查詢無 informative bigram（如 `人工智能初探`）時該檔自己寫明「量不到」不可讀成「零重疊」。唯讀；`--window` 每條 query 一次搜尋 |
| **vault bypass 人口普查（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s230_bypassCensus.ts --self-test` ／ 實跑：`... scripts/_s230_bypassCensus.ts <out.json> [--limit N]` | 要改動 vault judge-bypass 的條件之前（`_s230_gateSignalAB.ts` 只有 24 條校準 query、且無真值，不足以驗收任何閘） | 2026-09-22 (S230) — 對 185 題 gold 逐條量：誰在今日跳過判官，以及**按 gold 真值**該次跳過是否正確（`FALSE_BYPASS` 語料本來答不到／`WRONG_PASSAGE` 答得到但窗內無 expected signature／`SOUND`）。**單 arm 即足夠**：旗標開着令閘按主鍵讀回領先片段，攔那個 URL 就知道閘實測哪一片，而 `bypass_before` 由「領先片段是 vault_extract ＋ 應用尺分數 ≥0.70」推導 —— 此推導已對兩 arm 的 24 條逐條驗證，24/24 與觀察值相同。裸查詢向量取自 `searchChannelB` 本來就會做的 overlay embedding（同一個 fetch wrapper 攔下來），故不增加 embedding 呼叫。唯讀、stub 合成器與 stub 判官、零 LLM。`--self-test` 8 條含一條紅測（查詢無 informative bigram 時候選規則永不阻擋）|
| **判官接手測試（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s230_judgeTakeover.ts --self-test` ／ 實跑需明示批准：`... --run --approved-max-calls=<N> <out.json>` | 要斷言「收緊 bypass 之後用戶還有無答案」之前 | 2026-09-22 (S230) — **本線唯一花外部模型呼叫的探針**，內置 59 次硬上限（Leonard 2026-09-22 批准的批次）。對普查量出的 bypass 人口逐條跑**真判官**，三種結局分開記（保住捷徑／判官答／判官拒），**技術失敗與判決分開** —— `judgeCanAnswer()` 拋錯時 `return true`，一次失敗長得像一次「能」。`--self-test` 11 條含一條紅測（第一版用錯拒答文案判斷「有無答案」，會令整張表反轉）|
| **拒答逐條讀窗（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s230_readDeclines.ts [--chars N]` | 要判斷一次拒答是「損失」還是「正確棄權」之前 —— 這件事量不出來，只能讀 | 2026-09-22 (S230) — 印出判官實際看到的五格窗、gold 的 intent／expected_source／expected_signature，並標出命中。唯讀、`synthesize:false`、零 LLM。**用它捉到本專案評分器與探針的 NFKC 口徑不一致**（全形／半形括號令答案在第 0 格仍被判無答案）|
| **判官模型敏感度（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s230_judgeModelProbe.ts --run` | 要斷言某條判官錯判是「提示問題」還是「模型問題」之前 | 2026-09-22 (S230) — 由生產自己產生提示（stub 判官記錄其參數），再送去不同模型，**零重寫提示、零重建窗**，故差異只可能是模型。硬上限 6 次呼叫。⚠️ 必須設 `FEATURE_VAULT_GATE_RAWVEC=1`，否則目標查詢會被捷徑放行而判官根本不被問（第一版中過此錯）|
| **判官表重新推導（S230 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && python3 dev/_s230_relabel.py --self-test` ／ 實跑：`python3 dev/_s230_relabel.py` | 改正 gold 標籤口徑之後，要重新讀判官結果而**不想再花模型呼叫**時 | 2026-09-22 (S230) — 把已記錄的判官結局接上改正後的標籤重新統計並對照預先定案，**零新呼叫**（每條的結局與標籤無關）。`--self-test` 7 條含一條紅測（標籤改正只可能把列移入 `SOUND`，而 `SOUND` 拒答 ≥2 即不出貨，該紅測守住這條線）|
| 重排上限報告自檢 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && python3 dev/_s228_ceiling_report.py --self-test` | 同上 | 2026-09-16 (S228) — 7 條 ALL PASS（判分沿用 `_s213_run_gold.score_item`）|
| scripts/ 型別閘 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && npm run check` | 改 `backend/scripts/*.ts` 任何一支量度工具 | 2026-09-16 (S228) — exit 0。**S228 前 `npm run check` 完全看不到 `scripts/`**（`tsconfig.json` 的 `include` 只有 `src/**`），現已串 `tsconfig.scripts.json`；紅測：在 `scripts/` 注入型別錯準確轉紅 |
...
| 註腳 lead probe 自檢 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && python3 dev/source/footnote_lead_probe.py --self-test` | 改註腳 lead 邏輯前後；新增 footnote 前後（`--run --pace 6.5` 生產 before／after ＋ `--compare`） | 2026-09-24 (S232) — PASS；生產 before／after 正控 26/27 → 26/27、失去 lead 0 |
...
| **檢索 knob A/B（S226 新增登記）** | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft/backend" && node_modules/.bin/tsx --env-file=.env scripts/_s226_knobAB.ts --set KEY=VALUE [--set …] --label <節><knob>` 然後 `cd "…/Draft" && python3 dev/_s219_score_before_after.py --raw dev/source/eval_runs/<日期>_<label>_before_after.jsonl` | 量度任何檢索 knob 的 before→after（配額、重排、族群桶）；改 `maxPerSource`／排序邏輯之前 | 2026-09-15 (S226) — 三次實測：**`MAX_PER_SOURCE=8`** 來源層 122→118／片段層 58→63（一換一，不出貨）· **`FEATURE_LEXICAL_RERANK=1`** 三點劑量曲線（權重 0.02／0.05／0.10）來源層 122→126／**129**／132、片段層 58→61／**64**／64，全部零退步；**片段層在 0.05 停滯而擾動與判官繞過翻轉隨權重上升，故建議 0.05** · **`FAMILY_QUOTA=1`** 122→121／片段層不變（不成立）。⚠️ **此 harness 的延遲數字不可用**（同題一前一後跑，after 側享受快取）；亦不記錄 routed RPC，故計分器會印 `no-route 185` |
...
| Agent Handoff Kit workspace-health | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && npx --yes @adamchanadam/agent-handoff-kit@latest workspace-health --root .` | **v0.3.66 新增**：唯讀 git 探針（root / branch / commit / worktree / 未提交） | 2026-09-15 (S226) — **CLI 經 `npx` 實際跑得到**，更正 S222–S225「本機未安裝／registry 404」的記載。同節 `closeout-status --root .` 實際執行並第一次報 `blocked`（lifecycle：已完成的 OP① 仍留在 Next Priorities），依其 Blocker 行重生 Open Priorities 後重跑 |
