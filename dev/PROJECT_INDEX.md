...
| 知識庫品質檢查自檢 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && python3 dev/source/qc_report.py --self-test` | 改任何檢查邏輯或基準值 | 2026-09-14 (S224) — ALL PASS（S224 加了兩條 guidelines 漂移紅案，並以注入證明會轉紅） |
| 產生知識庫品質報告 | `cd "/Users/leonard/Downloads/Claude Project/Claude-edb-knowledge/Draft" && set -a && . backend/.env && set +a && python3 dev/source/qc_report.py --check` | 封版前；改入庫／切片邏輯之後 | 2026-09-14 (S224) — overall **ERROR**、**BLOCKER 0**（S224 修好 `FREEZE_CONTRACT` 基準值過時）、releaseGate 3/15 達標 |
