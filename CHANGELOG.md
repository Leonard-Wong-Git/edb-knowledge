...
### Fixed
- **「幼稚園每班師生比例是多少」曾答「至少 1:30、某些情況可達 1:14」。** 檢索窗內有三條幼稚園 footnote（每班最少一名教師當值、每班不超過 30 人、1:14 只適用於特殊幼兒中心），卻沒有真正的比例；載有比例的原文片段因查詢被路由到課程指引而從未取回。新增一條 curated footnote `footnote_fn_kg_teacher_pupil_ratio`（掛 `kg_admin_guide_2026`），內容逐字核對自庫內原文：《幼稚園行政手冊》5.1.1.1 —— 參加幼稚園教育計劃的幼稚園整體師生比例由 1:15（校長計算在內）提升至 1:11（校長不計算在內）；《學前機構辦學手冊》附錄 20（教育局通告第 26/2003 號）—— 每 15 名在場學生須有一名教師在場當值，每班最少一名教師。並明文指出 30 人是課室容納上限、1:14 只適用於特殊幼兒中心。全庫 17,006 → **17,007**。
...
### Fixed
- **清走 `kgecg_2017` 的 108 條片段。** 它與 `g29` 是同一份《幼稚園教育課程指引（2017）》的兩個 EDB 版式（`TC_KGECG_2017.pdf` vs `KGECG-TC-2017.pdf`）。全庫 17,112 → **17,004**；`g29` 那 107 條未受影響，現在是這份指引在庫內的唯一一份。**唯一的 `ZOMBIE` 由 1 歸 0。**

### Note
- **推翻了 S222 收回過的判斷，理由是 S222 當時沒有的證據。** S222 量得「逐條片段文本 0 條相同、互相只覆蓋約 84.5%」，據此收回刪除建議。但那次比較的是**兩批 live chunk 的文本**，而它們出自不同管道（`g29` 經 `repage_pdfs` page-carry、`kgecg_2017` 用 `fetch_extract` 無 page marker）與不同切割邊界 —— **量到的是管道，不是文件**。
- **重量方法（S222 自己在 SAG 那邊立下的那一套）**：同一個抽取器（PyMuPDF）、剝離 page marker／空白／數字，30 字視窗掃描，**對照組自己對自己 0 miss**。結果：兩份都是 **108 頁**、`U+FFFD` 都是 **0**、字元差 **0.9%**；視窗落差 **對稱**（11.7% vs 11.3%）—— 對稱正是次序噪音的特徵，而非一方多帶內容。再把 383 個落差視窗拆半：**88.5% 有一半在對方文件內**（接合處產物）、3.7% 兩半都在（純次序），**真正獨有只佔全文 0.91%**，而那 30 條全是目錄與章節簡介的接合文字。
- **教訓（與 S222 SAG 那條同一家族）**：比較兩份文件必須用同一個抽取器**並設對照組**；比較兩批**已入庫片段**更危險 —— 那等於同時比較了抽取器、切割邊界與閱讀次序三樣東西。證據：`dev/source/eval_runs/2026-09-14_s223_kgecg_g29_overlap.json`。
- **`kgecg_2017` 這個 id 不要重新入庫**：`checklists_bundle.json` 仍有 47 條 checklist item 以它作標題／連結標籤，`TC_KGECG_2017.pdf` 仍然 HTTP 200 且與 `g29` 那份頁碼一致，所以那些連結不受影響 —— 它只是不再服務 Channel B 片段。
...
### Fixed
把 176 條瀏覽條目逐條對照它們片段實際攜帶的標題與連結，26 條對不上。扣除良性的（瀏覽連 landing page、片段連深層 PDF，屬設計；書名號等排版差異），餘下按同一裁示處理 —— **保留有正確片段那個 id、改正它的描述，移除零片段的重複條目**。

- **六條標籤改正為它實際服務的內容**：
  - `g36`「（小一至**中三**）(**2002**)／HTML」→「（小一至**中六**）(**2017**)／PDF」（179 條片段全部是 `SEKLACG_CHI_2017.pdf`）—— **與 g37 完全同形**。
  - `g33`「英國語文教育課程指引（中一至中六）(**2007**)」→「英文課程及評估指引（中四至中六）(**2021**)」（421 條）。
  - `g22` 年份 **2021 → 2010**（59 條是 2010 年那本安全手冊）。
  - `nat_sec_edu`「國家安全教育課程資源（2024）」→「國家安全教育課程框架（**2025**）」（39 條）。
  - `chi_edu_curr_docs`／`apl_curr_docs` 兩個 hub 名 → 它們實際承載的 2017 年指引本身。
- **移除五條零片段的重複瀏覽條目**（其文件已由另一個 id 服務，用戶原本會見到同一份文件上榜兩次）：`sci_kla_guide_2017`→`g36`、`cle_kla_guide_2017`→`chi_edu_curr_docs`、`apl_ca_guide_2017`→`apl_curr_docs`、`pshe_kla_guide_2017`→`g35`、`g31`→`eng_pri_guide_2025`。短名對照同步清理。
- in-app 瀏覽庫 **176 → 171 份**；`source_registry` 七條同步。

### Note
- 這一批**推翻了先前「41 個可瀏覽指引搜尋不到 = 23% 缺口」的講法**：當中至少 4 個根本不是缺口，內容早已入庫，只是掛在另一個 id 底下。
- `guidelines.json`（公開端點 158 份）未同步，依凍結合約零接觸。

---

## [瀏覽清單對正] — 2026-09-13 — 同一份藝術教育指引上榜兩次，其中一個標籤講錯年份（S222）

> 平台版本 **v3.3.3 → v3.3.4**（`app.html` 有改動）。凍結合約零接觸（`knowledge.json` `_meta` 2.3.0 · facts 455 · `guidelines.json` 2.6.1）。

### Fixed
- **「📚EDB指引」瀏覽清單講 2002，搜尋答的是 2017。** `g37` 的條目寫住「藝術教育學習領域課程指引（小一至**中三**）(**2002**)」、格式 HTML、連結指去 arts-edu 索引頁；但它那 116 條片段的標題是「（**2017**）」、連結指去 `AE_KLACG__Chi___2017.pdf`，文本提及「中六」11 次。`url_primary` 一直都是 2017 那份 —— 錯的是標題與 `version_label`，而錯的那半正正是用戶見到的那半。屬 S194 `ict_sss_2021` 那個「200-但-錯檔」家族。`app.html` 與 `source_registry` 兩處已對正為 2017 版。**庫內從來沒有 2002 版**（S213 gold case `cur_arts_guide_2002_missing` 已證），故此更名不會遮蓋任何東西。
- **同一份文件在瀏覽清單上榜兩次。** `arts_kla_guide_2017` 與 `g37` 服務同一份 2017 PDF：116 條片段之中 **115 條文本逐條相同**，內容 **99.9%** 重疊（唯一例外是一頁鳴謝名單）。`g37` 那份標題與連結都正確，`arts_kla_guide_2017` 那份則是代號標題兼零連結。已移除後者的瀏覽條目與短名對照，registry 標記 `deprecated`；in-app 瀏覽庫 **177 → 176 份**。
  - 其 116 條片段**未清走**，屬另一次生產寫入，待批。
  - `guidelines.json`（公開端點，158 份）仍同時列住兩個 id，**依凍結合約不予改動**，故兩份清單對這一項的分歧為已知且刻意。
...
## [內容準確性] — 2026-09-01 — 班數編制答案改以全日制為準（S211）

> 平台版本維持 **v3.3.2**（純後端）。語料未動——半日制條文仍完整留在庫內。

### Changed
- **編制查詢預設只答全日制。** 上一則的詞彙層檢索會把同一班數的全日制與半日制兩行一併置前，答案因而寫成「全日制 5 名／半日制 4 名」，讀來像兩個並行的現行選項。實務上資助及官立小學已無半日制授課（Leonard，2026-09-01；教育局「小學全日制」頁面以全日制為政策方向，並列出四百多所全日制小學），這樣並排會令學校對照一個自己並不採用的安排去計算編制。
- **資料未刪。** 教育局該份文件仍同時刊載兩張表（全日制第 1–2 頁、半日制第 3 頁起，同為「由 2022/23 學年起生效」），半日制數字本身亦沒有錯，故條文照留庫內；問題只在於它不是一條未指明問題的答案。查詢中出現「半日制」時仍會如實作答——實測「半日制 12 班小學有幾多個學位教師」回覆校長 1 名、副校長 1 名、小學學位教師 4 名、助理 13 名。

---

## [檢索修正] — 2026-09-01 — 班數編制查詢：詞彙層檢索 ＋ 逐行切片（S211）

> 平台版本維持 **v3.3.2**。Supabase `staff_est_pri` 81 → **85** 片段，全庫 17,593 → **17,597**（display-sync 7 面）。凍結合約零接觸（`_meta.version` **2.3.0**、facts **455**、`guidelines.json` **2.6.1 / 158**）。

### Fixed
- **班數編制查詢會答出確定而錯誤的數字。** 改動前，「全日制資助小學核准開辦12班的教學人員編制」得到「校長1名、**不設副校長**、小學學位教師 **2** 名、助理 **7** 名、合計 **10** 名」；原文為校長1名（二級小學校長）、副校長 **1** 名、小學學位教師 **5** 名、助理 **14** 名、合計 **21** 名。系統取了同一張表的另一行，冠以「12 班」之名，且毫無保留語氣。判斷閘攔不住——該表每一行在它眼中都是同一語域的完整句子，這正是 S195 所述的限制。
  - **成因一（切片）**：`chunk_max_chars=160` 於 S204 已設，用意就是一行一片段，但 `overlap=60` 會把上一段末 60 字接到下一段開頭，而每行約 98 字，於是 81 個片段有 **74 個由半行開始**，該半行沒有班數標頭。`expand_vault.py` 新增 `chunk_overlap` 覆寫（接受 0，故另設一個讀取函式）；本源設為 0、`chunk_max_chars=110`，重新入庫後半行開頭降至 **2/85**。
  - **成因二（檢索）**：僅靠切片仍然答不對。36 行近乎相同的編制句彼此相差僅數個百分點，稠密向量無法在「12」與「7」之間對位——該行對其自身問題的 cosine 只有 0.5923，排不進前列；行與行分開之後，合成器改為由 6、7、8 班的數值**內插**。新增 `searchEstablishmentRows()`：全流程中唯一不經相似度的查詢，以 `N 班的教學人員編制` 作字面比對（故不會命中「12至17班」區間或頁碼），命中即強制置前。同一個 N 會命中兩行（全日制與半日制），對一條沒有指明的問題而言，這正是誠實的答案。
  - 實測（本機真編譯產物）：「12 班小學有幾多個學位教師」→ 全日制 5 名學位教師、助理 14 名（合計 21）；半日制 4 名、助理 13 名（合計 19）。「24 班小學有幾多個副校長」→ 3 名。皆與原文逐字相符。
- **兩處過度觸發已收窄。** `staffing` 路由新增的「數字＋班」單獨使用過寬：實測「小一派位第 1 班點分」與「中一 5 班嘅課程指引」都會被搶去，答案變成講教學人員編制。改為以兩個 lookahead 要求「數字＋班」與人事字眼（教師／教員／教學人員／編制／校長／副校長／主任／人手）同時出現。詞彙層 overlay 的觸發條件則直接跟隨 `staffing` 路由，不另寫一次判斷。改動後「小一派位第 1 班」「中一 5 班課程指引」「24 班學校要幾多間課室」所取得的 `staff_est_pri` 片段皆為 **0**；路由回歸 **33/33 PASS**。

---

## [檢索改善] — 2026-09-01 — 判斷閘改用獨立模型 gpt-4.1-mini（S211）

> 平台版本維持 **v3.3.2**（純後端）。凍結合約零接觸。

### Changed
- **相關性判斷閘改用 `gpt-4.1-mini`**，與合成器所用的 `OPENAI_MODEL` 分離（新環境變數 `JUDGE_MODEL`，程式預設 `gpt-4.1-mini`；不需改動 Render 環境設定）。凍結驗收集實測（同一組 case、同一份 prompt）：主集 **31/33 對 31/33** 打平、answer half **12/12** 打平、decline half **19/21** 打平且**為完全相同的兩個 false answer（D01、GN10），無新增虛答**；連 bare-noun 兩題計則為 **33/35 對 31/35**——即 `S01_ai_intro_bare_noun` 與 `S02_ict_guide_bare_noun` 兩題由誤拒轉為正確作答。此外它答得出本輪那條資歷題，而該題無論如何改寫判斷提示、`gpt-4o-mini` 與 `gpt-4o` 均拒答。
- **只換判斷閘、不換合成器**：合成品質未在此測量，而判斷閘是便宜的一次呼叫（實測約 1,910 input tokens、1 output token），故此改動每條查詢約增 **US$0.0005**（每月一千條查詢約 **US$0.48**）。若連合成一併改用該模型，成本約為現時 2.7 倍，而無任何實測依據支持。
