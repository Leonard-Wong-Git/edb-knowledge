...
## 6. guidelines.json 格式（S140：39 → 148 全集投影）

> S140（2026-06-03）：公開端點由 39 精選子集擴為 app 內庫 registry 全集投影 **148** 份（剔 7 統計表 / 1 申請表 / 1 壞連結；含 landing-curate 新增 9 真‧KLA/課程指引全文 PDF）。schema 不變；篩選方法（按 topic 取 `guidelines[topic]`）不變，只係每桶文件數增加（curriculum 桶 ~25→132）。建議 Circular System 注入時對 curriculum 類通告做數量上限或相關性排序。
